...
```{r avaliacao EI/EE, message=F, echo=F, fig.height = 6 ,fig.width = 9}
...
car::scatterplotMatrix(~k_perc + m + log(erro), 
                        reg.line = F, smoother = F,
                        df_sd_k)
```

fig. 3. % da Chuva de propagulos . ~ Log(par_calculado) + m + log(diferença entre o quantil calculado pelas equações (sd_k) e o resultado da função quantile para o quantile)
...
Agora vamos avaliar a conversão de parâmetros sd_k da simulação coalescente para o respectivo m do modelo de campo médio. Vou olhar os dados antes de corrigir o m (eq.0')

```{r echo=F}
df_m <- df_temp %>% filter(kernel_percentil != "campo_medio" & par.class != "theta")  %>%     
