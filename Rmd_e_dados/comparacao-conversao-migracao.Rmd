...
df_resultados[,1:7] %>% # sd_k EE
  ...
  arrange(.,SiteCode,kernel_percentil) %>% # colocando os dados em ordem
  mutate(p_class = cut(p,12)) %>% # criando p_class
  #grafico
  ggplot(aes(x=kernel_percentil,y=log(par.value),shape = par.method,color = SiteCode)) + geom_point(size=2.5) + 
...
figura 2. log(sd_k) ~ (%k + campo medio) + (~p_class); p_class = cut(p,12). Os pontos estão coloridos segundo o Site e a forma dos pontos segundo o método de obtenção do valor de sd_k
...
figura 3. m ~ sd_k + %k + método de calculo. em x = desvio padrão da função de dispersão (sd_k), y = m; os paineis estão divididos pelas respectivas porcentagens de propágulos que permanecem até $l_{cel}$ metros da árvore progenitora.


```{r fig 3, fig.height=4,fig.width=4}
df_m %>% 
...
figura 4. difença no valor calculado pelos dois métodos e a razão L / (sd_k / sqrt(2) )
...
Figura 6A. m ~ p + (~kernel_percentil)
...
Figura 6B. m' ~ p + (~kernel_percentil)
...
figura 7. $\theta$ ~ p + (~kernel_percentil). Cada painel corresponde ao percentil de propágulos que permanece até $l_{cel}$ metros da planta progenitora. O último painel 'campo_medio' corresponde ao $\theta$ obtido pelo ajuste da formula de Etienne (2005) às SADs observadas.
...
```
figura 8. $\theta$ ~ kernel_percentil + (~p_class). p_class = cut(p,12). Os pontos e linhas estão coloridos pelo Site. 
...
Figura 10. U ~ kernel_percentil + (~p_class); p_class = cut(p,12). Os pontos e linhas estão coloridos pelo Site.
