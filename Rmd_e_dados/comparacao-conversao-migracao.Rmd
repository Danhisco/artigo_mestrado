...
```{r avaliacao EI/EE, message=F, echo=F, fig.height = 4.5 ,fig.width = 5.75}
...
df_sd_k %>% ggplot(aes(x = m, y = log(par.value), colour = par.method)) + geom_point() + geom_smooth(method = "loess",se=F) +
  labs(y="log sd_k (desvio padrão da função de dispersão", x='m') + ggtitle(label="m -> sd_k")
...
fig. 3.k_perc = porcentagem estimada a partir do sd_k calculado, m = parêmetro modelo neutro, log.erro = log( diff (par.value[sd_k calculado] - quantile(.,k_perc,l_cel)) )
...
fig 5. difença no valor calculado pelos dois métodos e a correspondente porcentagem de propágulos.
