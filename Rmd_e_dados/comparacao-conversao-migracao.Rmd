...
l_p[[1]] <- ggplot(df_sd_k,aes(x = m, y = par.value, colour = par.method)) + 
  ...
  labs(y="sd_k (desvio padrão da função de dispersão)", x='m') + ggtitle(label="m -> sd_k") + theme(legend.position="none")
l_p[[2]] <- ggplot(df_sd_k,aes(x = m, y = log(par.value), colour = par.method)) + 
  geom_point() + geom_smooth(method = "loess",se=F) +
  labs(y="log sd_k (desvio padrão da função de dispersão)", x='m') + ggtitle(label="m -> log.sd_k")
