...
df_temp <- ddply(df_resultados,c("SiteCode","kernel_percentil"), 
...
df_sd_k <- df_temp %>% filter(kernel_percentil == "campo_medio" & par.class != "U") # %>% 
...
df_sd_k %>% ggplot(aes(x = m, y = log(par.value), colour = par.method)) + geom_point() + geom_smooth(method = "loess",se=F)
```
figura 1. Log(sd_k) ~ m * metodo de conversao. CaCL = método Coutinho apud C&L09; CL = equação 2 C&L09. A linha é uma aproximação 'loess' da função geom_smooth (ggplot2).
...
```
<!--
figura 3. Contra prova do percentil calculado para os valores de sd_k calculados a partir de m. No ponto os dois métodos estão misturados
-->
```{r echo=FALSE}
# summary(lm(par.value ~ q_perc, data=df_sd_k))
```

Eu gostaria de tirar a contra prova do valor de percentil estimado. Para isso eu tentei utilizar a função quantile com o percentil estimado pela função anterior, mas parece que eles são muito diferentes. Não sei se cometi algum erro. 


Olhando para as estimativas do percentil me parece que faz sentido com os valores estimados em par.value, mesmo que exista um erro consistente na estimativa do par.value. Pensando que em um modelo de campo médio o espaço é desconsiderado da dinâmica.

```{r}

car::scatterplotMatrix(~log(par.value) + m + log(erro), 
                        reg.line = F, smoother = F,
                        df_sd_k)
```
...
```{r funcoes chuva de propagulos, echo = FALSE}
library(rmutil)
