...
- de tif para png ajustado (figura cujo número de pixels = DA_obs * área da paisagem)

```{r tif para png ajustado}
# tabulação dos arquivos de imagem para uso de ferramentos de acoplamento de dados #
...
- png com resolução ajustada para matriz de paisagem onde ocorre a simulação do modelo neutro espacialmente explícito
- a matriz de paisagem apresenta 3 tipos de células: de não habitat; de habitat que não está dentro da área local; e de habitat que está dentro da área da comunidade local.
- para isso é necessário converter os valores dos pixels do png, que correspondem a porcentagem de cobertura original no pixel original (de 30m2), em ou habitat ou não habitat;
- estabelecemos que aqueles pixels que originalmente correspondem à pelo menos 70% de cobertura serão denominados de unidades de habitat e os que estiverem abaixo desse valor de unidade de não habitat
- no centro da matriz de paisagem N unidades de habitat, então, foram convertidas em unidades de habitat da comunidade local.


```{r png ajustado para matriz trinaria}
...
- a cobertura vegetal (p) foi definida como 1 - a proporção de unidades de não habitat na paisagem
...
[dado a importância da limitação à dispersão a teoria neutra é uma teoria espacial(Alonso et al. 2006)]
- 80 SADs amostradas em diferentes locais da mata atlântica que são organizados pela porcentagem de cobertura vegetal (p)
- dois modelos neutros que diferem em seus pressupostos auxiliares de implementação da limitação à dispersão (MNEI e MNEE)
- campo de valores da distância média de dispersão (d) varendo dois extremos: i) ausência do agente dispersor e ii) em paisagem não perturbada
- a partir de d estima-se U utilizando o método semi-análitico desenvolvido por Rosindell et al. (2008) [onde é necessário informar a riqueza observada (S) e matriz de paisagem (que por sua vez é um raster corrigido pela densidade (número de indivíduos/1 ha, DA) e que apresenta uma área central correspondendo a área amostrada ( A_{J} = l_J^2)]
- são simuladas 20 árvores genealógicas para cada sítio de amostragem e d; utilizamos o U médio para simular as SADs
- para MNEI: i) d -> m -> m_ -> I]; ii) U -> theta] --> 100 SADs réplicas
- para MNEE: i) d -> sd]; ii) U] --> 100 SADs réplicas
...
Para manter a coerente com o modelo de espaço explícito vamos utilizar a formula de amostragem desenvolvida por Etienne 2005, derivada a partir da abordagem coalescente. Para isso utilizo funções do pacote 'untb' (que precisam da biblioteca PARI/GD), que estimam os parâmetros do modelo, e uma função do pacote 'GUILDS', que simula SADs segundo o mesmo modelo.
...
# Futuramente vou adicionar valores de k para explorar a .
...
df_par.Etienne %<>% filter(k!="EI") %>% dplyr::select(SiteCode, k, GOF,U, p, J, S, DA, d,theta_,m_) %>% mutate(I = m_ * (J-1)/(1-m_)) %>% droplevels()
...
saveRDS(df_resultados,file = "/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_resultados.Rdata")
```
...
f_SAD.Etienne <- function(df_par){
  l_SAD.rep <- dlply(df_par,"k",function(X) replicate(100,generate.ESF(theta = X$theta_, I = X$I, J = X$J)),.parallel = TRUE)
}
#modificar para uma versão que paralelizar, talvez utilizar um llply %>% unlist()
# registerDoMC(3) # tentatida de paralelizar 1 [TESTAR]
...
```{r GOF_ S_m S_sd estatisticas resumo}
# df_resultados <- readRDS("/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_resultados.Rdata")
# funcao para aplicar o teste de Kolmogorov-Smirnov
# retorna: data frame 1 X 2 [,c("maior dist. entre as SADs acumuladas","p valor associado ao teste")]
# input: SAD_obs e 

### teste ###
# para um SiteCode; função vai varrer todos os sitecodes #

# data frame coms as informações #
# df_data <- df_resultados$data[[1]]
# df_teste %>% str

# vetor de abundância observado #
# SAD.obs <- df_resultados$SAD_obs[[1]]$N
# SAD_obs.teste %>% str

# lista com os vetores de abundância replicas para cada k #
# l_SAD.rep <- df_resultados$SAD_rep[[1]]
# SAD_rep.teste %>% class
# l_SAD_rep.teste %>% length
# todos os vetores em SAD_rep.teste irão ser comparados com o único vetor de SAD_obs.teste

# função que era para usar no ddply :-(
f_GOF.S_ <- function(df_data, SAD.obs, l_SAD.rep){
  # teste de Kolmogoro-Smirnov #
  f_ks.test0 <- function(par1.SAD_obs = SAD.obs, par2.SAD_sim){
      a <- suppressWarnings(ks.test(x = par1.SAD_obs,
                                    y = par2.SAD_sim))
      a <- data.frame(KS.D = a$statistic, KS.p = a$p.value)
  }  
  # teste de KS e armazenamento #
  df_ks.teste <- ldply(l_SAD.rep,function(X) ldply(X,function(Y) f_ks.test0(par2.SAD_sim = Y)) )
  # estatística resumo do teste de KS #
  # GOF, número de vezes em que não foi possível refutar a hipótese nula de que os dois vetores de abund. são amostras de uma mesma dist. #
  df_ks.teste %<>% inner_join(x=ddply(.,"k",summarise, KS.D_mean = mean(KS.D), KS.D_sd = sd(KS.D),KS.p_mean = mean(KS.p), KS.p_sd = sd(KS.p)),
                              y=ddply(.,"k",function(x) nrow(x[x$KS.p >= 0.05,])),
                              by="k")
  names(df_ks.teste)[6] <- "GOF_"
  # juntando com as info de cada SiteCode #
  df_ks.teste %<>% inner_join(x=.,y=df_data,by="k")
  # S_m e S_sd #
  df_S <- ldply(l_SAD.rep,function(X) ldply(X,function(Y) length(Y)) )
  df_S %<>% ddply(.,"k",summarise, S_m = mean(V1), S_sd = sd(V1) )
  # juntando com df_ks.test #
  df_ks.teste %<>% inner_join(x.,y=df_S,by="k")
  df_ks.teste %<>% select(k,p,J,S,DA,d,theta_,m_,I,GOF,GOF_,S_m,S_sd,KS.D_mean,KS.D_sd,KS.p_mean,KS.p_sd)
  return(df_ks.teste)
}

# armazenando #

# Danilo 14jun: não sei o motivo de não estar funcionando; 

# df_resultados0 <- df_resultados
# registerDoMC(3)
# df_teste <- df_resultados %>% dlply(.,"SiteCode", function(X) f_GOF.S_(df_data = X$data, SAD.obs = X$SAD_obs$N, l_SAD.rep = X$SAD_rep))
# df_resultados %<>% mutate(df_ks.teste = map(.f = function(X) f_GOF.S_(df_data = X$data, SAD.obs = X$SAD_obs$N, l_SAD.rep = X$SAD_rep)) )  
  
# Danilo 15jun: vou usar um for mesmo;

# df_ para armazenar
df_ks.teste <- vector("list",nrow(df_resultados))
# paralelizando o ldply #
# mudar para llply e paralelizar #
registerDoMC(3)
for(i in 1:nrow(df_resultados)){
# dados
  df_data <- df_resultados$data[[i]]
  SAD.obs <- df_resultados$SAD_obs[[i]]$N
  l_SAD.rep <- df_resultados$SAD_rep[[i]]
# teste de Kolmogoro-Smirnov #
  f_ks.test0 <- function(par1.SAD_obs = SAD.obs, par2.SAD_sim){
      a <- suppressWarnings(ks.test(x = par1.SAD_obs,
                                    y = par2.SAD_sim))
  ...
  # teste de KS e armazenamento #
  df_ks.teste[[i]] <- ldply(l_SAD.rep,function(X) ldply(X,function(Y) f_ks.test0(par2.SAD_sim = Y)), .parallel = TRUE )
  # estatística resumo do teste de KS #
  # GOF, número de vezes em que não foi possível refutar a hipótese nula de que os dois vetores de abund. são amostras de uma mesma dist. #
  df_ks.teste[[i]] %<>% inner_join(x=ddply(.,"k",summarise, KS.D_mean = mean(KS.D), KS.D_sd = sd(KS.D),KS.p_mean = mean(KS.p), KS.p_sd = sd(KS.p)),
                                   y=ddply(.,"k",function(x) nrow(x[x$KS.p >= 0.05,])),
                                   by="k")
  names(df_ks.teste[[i]])[6] <- "GOF_"
  # juntando com as info de cada SiteCode #
  df_ks.teste[[i]] %<>% inner_join(x=.,y=df_data,by="k")
  # S_m e S_sd #
  df_S <- ldply(l_SAD.rep,function(X) ldply(X,function(Y) length(Y)) )
  df_S %<>% ddply(.,"k",summarise, S_m = mean(V1), S_sd = sd(V1) )
  # juntando com df_ks.test #
  df_ks.teste[[i]] %<>% inner_join(x.,y=df_S,by="k")
  df_ks.teste[[i]] %<>% dplyr::select(k,p,J,S,DA,d,theta_,m_,I,U,GOF,GOF_,S_m,S_sd,KS.D_mean,KS.D_sd,KS.p_mean,KS.p_sd)
}
#adicionando ao df_resultados
df_resultados %<>% add_column(.,df_ks.teste)
# armazenando em disco
df_resultados0 <- df_resultados
saveRDS(df_resultados0,file = "/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_resultados.Rdata")
# df_resultados <- readRDS(file = "/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_resultados.Rdata")

#salvando em csv para upar para o git.
df_resultados %<>% dplyr::select(SiteCode,df_ks.teste) %>% unnest() %>% as.data.frame()
write_csv(df_resultados,path = "/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_resultados.csv")
```
...
```{r auditoria dados Etienne, echo=F,include=F}
...
```{r auditoria par_EE -> MNEI}
df_resultados <- read_csv("/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_resultados.csv")
# x11()
car::scatterplotMatrix(~ GOF_ + KS.p_mean + KS.p_sd + KS.D_mean + KS.D_sd + S_m + S_sd, 
                       reg.line = "", smoother = "" ,
                       data=df_resultados, main = "Avaliação Visual de GOF e estats resumo modelo EI")

```

figura X. Auditoria dos valores de GOF_. Quanto maior o valor de GOF maior a média do p valor associado ao teste de KS.

```{r S_m k e GOF}
l_p <- vector("list",length = 2)
l_p[[1]] <- df_resultados %>% ggplot(aes(x=k,y=S_m,group=k)) + geom_jitter() + geom_boxplot()
l_p[[2]] <- df_resultados %>% ggplot(aes(x=S_m,y=GOF)) + geom_point() + geom_smooth(se=F) + facet_wrap(~k,scales="free") + coord_flip()
do.call("grid.arrange",c(l_p,ncol=2))
```

figura X. Riqueza média das réplicas (S_m), proporção de propágulos na área da copa da planta progenitora (k), e GOF
 
Diria que está autocoerente: GOF é proporcional à KS.p_mean e KS.p_mean é inversamente proporcional à KS.D_mean. 

-> análise exploratória de GOF:
-melt de GOF e GOF_
-juntar S_var e S_mean de MNEE para comparação.

 
```{r df_resultados final}
df_resultados %<>% gather(MN,GOF,GOF:GOF_)
df_resultados$MN[df_resultados$MN=="GOF"] <- "EE"
df_resultados$MN[df_resultados$MN=="GOF_"] <- "EI"
df_resultados$MN %<>% factor()
df_resultados %<>% select(SiteCode,k,MN,U,GOF,p,d,m_,I,J,S,DA,S_m,S_sd,KS.D_mean,KS.D_sd,KS.p_mean,KS.p_sd)

df_resultados %>% ggplot(aes(x=p,y=GOF,color=MN)) + geom_point() + geom_smooth(method = "glm", se=F) + facet_wrap(~k,ncol=4)
