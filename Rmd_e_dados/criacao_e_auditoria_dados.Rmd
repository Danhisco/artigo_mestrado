...
# SdT: i) adicionar as informações observadas necessárias para a conversão (p e DA); ii) converter theta -> U e m -> d; iii) calcular a correspondente chuva de propágulos
...
-> Juntando o data frame df_EE e df_EE_viaEI
...
df_EE_viaEI <- read.table(file = "/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/df_SAD_EE-parEI-viaEI.txt", header = T, as.is = T)

df_temp <- ddply(df_EE_viaEI,c("SiteCode", "kernel"),summarise, 
                 theta_mean = mean(theta),theta_var = var(theta),
                 m_mean = mean(m), m_var = var(m))
