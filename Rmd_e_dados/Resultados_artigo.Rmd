...
O modelos cheio foi o único modelo plausível somando peso de evidência próximo de 1. Para fazer a interpretação irei construir um gráfico do predito com intervalo de confiança contra o observado na escala da função de ligação logito.
...
## funções para serem calculados a cada simulação ##
# previsto pelo modelo completo: efeitos aleatórios e fixos #
f1 <- function(.) predict(.,newdata=df_new.data)
# previsto pelo modelo completo: fixos #
...
b1 <- bootMer(md_GOF, FUN = f1, nsim=1000, parallel="multicore", ncpus=3)
b2 <- bootMer(md_GOF, FUN = f2, nsim=1000, parallel="multicore", ncpus=3)
```
...
```{r graf exp dGOF 1, fig.height=6, fig.width=9}
## data frame
...
__Figura 11__ dGOF = EE/EI e log.dGOF = log(dGOF) pela cobertura vegetal (p) e proporção de propágulos (k)

- dGOF intrinsicamente perde informação valiosa para entender como as predições dos modelos neutros se aproximam ou não do observado
- concluímos que manter a variável GOF é a melhor forma de continuar na análise estatística
...
```{r tabela selecao GOF}
df_ad <- df_resultados %>% filter(MN == "EE")
...
__R2 marginal e condicional__

```{r R2 m e c, warning=FALSE,message=FALSE}
# (r2_GOF <- sapply(l_md1, r.squaredGLMM))
```

__Porcentagem explicada pelos efeitos fixos__
