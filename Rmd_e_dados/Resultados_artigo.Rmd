...
__ Estrutura dos Resultados Completos: __
...
###### distância média de dispersão (d)
...
df_ef.d  <- reshape2::melt(ef_d,value.name = "replica") %>% 
...
ggplot(data=df_ef.d[,c(1,5:7)], aes(x=variable,y=media)) + geom_pointrange(aes(y = media, ymin=IC.025, ymax=IC.975)) # +
  # geom_ribbon(aes(y = fit, ymin=lwr, ymax=upr), alpha=0.5) +
# existe grande variação entre sítios que é explicada em função da densidade
```

<!--
-->
 I = número fundamental da imigração (Etienne 2005)
...
```{r matrixplot I}
# ,fig.width=10
car::scatterplotMatrix(~I+p+d+k+J,data=df_resultados)
# hist(sqrt(df_resultados$I),breaks = 60)

```

  O número fundamental da dispersão (I) depende do tamanho da amostra (J) e de m, probabilidade de um imigrante colonizar uma unidade de habitat na comunidade local. Essa probabilidade é deduzida a partir de um equação que relaciona a distância média de dispersão (d) e J, então corrigimos essa estimativa para a proporção de habitat da paisagem, para incluir a falta de indivíduos na paisagem ao redor da comunidade local caso tenha ocorrido remoção de habitat. Obtemos d a partir da proporção de propágulos que permanece até a vizinhança imediata da planta progenitora (k). Estabelecemos a priori os valores de k para utilizar em todas as paisagens, assim podemos comparar paisagens com diferentes densidades de indivíduos (para não violar o pressuposto de soma-zero).
  O efeito de J pode ser incluido na variação do agrupamento de sítio de amostragem, as variáveis mais importantes são p, k e d. Uma possibilidade é permitir que para cada agrupamento de dados pelo sítio de amostragem, seja possível existir uma inclinação para d. Então o modelo cheio é I ~ p.z + d.z + k + (d.z|Site).

```{r graf I}
df_resultados %>% mutate(p_class = as.factor(cut(p,12))) %>% filter(p_class == levels(p_class)[9]) %>% #%in% levels(p_class)[c(9,12)] 
  ggplot(aes(x=d.z,y=sqrt(I))) + 
...
# df_resultados %>% str
## Seleção da estrutura eleatória ##
l_md <- vector("list",length = 9)
names(l_md) <- c("log Gamma 1|Site","log Gamma d.z|Site","log Gamma",
                 "normal 1|Site","normal d.z|Site", "normal",
                 "log normal 1|Site","log normal d.z|Site", "log normal")
l_md[[1]] <-  glmer(I ~ p.z + d.z + k + (1|Site),family = "Gamma"(link=log),data = df_resultados)
l_md[[2]] <-  glmer(I ~ p.z + d.z + k + (d.z|Site),family = "Gamma"(link=log),data = df_resultados)
l_md[[3]] <-  glm(I ~ p.z + d.z + k,family = "Gamma"(link=log),data = df_resultados)
l_md[[4]] <-  lmer(I ~ p.z + d.z + k + (1|Site), data = df_resultados)
l_md[[5]] <-  lmer(I ~ p.z + d.z + k + (d.z|Site), data = df_resultados)
l_md[[6]] <-  lm(I ~ p.z + d.z + k, data = df_resultados)
l_md[[7]] <-  glmer(I ~ p.z + d.z + k + (1|Site),family = "gaussian"(link=log),data = df_resultados)
l_md[[8]] <-  glmer(I ~ p.z + d.z + k + (d.z|Site),family = "gaussian"(link=log),data = df_resultados)
l_md[[9]] <-  glm(I ~ p.z + d.z + k,family = "gaussian"(link=log),data = df_resultados)
AICctab(l_md,weights=T)
```
<!--
A estrutura aleatória selecionada foi (1|Site).
...
```{r d k R2,echo=TRUE}
# fastdisp(l_md[[2]]) # problema de convergência, indicam para reescalonar as variáveis
md_I <- update(l_md[[2]],control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
summary(l_md[[2]]) # problema de convergência, indicam para reescalonar as variáveis
summary(md_I)
...
- Problema de convergência:
convergence code: 0
Model failed to converge with max|grad| = 148.862 (tol = 0.001, component 1)
Model is nearly unidentifiable: very large eigenvalue
 - Rescale variables?
failure to converge in 10000 evaluations

```{r comparacao modelo cheio }
l_md <- vector("list",2)
names(l_md) <- c("d.z","-d.z")
l_md[[1]] <-  glmer(I ~ p.z + d.z + k + (1|Site),family = "Gamma"(link=log),data = df_resultados)
l_md[[2]] <-  glmer(I ~ p.z + k + (1|Site),family = "Gamma"(link=log),data = df_resultados)
AICctab(l_md)
```
...
Quando desenvolvi os dados de MNEI eu substitui as informações da riqueza estimado de MNEE para o correspodente. Não há motivo para questionar se cometi o mesmo erro para GOF uma vez que eles diferem em seu perfil e me recordo de fazer uma bateria de auditorias na época. Contudo, vi que os .Rmds não estão organizados e se faz necessário organizar tudo para pelo menos ter dos dados brutos (SAD obs, mapas de cobetura vegetal concentricos à área amostrada) até as SADs preditas segundo os modelos neutros e a taxa de especiação; uma sessão para a primeira sessão de análise dos dados (teste KS e riqueza das espécies simuladas). 
