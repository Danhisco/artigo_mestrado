...
l_p[[1]] <- ggplot(filter(df_ef,termo == "alfa"),aes(x=k,y=invlogit(media),color=MN)) +
  ...
  scale_colour_manual(values = c("Blue", "Red")) +
  # labs(y="probabilidade média de não refutar neutralidade") +
  theme(legend.position = "none")
l_p[[2]] <- ggplot(filter(df_ef,termo == "beta"),aes(x=k,y=invlogit(media),color=MN)) +
  geom_linerange(aes(ymin=invlogit(lwr),ymax=invlogit(upr))) +
  geom_crossbar(aes(x=k,ymin=invlogit(lwr.m),ymax=invlogit(upr.m))) +
  scale_colour_manual(values = c("Blue", "Red")) +
  # labs(y="probabilidade média de não refutar neutralidade") +
...
- coisas que precisam ser feitas: avaliar a qualidade das regressões lineares ajustadas às réplicas - posso plotar o boxplot do R2 para o bootMer condional e marginal
