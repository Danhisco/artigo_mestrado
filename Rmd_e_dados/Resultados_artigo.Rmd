...
names(l_md) <- c("1|Site ID","k|Site ID","1|Site log","k|Site log")
l_md[[3]] <-  glmer(d ~ k * DA + (1|Site),family = "gaussian"(link=log),data = df_resultados)
l_md[[4]] <-  glmer(d ~ k.z * DA + (k.z|Site),family = "gaussian"(link=log),data = df_resultados)
AICctab(l_md)
```
<!--
A estrutura aleatória selecionada foi (1|Site). Segue o R^2 explicado pelos dois modelos:
-->
Coisas a fazer: 
i) estudar a nova forma de calcular o coeficiente de explicação[é esse o nome?] (R^2); intervalo de confiança (IC) e uma terceira métrica (Nakagawa et al. 2017)
ii) avaliar a opção mais prática e implementar
```{r d k R2}
...
names(l_md) <- c("k+DA","DA","k","1","k*DA")
l_md[[1]] <-  lmer(d ~ k + DA + (1|Site),data = df_resultados)
l_md[[2]] <-  lmer(d ~ DA + (1|Site),data = df_resultados)
l_md[[3]] <-  lmer(d ~ k + (1|Site),data = df_resultados)
l_md[[4]] <-  lmer(d ~ 1 + (1|Site),data = df_resultados)
l_md[[5]] <-  lmer(d ~ k * DA + (1|Site),data = df_resultados)
AICctab(l_md)
```

A distância média de dispersão variou em função da proporção de propágulos que permanece na vizinhança da planta progenitora (k) e da densidade observada (DA). Segue os dados plotados e a estimativa linear

```{r}
# gráfico #
df_resultados %>% ggplot(aes(x=DA,y=d)) + geom_smooth(method = "lm") + geom_point() + facet_wrap(~k,ncol=3,scales="free")
# broom::tidy(l_md[[1]])
df_diag <- broom::augment(l_md[[1]]) %>% inner_join(x=.,y=df_resultados[,-c(7,12)],by=c("Site","k"))
```

figura X. d ~ k * DA + (1|Site). Parece que o modelo faz ótimo ajuste o que mostra que a função que desenvolvemos está fazendo boas aproximações.

```{r}
df_diag %>% ggplot(aes(x=DA,y=.resid)) + geom_smooth(method = "lm") + geom_point() + facet_wrap(~k,ncol=3,scales = "free")
```
