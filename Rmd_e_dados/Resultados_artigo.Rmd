...
__Figura 1__ Distância média de dispersão (d) e o predito segundo d ~ DA.z + k + (1 | Sítio), family=Gamma. Os pontos são as distâncias médias estimados para o determinado percentil (k) de propágulos que permanecem até l_cel metros da planta progenitora [ADICIONAR FORMULA l_cel ~ DA,J]; em vermelho o predito.
...
2) n_ref = número de SADs réplicas neutras refutadas
...
### I - número fundamental da dispersão (Etienne 2005) ###
...
# x11()
par(mfrow=c(2,2))
hist(log(df_resultados$I),breaks=40,main = "",xlab = "log(I)",ylab="")
## d e d.z ##
# ~ norm
qqp(df_resultados$I, "norm",ylab="I") 
# ~ lnorm
qqp(df_resultados$I, "lnorm",ylab="I")
# ~ gamma
fit_gamma1 <- fitdistr(df_resultados$d, "gamma")
qqp(df_resultados$I, "gamma", shape = fit_gamma1$estimate[[1]], rate = fit_gamma1$estimate[[2]],ylab="I")
## d.z ##
# ~ norm
# ~ lnorm
# ~ gamma
...
-exploração gráfica das hipóteses levantadas até o momento
-inclui código mas não inclui as figuras pois iria ficar muito longo

```{r explor graf n_ref,echo=TRUE,include=FALSE}
df_resultados %<>% mutate(n_ref=100-GOF)
# df_resultados$k %>% levels

### lattice ###

# par(mfrow=c(1,2))
# lattice::xyplot(n_ref ~ p | k, data=df_resultados,as.table=T,
#                 group=df_resultados$MN, auto.key=list(text=c("EI", "EE")))
# xyplot(n_ref ~ MN | k, data=df_resultados,as.table=T,
#                 group=df_resultados$MN, auto.key=list(text=c("EI", "EE")))

### ggplot2 ###
l_p <- vector("list",3) # número de hipóteses 
l_p[[1]] <- ggplot(df_resultados,aes(x=p,y=n_ref)) + #P1: n_ref ~p
  geom_point() + geom_smooth(method="loess")
l_p[[2]] <- ggplot(df_resultados,aes(x=k,y=n_ref)) + #P2: n_ref ~ k
  geom_jitter() +
  geom_boxplot() +
  labs(y="")
l_p[[3]] <- ggplot(df_resultados,aes(x=MN,y=n_ref)) + #P3: n_ref ~ MN
  geom_jitter() +
  geom_boxplot() +
  labs(y="")
l_p[[4]] <- ggplot(df_resultados,aes(x=p,y=n_ref)) + #P4: n_ref ~ p : k
  geom_point() + geom_smooth(method="loess") + facet_wrap(~k,ncol=3)
l_p[[5]] <- ggplot(df_resultados,aes(x=MN,y=n_ref)) + #P5: n_ref ~ MN : k [?]
  geom_jitter() + geom_boxplot() + facet_wrap(~k,ncol=3)
l_p[[6]] <- ggplot(df_resultados,aes(x=p,y=n_ref)) + #P4: n_ref ~ p : MN [?]
  geom_point() + geom_smooth(method = "loess") + facet_wrap(~MN,ncol=2)
l_p[[7]] <- ggplot(df_resultados,aes(x=p,y=n_ref,color=MN)) + # P7: p:k:MN [?]
  geom_point() + geom_smooth(method="loess") + facet_wrap(~k,ncol=3)
...
```{r GOF selecao de funcao de ligacao, echo=FALSE, include=FALSE}
