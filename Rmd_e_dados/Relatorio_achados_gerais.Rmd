...
```{r graf exp logit U}
df_resultados %<>% mutate(logit_U = log(U/(1-U)),
                          log_S.z = (log(S) - mean(log(S)))/sd(log(S)))
...
l_md[[1]] <- lmer(logit_U ~ p.z * k + log_S.z + (1 | Site), data = df_resultados)
l_md[[2]] <- lmer(logit_U ~ p.z + k + log_S.z + (1 | Site), data = df_resultados)
l_md[[3]] <- lmer(logit_U ~ p.z + log_S.z + (1 | Site), data = df_resultados)
l_md[[4]] <- lmer(logit_U ~ k + log_S.z + (1 | Site), data = df_resultados)
l_md[[5]] <- lmer(logit_U ~ log_S.z + (1 | Site), data = df_resultados)
l_md[[6]] <- lmer(logit_U ~ p.z * k + (1 | Site), data = df_resultados)
l_md[[7]] <- lmer(logit_U ~ p.z + k + (1 | Site), data = df_resultados)
l_md[[8]] <- lmer(logit_U ~ p.z + (1 | Site), data = df_resultados)
l_md[[9]] <- lmer(logit_U ~ k + (1 | Site), data = df_resultados)
l_md[[10]] <- lmer(logit_U ~ 1 + (1 | Site), data = df_resultados)
# selecao #
