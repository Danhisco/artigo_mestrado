...
ii) determinamos 12 valores de proporção de propágulos que se mantêm até a vizinhança imediata (l_cel metros) da árvore progenitora (k): 0.99,0.95:0.50,0.25
iii) então estimamos a distância média de dispersão (d) que gerava o correspondente percentil, a parti de uma função baseada em simulação e quantis. 
...
## Danilo 28dez2018: A primeira figura de U deve ser o observado na escala padrão, para o leitor poder avaliar a congruência entre predito pelo modelo estatístico e o observado [permite ponderar a estimativa dos efeitos]

### preparação dos dados ###

df_newdat %<>% mutate(std_mean = arm::invlogit(mean),
                     std_IC.low = arm::invlogit(IC.low),
                     std_IC.upp = arm::invlogit(IC.upp),
                     std_mean.fixed = arm::invlogit(mean.fixed),
                     std_IC.low.fixed = arm::invlogit(IC.low),
                     std_IC.upp.fixed = arm::invlogit(IC.upp.fixed))
...
df_resultados %>%
  ...
  geom_line(aes(x=p, y=std_mean.fixed), data=df_newdat,color="red") +
  geom_point() +
  labs(x="% cobertura vegetal", y = "U") +
  facet_wrap(~k,ncol=3)
```

__Figura 7__ U pela porcentagem de cobertura vegetal. A linha vermelha é a estimativa da média, a região em branco representa o intervalo de confiança de 95% marginal ao agrupamento dos dados; e a região em cinza representa o intervalo de confiança de 95% condicional ao agrupamento dos dados.


```{r coef U e IC bootmer 1}
f_coef.bootMer <- function(df_=df_newdat,X){
  lm.data <- cbind(X,df_newdat)
  names(lm.data)[1] <- "replica"
  lm.object <- coef(lm(replica ~ p.z * k, data = lm.data))
  return(lm.object)
}

# replicas dos coeficientes armazenados em um df
registerDoMC(3)
df_lm.coef1 <- adply(b3$t,1,function(a) f_coef.bootMer(X=a),.parallel = TRUE) %>% select(-X1)
df_lm.coef2 <- adply(b4$t,1,function(a) f_coef.bootMer(X=a),.parallel = TRUE) %>% select(-X1)

## funcao para combinar e sumarizar as replicas de cada coeficiente
f_par.md2ordem <- function(df_coef,df_=df_resultados){
  l_ef <- vector("list",12)
  l_ef[[1]] <- data.frame(k = factor(levels(df_$k)[1], levels = levels(df_$k)), 
                          termo = rep(rep(c("intercepto","inclinação"),each=2),each=1000),
...
__Figura 8__ Efeitos estimados de p, k e S para descrever logito(U). 
...
# grafico #
plotFEsim(fe_GOF,stat = "mean",intercept = TRUE) + theme_bw() + labs(y="logito(prob de não refutar)",x="coef")
...
```{r obs e predito escala resposta,fig.width=10}
#inv logit
...
#   theme(legend.position = "bottom")
# l_p[[1]] <- ggplot(df_ef,aes(x=k,y=media)) +
  # geom_linerange(aes(ymin=lwr,ymax=upr)) +
  # geom_crossbar(aes(x=k,ymin=lwr.m,ymax=upr.m)) +
  # scale_colour_manual(values = c("Blue", "Red")) +
  # labs(y="logito(Probabilidade de não refutar uma predição neutra") +
  # facet_grid(termo~MN)
  # theme(legend.position = "none")
# l_p[[2]] <- ggplot(filter(df_ef,termo == "beta"),aes(x=k,y=media,color=MN)) +
#   geom_linerange(aes(ymin=lwr,ymax=upr)) +
#   geom_crossbar(aes(x=k,ymin=lwr.m,ymax=upr.m)) +
#   scale_colour_manual(values = c("Blue", "Red")) +
#   labs(y="beta") +
