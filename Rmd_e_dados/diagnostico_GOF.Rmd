...
__Desvios devido à inflamação de zeros__

  Para avaliar o efeito da inflamação de zeros vou utilizar o teste específico disponível no pacote DHARMa:

```{r teste de inflamcao de zeros}
testZeroInflation(residuos_GOF)
```

__Figura 4__ Teste de Inflamação de Zeros: o histograma representa o número de zeros esperado se o modelo estiver correto e a linha vermelha os zeros observados.

  O teste indica que há problemas de inflamação por zeros. 
