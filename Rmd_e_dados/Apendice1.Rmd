...
```{r}
df_references <- read.csv(file="/home/danilo/Documentos/Doutorado/artigo_mestrado/Rmd_e_dados/references - TreeCo.csv",as.is = TRUE,header =T, na.strings = c("","NA")) %>%
   select(refID,status,method,arrangement,samples,effort_ha,dbh_cutoff,SiteCode,lat,long,lat_municipio,long_municipio,lat_correct,long_correct,confiabilidade, Unidade_de_conservacao,Unidade_de_conservacao1,UC_area_ha,year_data,year,state)
# df_references %>% names
df_references %>% filter(method=="parcelas" & arrangement == "contiguous" & effort_ha>=1 & status == "yes")
```
