...
```{r tree cover}
df_simulacao <- read.csv("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/df_dados_auditados.csv",header = TRUE,as.is = TRUE)
...
for(i in 1:nrow(df_simulacao)){
  par(mar=c(0,0,2,0))
  # i <- 2
 txt_file <- read.table(df_simulacao$txt.name[i],
                        header = TRUE,sep = " ",as.is = FALSE) %>% as.matrix
 image(txt_file,
       main=paste0(df_simulacao$SiteCode[i]," p=",df_simulacao$p[i]),
