...
--GOcaldas - remover: parece não estar centrado 
--MSlada5
-- 
...
func_tif.png <- function(file){
  ...
  raster <- raster(df_dados.disponiveis$png.name) #leitura do raster
...
## leitura 
df_dados.disponiveis %<>% left_join(x=.,
                                    y=data.frame(txt.name = Sys.glob("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/dados_brutos/*.txt"),
                                                 tif.name = gsub(".txt",".tif",Sys.glob("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/dados_brutos/*.txt")),
                                    by="tif.name"))
...
- txt.file para avaliação:
"MGlavr4","MSlada5", "MSlada3", "BAuruc", "ESsoor", "MGlavr6","SCcric1","SPpesmK","SPpesmN"

```{r auditoria de valores}
...
# png para matriz binaria
    ...
    # image(raster_return,
    #       main=paste(df_dados.disponiveis$SiteCode[1],"focal(sum(X[X==1]>=2,1,0)")
    #       ,xaxt='n',yaxt='n',xlab="",ylab="")
    # image(raster_focal,
    #       main=paste(df_dados.disponiveis$SiteCode[1],"tree cover>=0.70,1,0")
    #       ,xaxt='n',yaxt='n',xlab="",ylab="")
    # image(raster(df_dados.disponiveis$png.name[1]),
    #       main=paste(df_dados.disponiveis$SiteCode[1],"png")
    #       ,xaxt='n',yaxt='n',xlab="",ylab="")

f_mat_binaria <- function(landscape.path,file.name){
  #@ landscape.path::caminho de arquivo de imagem (tiff ou png)
  #@ file.name:: nome que a matriz binaria sera gravada
  #####################
  ### funcao tree cover
  ## raster de interesse
  # raster_ <- raster(df_dados.disponiveis$png.name[1])
  raster_ <- raster(landscape.path)
  values_raster <- getValues(raster_)
  values_raster <- values_raster/max(values_raster)
  ## raster para a função focal
  raster_focal <- raster(ncol=2+ncol(raster_),nrow=2+nrow(raster_))
  values(raster_focal) <- 0
  mat_focal <- as.matrix(raster_focal)
  .linhas <- 2:(nrow(mat_focal)-1)
  .colunas <- 2:(ncol(mat_focal)-1)
  #####################
  ### funcoes
  ## função tree cover cut off: [0,1] -> c(0,1)
  f_tree.cover <- function(Y) ifelse(Y>=0.70,1,0)
  values(raster_) <- sapply(values_raster,f_tree.cover) 
  # atribuicao
  mat_focal[.linhas,.colunas] <- matrix(getValues(raster_),
                                        nrow = dim(raster_)[1],
                                        ncol = dim(raster_)[2])
  values(raster_focal) <- as.vector(mat_focal)
  ## função focal 
  fun_focal <- function(X) ifelse(sum(X[X==1],na.rm = TRUE) >= 2,1,0)
  # atribuicao
  raster_return <- focal(raster_focal,
                         w=matrix(1,nrow = 3, ncol = 3),
                         fun = fun_focal,
                         pad = FALSE)
  #####################
  ### gravar
  # file.name <- gsub(".png",".txt",df_dados.disponiveis$png.name[1])
  write.table(as.matrix(raster_return)[.linhas,.colunas],file = file.name, row.names = FALSE)
}
df_dados.disponiveis$png.name %<>% as.character
registerDoMC(2)
a_ply(df_dados.disponiveis,1,function(X) f_mat_binaria(landscape.path = X$png.name,file.name = gsub(".png",".txt",X$png.name)),.parallel = TRUE) 
```
