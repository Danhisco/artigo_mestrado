...
EE foi utilizado para estimar a taxa de especiação necessária para obter a riqueza observada, em média o viés da riqueza estimada para MNEE é igual -7.14e-3 e com máximo e mínimo igual -6.94e-2 e 4.03e-2, respectivamente. As linhas nas duas primeiras colunas correspondem aos contornos de diffS. Na segunda coluna há o predito para Pr(nRef) de EI. Na terceira coluna há o predito para o log da razão de chance (lot OR) de EE em relação EI de não refutar uma SAD neutra. A região hachurada para log OR corresponde aquela área onde Pr(nRef)>0.5 e |diffS|>0.25 para MNEI, ou seja, região onde o teste Kolmogorov-Smirnov bootstrap está inflando a chance de não refutar uma SAD neutra para MNEI.
