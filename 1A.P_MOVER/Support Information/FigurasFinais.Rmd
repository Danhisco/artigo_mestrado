...
```{r figura 2 logOR diffS,echo=FALSE,include=TRUE,fig.height=5,fig.width=12}
################### diffS ##############################
# EE
...
df_plot %>% 
  ...
  geom_contour(aes(z=diffS),
               breaks=df_predEE$fit,color="red",alpha=0.3,linetype="dotdash") +
  ## texto do contorno
...
```{r parametros MNEI discusssao, message=FALSE,warning=FALSE}
#########
# Dados #
#########
...
############
l_p <- list()
l_p[[1]] <- ggplot(df_simulacao,aes(x=p,y=m_)) + #,color=d
  geom_smooth(method="loess",se=FALSE,alpha=0.3,color="red") +
  geom_point(alpha=0.3) +
  scale_y_continuous(trans='log10')
l_p[[2]] <- ggplot(df_simulacao,aes(x=p,y=theta)) + #,color=d
  geom_smooth(method="loess",se=FALSE,alpha=0.3,color="red") +
  geom_point(alpha=0.3) +
  scale_y_continuous(trans='log10')
# l_p[[3]] <- ggplot(df_simulacao,aes(x=theta,y=m_,fill=U_med)) +
...
#### Qual a variação de área amostral de habitat / área de habitat paisagem local ~ p


```{r area amostral / area paisagem por p}
df_p <- df_resultados %>% mutate(ratio_J = Ntotal /(10404*DA*p)) 
ggplot(df_p, aes(x=p,y=ratio_J)) +
  geom_point(alpha = 1/10) +
  labs(x = "p",y="N_sample / N_landscape") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
              labels = trans_format("log10", math_format(10^.x))) +
  annotation_logticks(sides = "l") +
  labs(x="p","Namostra / Npaisagem")


ggplot(df_p,aes(x=p,y=d))+geom_point(alpha=0.3)
```
