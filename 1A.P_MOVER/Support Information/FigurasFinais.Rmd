...
```{r figura 2 logOR diffS,echo=FALSE,include=TRUE}
################### diffS ##############################
# EE
...
# v_limits <- df_plot_diffS %>% select(mod.avg.pred,upper.CL,lower.CL) %>% as.matrix %>% as.vector %>% range
l_p <- list()
df_plot_diffS$MN <- "EI"
l_p[[1]] <- ggplot(df_plot_diffS,aes(x=p,y=d,fill=mod.avg.pred,z=mod.avg.pred)) +
  geom_tile() +
  coord_cartesian(expand = FALSE) +
  scale_fill_gradient2(midpoint=0, low="blue", high="red",mid="green",limits=v_limits) +
  #####################################
  ## responsável pela área rachurada ##
...
# curvas de nível de logOR #
br <- 0 #-5,-1,0,1,5,30
# para área rachurada
v_range <- df_predEE_diffS[,3:2] %>% as.numeric() %>% unname()
############# gráficos #############
l_p <- list()
df_plot1 %<>% mutate(label="Mean")
l_p[[1]] <- ggplot(df_plot1,aes(x=p,y=d,fill=logOR_EE.EI)) +
  ...
  geom_contour(aes(z=logOR_EE.EI),alpha=0.7, col="black",
               breaks = br) +
  ...
  # diffS
  ## responsável pela área rachurada ##
  stat_subset(aes(z=mod.avg.pred,
                  subset = mod.avg.pred >= v_range[1] & mod.avg.pred <= v_range[2]),
              alpha=0.3,color="black",size=0.1) +
