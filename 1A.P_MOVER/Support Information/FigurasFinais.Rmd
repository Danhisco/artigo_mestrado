...
l_p <- vector("list")
l_p[[1]] <- df_resultados %>% 
  filter(p<=0.35 & d>5, MN == "EI",n_nRef>=50) %>% 
  ...
  labs(x="diff(S,Sobs)/Sobs",y="# non refuted SADs KS boot test")
v_2 <- df_resultados %>% 
  filter(p<=0.35 & d>5, MN == "EI",n_nRef>=50) %>% 
  select(SiteCode, S_obs) %>% unique %>% 
  .$S_obs %>% quantile(.,probs = c(5,25,50,75,95)/100) %>% unname
v_3 <- df_resultados %>% 
  filter(p<=0.35 & d>5, MN == "EI",n_nRef>=50) %>% .$diffS_mean %>% quantile
l_p[[2]] <- df_resultados %>% 
  filter(p<=0.35 & d>5, MN == "EI",n_nRef>=50) %>% 
  ...
  labs(y="diff(S,Sobs)/Sobs",x="Sobs") #+
  # theme(axis.text.x = element_text(angle=60,vjust = 1, hjust=1))
# grid.arrange(grobs=l_p,ncol=2,top="SINM, p < 0.35 & d > 5 m")
l_p[[1]] + labs(title = "SINM, p < 0.35 & d > 5 m")
```
...
df_plot %>% select(-fit_class) %>% gt::gt()
...
Esses resultados indicam que há uma tendência da SAD predita por SINM subestimar o número de espécies

A maior parte das ocorrências se encontram abaixo do piso do critério de tolerância ()
