...
```{r parametros MNEI discusssao, message=FALSE,warning=FALSE,fig.width=10,fig.height=4}
#########
# Dados #
#########
...
l_p[[2]] <- df_simulacao %>% mutate(legend = "theta") %>%
  ...
  labs(y="") +
  facet_wrap(~legend)
l_p[[3]] <- df_resultados %>% mutate(ratio_J = Ntotal /(10404*DA*p),legend = "N_sample / N_landscape") %>%
...
colornum <- cut(rank(df_resultados$U_med[df_resultados$MN=="EE"]), 9, labels=FALSE)
colcode <- plotclr[colornum]
l_p <- list()
# par(mfrow=c(1,3))
l_p[[1]] <- with(filter(df_resultados,MN=="EE"),scatterplot3d(x=U_med,y=p,z=n_nRef,
...
df_resultados[which(df_resultados$n_nRef>50 & abs(df_resultados$diffS_mean)<0.25),"factor_cong"] <- "OK"
df_resultados[-which(df_resultados$n_nRef>50 & abs(df_resultados$diffS_mean)<0.25),"factor_cong"] <- "ruim"
df_resultados$factor_cong <- factor(df_resultados$factor_cong)
l_p <- list()
l_p[[1]] <- plot_ly(filter(df_resultados,MN=="EI"),
                    x = ~p, y = ~diffS_mean, z = ~n_nRef,
                    color= ~factor_cong, colors = c('#BF382A', '#0C4B8E')) %>% 
  add_markers() %>% 
  layout(scene=list(xaxis = list(title="p"),
                    yaxis = list(title="vies Sobs"),
                    zaxis = list(title="# SADs não refutadas")))
l_p[[1]]


l_p[[1]] <- with(filter(df_resultados,MN=="EI"),
                 scatterplot3d(y=diffS_mean,x=p,z=n_nRef,
                               angle=-60,color=alpha("black",0.4),
                               # main = "nRef ~ U médio * p",
                               pch=16,type="p",zlab = "nRef",
                               zlim=c(0,100),ylim=c(0,1)))
