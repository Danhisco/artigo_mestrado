...
```{r}
df_resultados %>% 
  filter(MN == "EE" & k == "0.99" & p<=0.35) %>% 
  select(SiteCode, p , S_obs, Ntotal) 
#%>% 
 # summary
...
write.csv(df_plot1,
          row.names = FALSE,
          file = "~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/df_pred_congEE-EI.csv")
v_limits <- df_plot1 %>% 
  select(logOR_EE.EI:upper.logOR_EE.EI) %>% as.matrix %>% as.vector %>% range
v_limits <- c(v_limits,mean(df_plot1$logOR_EE.EI))
v_range <- df_predEE[,3:2] %>% as.numeric() %>% unname()
...
############
l_p <- list()
v_m <- df_simulacao %>% filter(p<=0.35) %>% .$m_ %>% max
l_p[[1]] <- df_simulacao %>% mutate(legend = "m'") %>%
  ggplot(aes(x=p,y=m_)) + #,color=d
  geom_smooth(method="loess",se=FALSE,alpha=0.3,color="red") +
  geom_vline(xintercept = 0.35,color="black",alpha=0.6) +
  geom_segment(aes(x=0,xend=0.35,y=v_m,yend=v_m),color="black",alpha=0.01,linetype="dotted") +
  annotate(geom="text",x=0.15, y=v_m*1.2, label=as.character(round(v_m,digits = 2)),size=3) +
  geom_point(alpha=0.3) +
  scale_y_continuous(trans='log10') +
  annotation_logticks(sides = "l") +
  labs(y="") +
  facet_wrap(~legend)
v_t <- df_simulacao %>% filter(p<=0.35) %>% .$theta %>% max
l_p[[2]] <- df_simulacao %>% mutate(legend = "theta") %>%
  ggplot(aes(x=p,y=theta)) + #,color=d
  geom_smooth(method="loess",se=FALSE,alpha=0.3,color="red") +
  geom_vline(xintercept = 0.35,color="black",alpha=0.6) +
  geom_segment(aes(x=0,xend=0.35,y=v_t,yend=v_t),color="black",alpha=0.01,linetype="dotted") +
  annotate(geom="text",x=0.15, y=v_t*1.2, label=as.character(formatC(round(v_t,digits = 0),format = "e")),size=3) +
  geom_point(alpha=0.3) +
  scale_y_continuous(trans='log10') +
  annotation_logticks(sides = "l") +
  labs(y="") +
  facet_wrap(~legend)
df_plot <- df_resultados %>% mutate(ratio_J = Ntotal /(10404*DA*p),legend = "N_sample / N_landscape") %>% select(p,ratio_J,SiteCode,legend) %>% unique
v_ <- df_plot %>% filter(p<=0.35) %>% .$ratio_J %>% min
l_p[[3]] <- df_plot %>% 
  ggplot(aes(x=p,y=ratio_J)) +
  geom_vline(xintercept = 0.35,color="black",alpha=0.6) +
  geom_segment(aes(x=0,xend=0.35,y=v_,yend=v_),color="black",alpha=0.01,linetype="dotted") +
  annotate(geom="text",x=0.15, y=v_*1.2, label=as.character(formatC(round(v_,digits = 6),format = "e")),size=3) +
...
```
```{r}
df_plot$factor_cong %>% levels
df_plot[is.na(df_plot$factor_cong),]
df_plot %>% filter(factor_cong == "false positive")

df_plot[which(df_plot$n_nRef>50 & abs(df_plot$diffS_mean)<0.25),] # 
df_plot[which(df_plot$n_nRef>50 & abs(df_plot$diffS_mean)<0.25),] # 
```
...
# df_plot[which(df_plot$n_nRef>50 & abs(df_plot$diffS_mean)>0.25),"factor_cong"] <- "false positive"
...
l_p <- df_plot %>% 
  ...
  gather(par_class,par_value,theta:m_) %>% mutate(legenda = as.character(par_class)) %>% 
  dlply("par_class",function(df){
    ggplot(df,aes(x=p,y=par_value)) +
    geom_point(alpha=0.5) +
    scale_y_continuous(trans = "log10") +
    # ggtitle(label = head(df$legenda,n=1))
    facet_wrap(~factor_cong,nrow=1)
  })
l_p[[1]] <- l_p[[1]] + labs(y="m'")
l_p[[2]] <- l_p[[2]] + labs(y="theta")
# names(l_p)
grid.arrange(grobs=l_p,ncol=1)

l_p <- df_plot %>% 
  select(p,factor_cong,U_med,d) %>% 
  gather(par_class,par_value,U_med:d) %>%
  dlply("par_class",function(df){
    ggplot(df,aes(x=p,y=par_value)) +
    geom_point(alpha=0.5) +
    # scale_y_continuous(trans = "log10") +
...
l_p[[1]] <- l_p[[1]] + labs(y="d")
l_p[[2]] <- l_p[[2]] + labs(y="U")
# names(l_p)
grid.arrange(grobs=l_p,ncol=1)
```
- A amplitude de valores de theta parece ter pouco efeito na congruência de MNEI: as 3 classes coocorem; com uma pequena tendência de valores superiores em sítios que subestimaram a riqueza observada
- A amplitude de valores de m' tem maior efeito na congruência de MNEI: subestimar são até maiores do 1 ordem de grandeza menor do que os de bom boa congruência, que por sua vez são meia ordem de grandeza menor do que superestima a riqueza.


```{r fig.height=6,fig.width=6}
l_p[[2]] <- ggplot(df_plot,aes(y=m_,x=theta)) + #
  theme_bw() +
...
```{r figura X n_nRef diffS MNEI p<0.35 e d>5}
# v_filter

v_ <- df_resultados %>% 
  filter(p<=0.35 & d>4, MN == "EE") %>%
  .$diffS_mean %>% range
l_p <- vector("list")
l_p[[1]] <- df_resultados %>% 
  filter(p<=0.35 & d>4, MN == "EI",n_nRef>=50) %>% 
  ggplot(aes(x=diffS_mean,y = n_nRef,color=S_obs)) +
  geom_point(alpha=0.75) + 
  geom_vline(xintercept = c(-0.25,0.25),alpha=0.6) + geom_vline(xintercept = v_,color="red",alpha=0.6) +
  scale_color_continuous(name="Sobs") +
  labs(x="(S-Sobs)/Sobs",y="# non refuted SADs KS boot test")
v_2 <- df_resultados %>% 
  filter(p<=0.35 & d>4, MN == "EI",n_nRef>=50) %>% 
  select(SiteCode, S_obs) %>% unique %>% 
  .$S_obs %>% quantile(.,probs = c(5,25,50,75,95)/100) %>% unname
v_3 <- df_resultados %>% 
  filter(p<=0.35 & d>4, MN == "EI",n_nRef>=50) %>% .$diffS_mean %>% quantile
l_p[[2]] <- df_resultados %>% 
  filter(p<=0.35 & d>4, MN == "EI",n_nRef>=50) %>% 
  ggplot(aes(y=diffS_mean,x=S_obs)) +
  geom_smooth(method="loess",se=FALSE,formula = "y ~ x",alpha=0.6,color="blue") +
  geom_point(alpha=0.4) +
  geom_hline(yintercept = c(-0.25,0.25),alpha=0.6) + geom_hline(yintercept = v_,color="red",alpha=0.6) +
  scale_x_continuous(breaks = v_2) +
  labs(y="(S-Sobs)/Sobs",x="Sobs") #+
  # theme(axis.text.x = element_text(angle=60,vjust = 1, hjust=1))
# grid.arrange(grobs=l_p,ncol=2,top="SINM: p < 0.35, d > 4 m")
l_p[[1]] + labs(title = "SINM: p < 0.35, d > 4 m")
```
Figura X. Ajuste entre SADs preditas pelo modelo espacialmente implícito (SINM) quando a distância média de dispersão per capita (d) > 5 metros e a SAD observada em paisagens com proporção de cobertura vegetal (p) < 0.35. A figura mostra apenas os casos em que o número de SADs não refutadas no teste Kolmogorov-Smirnov bootstrap (# non refuted SADs KS boot test) é maior do 50 em 100 SADs preditas. O par de linhas em preto marcam o critério de tolerância para o viés no número de espécies predita por SINM (diff(S,Sobs)/Sobs). O par de linhas em vermelho marca a amplitude de valores estimados pelo modelo espacialmente explícito (SENM) para o viés no número de espécies predito. Painel da direita: # non refuted SADs KS boot test e diff(S,Sobs)/Sobs. Cerca de 47% das ocorrências estão abaixo do piso do critério de tolerância; e mais de 63% estão abaixo do menor viés para SENM (tabela X). Cerca de 17% estão acima do piso do critério de tolerância; e mais de 31% estão acima do maior viés para SENM (tabela X).
...
df_plot <- df_resultados %>% 
  ...
  mutate(
    f_fit1 = case_when(
      diffS_mean < -0.25 ~ "under -0.25",
      diffS_mean > 0.25 ~ "over 0.25",
      TRUE ~ "within criteria"),
    f_fit2 = case_when(
      diffS_mean < v_[1] ~ "under min(SENM)",
      diffS_mean > v_[2] ~ "over max(SENM)",
      TRUE ~ "within range(SENM)"))
f_table <- function(x){
  return_ <- 100*table(x) / length(x)
  return_ <- round(return_,2)
  return_ <- as.data.frame(return_)
  names(return_) <- c("factor","freq")
  return(return_)
}
df_plot %<>% 
  select(diffS_mean,f_fit1,f_fit2) %>% 
  gather(.,key=fit_class,value=fit_value,-diffS_mean) %>% #filter(fit_class =="f_congruencia2") %>% .$fit_value
  ddply(.,"fit_class",function(X) f_table(x=X$fit_value))
df_plot %>% select(-fit_class) %>% 
  gt::gt() #%>%
# df_plot[c(2,5),"freq"] %>% diff
# df_plot[c(1,4),"freq"] %>% diff
```


e para SADs observadas em paisagens com proporção vegetal (p) < 0.35 quando a distância média de dispersão per capita (d) > 5 metros


Painel da Direita. 

Número de SADs preditas pelo modelo espacialmente implícito (SINM) não refutadas no teste Kolmogorov-Smirnov bootstrap (eixo y) e o viés no número de espécies predito (eixo x). A figura mostra apenas os casos em que o número de SADs não refutadas é maior do 50 em 100 SADs preditas. O par de linhas verticais em preto marcam o critério de tolerância para o viés no número de espécies predita por SINM. O par de linhas verticais em vermelho marca a amplitude de valores estimados pelo modelo espacialmente explícito (SENM). Cerca de 47% das ocorrências estão abaixo do piso do critério de tolerância; e mais de 63% estão abaixo do menor viés para SENM (tabela X). Cerca de 17% estão acima do piso do critério de tolerância; e mais de 31% estão acima do maior viés para SENM (tabela X). 


Há uma tendência de subestimar?
