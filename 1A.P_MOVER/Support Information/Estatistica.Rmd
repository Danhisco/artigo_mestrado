...
df_U <- df_resultados %>% 
  filter(MN=="EE") %>%
  select(U_med,log10_U,SiteCode,k,d,d.z,p,p.z,log10_Sobs,log10_Sobs.z,log10_d,log10_d.z,Ntotal,DA) %>% 
  distinct |> 
  mutate(J_M = 1713.96 * DA * p,
...
<!-- rm(list=setdiff(ls(), "df_U")) -->

É necessário desconsiderar o efeito da riqueza? Diria que não. A pergunta é se, nos eventos de substituição de indivíduos locais por de fora da parcela (1-m), a chance de ser de uma nova espécies na paisagem (U) é maior do que ser de um indivíduo da paisagem ao redor da parcela (m) em paisagens com baixa proporção de cobertura vegetal. E pra isso não é necessário descontar o efeito da riqueza observada em determinar a magnitude de U, na verdade essa relação que é o interesse. 
  
    
```{r criacao logOR U m}
# dados
f_logOR <- function(x_num,y_den){
  (v <- log((x_num/(1-x_num))/(y_den/(1-y_den))))
}
df_U <- df_U |> 
  mutate(logOR_U.m = f_logOR(x_num = U_med,y_den = m_ ))
l_p <- list()
l_p[[1]] <- df_U |> 
  ggplot(aes(x=logOR_U.m)) +
  geom_vline(xintercept = 0,color="red",alpha=0.5) +
  geom_density() +
  labs(title="log odds ratio (U/m)",x="") +
  theme(plot.title = element_text(hjust = 0.5))
l_p[[2]] <- df_U |> 
  ggplot(aes(x=p,y=logOR_U.m)) +
  geom_point() +
  geom_smooth()
...
```{r gamm logOR Um}
#####
df_U$SiteCode <- factor(df_U$SiteCode)
# +- 1h:
md_logOR_Um <- gam(logOR_U.m ~ s(d.z,bs="cr",k=15) + s(p.z,bs="cr",k=10) + 
                     ti(d.z,p.z,bs=c("cr","cr"),k=c(15,10)) +
                     s(d.z,SiteCode,bs="fs",k=15,xt=list("cr"),m=1),
                   data = df_U, family=gaussian, method = "REML")
gam.check(md_logOR_Um)
save(md_logOR_Um,file = "~/Documentos/artigo_mestrado/1A.P_MOVER/resultados_e_SI/md_logOR_Um.Rdata")
```
```{r logOR Um por sitio,fig.width=8, fig.height=27}
df_plot <- df_U %>%
  mutate(title = paste0(SiteCode," p=",round(p,3))) %>% arrange(p)
df_plot$title <- factor(df_plot$title,levels = unique(df_plot$title))
df_plot %>% ggplot(aes(x=log10_d,y=logOR_U.m)) +
  geom_smooth(method = "loess",se = FALSE,color="red",alpha=0.5,formula="y~x") +
...
```{r figura 11 possiveis variaveis preditoras,echo=TRUE,include=FALSE,fig.height=7,fig.align='center',fig.width=7,include=F,eval=FALSE}
l_p <- list()
...
```{r desvio padrao funcao de dispersao por k ,fig.width=12,fig.height=5}
l_p <- list()
l_p[[1]] <- df_resultados %>% filter(MN == "EE") %>% select(SiteCode,k,d,DA) %>% mutate(k_cont = as.numeric(as.character(k))) %>% 
  ggplot(aes(x=k,y=d,color=DA)) +
  geom_boxplot(aes(x=k,y=d)) +
  geom_point()
l_p[[2]] <- df_resultados %>% filter(MN == "EE") %>% select(SiteCode,k,d,DA) %>% mutate(k_cont = as.numeric(as.character(k))) %>% 
  ggplot(aes(x=k,y=d,color=DA)) +
  geom_boxplot(aes(x=k,y=d)) +
  geom_point() +
...
```{r parametros de MNEI pelos parametros de MNEE e p}
df_resultados %<>% 
...
l_p <- list()
l_p[[1]] <- df_resultados %>% 
  ggplot(aes(U_med,theta,color=p)) +
  geom_point()
l_p[[2]] <- df_resultados %>% 
  ggplot(aes(U_med,theta,color=p)) +
  geom_point() +
...
```{r distribuicao de valores de U escala padrao e ln}
l_p <- list()
l_p[[1]] <- df_resultados %>% 
  ggplot(aes(x="",y=U_med)) +
  geom_jitter() +
  geom_boxplot() +
  scale_y_continuous(labels = scales::scientific,
                     breaks= as.vector(summary(df_resultados$U_med))) +
  labs(y="taxa U")
l_p[[2]] <- df_resultados %>% 
  ggplot(aes(x="",y=U_med)) +
  geom_jitter() +
  geom_boxplot() +
  scale_y_continuous(trans="log",
                     labels = scales::scientific,  # = function(x) format(x, scientific = TRUE)
                     breaks= as.vector(summary(df_resultados$U_med))) +
  labs(y="taxa U")
grid.arrange(grobs=l_p,ncol=2)
```
...
```{r contexto de MNEE pior do que MNEI}
l_p <- list()
l_p[[1]] <- df_resultados %>% filter(d>=5 & p>0.35) %>%
  ggplot(aes(x="",y= m_ ) ) +
  geom_jitter() +
  geom_boxplot() +
  scale_y_continuous(breaks=(df_resultados %>% filter(d>=5 & p>0.35) %>% .$m_ %>% summary() %>% as.vector() %>% round(.,4))) +
  labs(y="m'") +
  ggtitle(label="p>0.35 e d>5 metros") +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        plot.title = element_text(hjust = 0.5))
v_ratioJ <- df_resultados %>% 
  mutate(ratio_J = 100 *(Ntotal /(10404*DA*p))) %>% filter(p<=0.35) %>% .$ratio_J %>% summary %>% as.vector %>% round(.,4) 
l_p[[2]] <- df_resultados %>% 
  mutate(ratio_J = 100 *(Ntotal /(10404*DA*p)),legend = "N sample / N landscape") %>% 
  select(p,ratio_J,SiteCode,legend) %>% 
  unique %>% 
  ggplot(aes(x=p,y=ratio_J)) +
  geom_point(alpha = 0.8) +
  geom_vline(xintercept = 0.35) +
  labs(x = "p",y="") +
  scale_y_continuous(breaks=v_ratioJ[-4],trans="log10") +
  ggtitle("100 * (N_sample / N_landscape)") +
  theme(plot.title = element_text(hjust = 0.5))
grid.arrange(grobs=l_p,ncol=2)
```
...
```{r pontos usados para ajustar os modelos estatisticos}
df_resultados %>% 
  ggplot(aes(p,d)) +
  geom_point()
# df_resultados %>% 
...
# geom_point()
```
```{r figura de criterio de parcimonia}
eixo_x <- 0.5
eixo_y <- 3
ggplot() +
  geom_segment(aes(x = 0, y = eixo_y, xend = 0, yend = -eixo_y),
                  arrow = arrow(length = unit(0.3, "cm"))) +
  geom_segment(aes(x = 0, y = -eixo_y, xend = 0, yend = eixo_y),
                  arrow = arrow(length = unit(0.3, "cm"))) +
  geom_segment(aes(x = eixo_x, y = 0, xend = -eixo_x, yend = 0),
                  arrow = arrow(length = unit(0.3, "cm"))) +
  geom_segment(aes(x = -eixo_x, y = 0, xend = eixo_x, yend = 0),
                  arrow = arrow(length = unit(0.3, "cm"))) +
  geom_rect(aes(xmin = -0.25, xmax = 0.25, ymin = -3, ymax = 1),fill="darkolivegreen4",alpha=0.6) +
  annotate("text",x=0.1, y=3.1, label="Log OR") +
  annotate("text",x=eixo_x-0.01, y=0.25, label="diff S") +
  geom_segment(aes(x = -0.01, y = 1, xend = 0.01, yend = 1)) +
  annotate("text",x=0.02, y=1.1, label="1") +
  geom_segment(aes(x = -0.25, y = -0.1, xend = -0.25, yend = 0.1)) +
  annotate("text",x=-0.28, y=0.5, label="-0.25") +
  geom_segment(aes(x = 0.25, y = -0.1, xend = 0.25, yend = 0.1)) +
  annotate("text",x=0.28, y=0.5, label="0.25") +
  labs(title = "SAD predita por MNEI >= MNEE na área sombreada") +
  theme(legend.position = "none",
...
save(l_nRef__modeloCheio,file="~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_md_nRef_GLMM_4x4.Rdata")
```
...
save(l_nRef__modeloCheio2,file="~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_md_nRef_GLMM_4x4.Rdata")
```
...
load("~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_md_nRef_GLMM_4x4.Rdata")
# avaliação e melhora do ajuste dos modelos
## warnings
f_warning <- function(glmm_object){
  v_message <- glmm_object@optinfo$conv$lme4$messages %>% 
...
save(l_nRefEE_mdCheio,file="~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_nRefEE_mdCheio.Rdata")
```
...
load("~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_nRefEE_mdCheio.Rdata")
...
save(l_md.dredge_nRefEE,file="~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_nRefEE_mdCheio.Rdata")
```
...
save(mdAvg_nRefEE__MuMIn,file="/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/mdAvg_nRefEE__MuMIn.Rdata")
# Model Averaging for new data predictions
df_pred <- expand.grid(SiteCode=unique(df_resultados$SiteCode)[1],
...
mdAvg_nRefEE__AICcmodavg <- modavgPred(l_md.dredge_nRefEE,newdata = df_pred) %>%
  cbind(df_pred)
save(mdAvg_nRefEE__AICcmodavg,
     file="/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/mdAvg_nRefEE__AICcmodavg.Rdata")
```
...
df_dados <- df_resultados %>% filter(MN=="EE") %>% distinct()
df_dados$diffS_mean %>% range %>% round(.,4)
md_diffS <- lmer(diffS_mean ~ 1 + (1|SiteCode), 
                 data=df_dados,na.action = "na.fail")
MuMIn::r.squaredGLMM(md_diffS)
# [,2] %>% round(.,4) %>% as.numeric %>% .*100
summary(md_diffS)
```
...
md_ <- lmer(diffS_mean ~ 
                  ...
                  (d.z|SiteCode),
                   data=df_dados, na.action = "na.fail")
a <- MuMIn::r.squaredGLMM(md_) %>% as.matrix
a[1,2] %>% as.numeric %>% round(.,4)
```
...
## Versão de atualização dos modelos estatísticos usando a %CV para 4x4 km2


```{r GAMM U para 4x4 km2}
lmer_U <- lmer(log10_U ~ log10_Sobs + (1|SiteCode),data=df_U)
beta_log10_Sobs <- fixef(lmer_U)[2] %>% as.numeric() %>% round(.,digits = 3)
# usar o SiteCode como fator foi fundamental para usar a GAMM
df_U$SiteCode <- factor(df_U$SiteCode)
l_md <- list()
l_md[[1]] <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
                 s(log10_d.z,bs="cr",k=15) + s(p.z,bs="cr",k=10) + ti(log10_d.z,p.z,bs=c("cr","cr"),k=c(15,10)) +
                 s(log10_d.z,SiteCode,bs="fs",k=15,xt=list("cr"),m=1),
                data = df_U, family=gaussian, method = "REML")
l_md[[2]] <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
                 s(d.z,bs="cr",k=15) + s(p.z,bs="cr",k=10) + ti(d.z,p.z,bs=c("cr","cr"),k=c(15,10)) +
                 s(d.z,SiteCode,bs="fs",k=15,xt=list("cr"),m=1),
                data = df_U, family=gaussian, method = "REML")
names(l_md) <- c("log(d)","d")
AICctab(l_md,weights=TRUE)
md_U <- l_md[["d"]]
save(md_U,file = "/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/md_U.Rdata")
# exclui o arquivo a seguir:
# save(l_md,file = "/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_md_U.Rdata")
```

```{r}
load("/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/md_U.Rdata")
...
lmer_U <- lmer(log10_U ~ log10_Sobs + (1|SiteCode),data=df_U)
beta_log10_Sobs <- fixef(lmer_U)[2] %>% as.numeric() %>% round(.,digits = 3)
df_newdata <- expand.grid(SiteCode=unique(df_U$SiteCode)[1],
                          # d=seq(min(df_U$d),max(df_U$d),length=40),
                          d.z=seq(min(df_U$d.z),max(df_U$d.z),length=40),
                          p.z=seq(min(df_U$p.z),max(df_U$p.z),length=200),
                          log10_Sobs=0)
df_newdata$predito <- predict(md_U, type = "response",newdata=df_newdata,newdata.guaranteed=TRUE)
beta <- coef(md_U)
V <- vcov.gam(md_U)
...
# dim(beta_sims)
matrix_lprediction <- predict(md_U,type="lpmatrix",newdata=df_newdata,newdata.guaranteed=TRUE) 
...
# grafico
df_plot_U <- cbind(df_newdata,df_pred)
df_plot_U %<>% mutate(p = p.z*sd(df_U$p) + mean(df_U$p),
                      d = d.z*sd(df_U$d) + mean(df_U$d),
                      fit = 10^median,
                      lower = 10^IClower,
                      upper = 10^ICupper)
# gamm atualizado com o que foi usado na última versão, espero que não seja necessário muitos mais ajustes
write.csv(df_plot_U,file="~/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/df_plot_U.Rdata",row.names = FALSE)
```

```{r GAMM U predito, fig.height=6,fig.width=13.5}
# dados
...
l_p[[3]] <- ggplot(df_plot,aes(x=p,y=d,fill=upper)) +
  ...
  geom_text_contour(aes(z=upper),stroke=0.2,
                    ...
                    rotate = FALSE,
                    size=2.7)
grid.arrange(grobs=l_p,ncol=3,top="taxa de colonização de novas espécies na paisagem necessária para manter a riqueza na área amostral")
```
...
```{r gamm U log_d.z k10 , echo=TRUE,eval=FALSE}
#transformar a variavel d foi uma estrategia para melhorar o ajuste
beta_log10_Sobs <- fixef(lmer_U)[2] %>% as.numeric() %>% round(.,digits = 3)
l_md <- list()
l_md[[1]] <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
...
```{r md_U escala d na escala log10__DESCONSIDERAR, eval=TRUE,echo=FALSE,fig.width=9, fig.height=29}
# dados
...
df_plot %>% ggplot(aes(x=log10_d.z,y=log10_U)) +
  ...
  # scale_colour_manual(values=c("red", "blue")) +
  theme(legend.position = "top") + labs(title="log(d).z, bs=tp, k=10") +
  facet_wrap(~title,ncol=5,scales="free")
```
...
```{r aumento do numero de funcoes base de 15 para 18}
beta_log10_Sobs <- fixef(lmer_U)[2] %>% as.numeric() %>% round(.,digits = 3)
l_md <- list()
l_md[[1]] <- md_U
l_md[[2]] <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
                   s(log10_d.z,bs="cr",k=18) + s(p.z,bs="cr",k=10) + ti(log10_d.z,p.z,bs=c("cr","cr"),k=c(15,10)) +
                   s(log10_d.z,SiteCode,bs="fs",k=15,xt = list("cr"),m=1),
                 data = df_U, family=gaussian,method = "REML")
names(l_md) <- c("k=15","k=18")
AICctab(l_md,weights=T)
md_U <- l_md[[1]]
l_md_U_k15 <- l_md 
save(l_md_U_k15,file = "~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/l_md_U_k15.Rdata")
save(md_U,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U_k15_cr.Rdata")
...
df_plot$title <- factor(df_plot$title,levels = unique(df_plot$title))
# gráfico
df_plot %>% ggplot(aes(x=d.z,y=log10_U)) + #log10_
  geom_point(alpha=0.5) + 
  geom_line(aes(x=d.z,y=fit),color="red") + #log10_
  # scale_colour_manual(values=c("red", "blue")) +
  theme(legend.position = "top") +
  facet_wrap(~title,ncol=5,scales="free")
```
Ainda há muitos problemas na fase de patamares, vou tentar ajustar um modelo com penalização por sitio de amostragem

```{r}
lmer_U <- lmer(log10_U ~ log10_Sobs + (1|SiteCode),data=df_U)
beta_log10_Sobs <- fixef(lmer_U)[2] %>% as.numeric() %>% round(.,digits = 3)
md_U_GI <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
                 s(log10_d.z,bs="tp",k=10,m=2) + s(p.z,bs="tp",k=8) + ti(log10_d.z,p.z,bs=c("tp","tp"),k=c(10,8),m=2) +
                 s(SiteCode,bs="re") + s(log10_d.z,by=SiteCode,bs="tp",m=2,k=10),
                data = df_U, family=gaussian,method = "REML")
save(md_U_GI,file = "~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U_GI.Rdata")
```

Meu computador não possível memória suficiente para rodar esse modelo. Vou voltar uma casa e trocar a função de base do modelo anterior para 'cr'
...
l_md[[1]] <- md_U
l_md[[2]] <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
                   s(log10_d.z,bs="cr",k=15) + s(p.z,bs="tp",k=10) + ti(log10_d.z,p.z,bs=c("cr","tp"),k=c(15,10)) +
                   s(log10_d.z,SiteCode,bs="fs",k=15,xt = list("cr")),
                 data = df_U, family=gaussian,method = "REML")
names(l_md) <- c("tp","cr")
AICctab(l_md,weights=T)
l_md_U_k15_comparacao <- l_md 
save(l_md_U_k15_comparacao,file = "~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/l_md_U_k15_comparacao.Rdata")
```

O modelo tp é o único plausível. Acredito que com a atual abordagem de GAM para todo o conjunto de dados, o gráfico anterior será o melhor ajuste que irei obter.

Por final vou comparar os melhores gams para a variável de dispersão per capita 
...
l_md <- list()
l_md[[1]] <- md_U
gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
      s(d.z,bs="cr",k=18) + s(p.z,bs="cr",k=10) + 
      ti(d.z,p.z,bs=c("cr","cr"),k=c(15,10)) +
      s(d.z,SiteCode,bs="fs",k=15,xt = list("cr"),m=1), 
...
l_md[[2]] <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
                   s(d.z,bs="cr",k=18) + s(p.z,bs="cr",k=10) + ti(d.z,p.z,bs=c("cr","cr"),k=c(15,10)) +
                   s(d.z,SiteCode,bs="fs",k=15,xt = list("cr"),m=1),
                 data = df_U, family=gaussian,method = "REML")
names(l_md) <- c("log10(d)","d")
AICctab(l_md,weights=T)
md_U <- l_md[[2]]
gratia::appraise(md_U)
gratia::appraise(l_md[["log10(d)"]])
# df_ <- cbind(data.frame(fitado = l_md[["d"]]$fitted.values,
#                         residuo = l_md[["d"]]$residuals),
#              l_md[["d"]]$model)
# df_ %>% ggplot(aes(x=d.z,y=residuo)) +
#   geom_point()
md_U <- l_md[[2]]
gam.check(l_md[["log10(d)"]])
gam.check(l_md[["d"]])
```
...
```{r}
gratia::appraise(md_U)
model.frame(md_U) # , data = newdata, na.action = na.act
eval(model.frame(md_U, data = df_newdata, na.action = "na.act"), parent.frame())
```
...
df_newdata <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                          ...
                          p.z=seq(min(df_U$p.z),max(df_U$p.z),length=200),
                          log10_Sobs=0)
df_newdata$predito <- predict(gamm, type = "response",newdata=df_newdata,newdata.guaranteed=TRUE)
df_newdata %<>% mutate(p = p.z*sd(df_resultados$p) + mean(df_resultados$p),
                       d = d.z*sd(df_resultados$d) + mean(df_resultados$d),
                       U = 10^predito)
# plots
l_p <- list()
l_p[[1]] <- ggplot(df_newdata,aes(x=p,y=d,fill=U)) +
  coord_cartesian(expand = FALSE) + 
  geom_tile() +
  scale_fill_distiller(palette = "Spectral") +
  theme_classic() + labs(title = "mais plausível (basis=cr)") +
  # facet_wrap(~IC_pred,ncol=1) +
  theme(legend.position="bottom",legend.title = element_blank()) +
  geom_contour(aes(z=U),alpha=0.4, col="black",
               breaks = round(quantile(df_newdata$U,probs = c(0.1,0.25,0.5,0.75,0.9)),5)) +
  geom_text_contour(aes(z=U),stroke0.2,
                    breaks = format(quantile(df_newdata$U,probs = c(0.1,0.25,0.5,0.75,0.9)),scientific=TRUE),
                    rotate = FALSE,
                    size=2.7)
  # dados gerais
gamm <- l_md[[1]]
...
df_newdata <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                          ...
                          p.z=seq(min(df_U$p.z),max(df_U$p.z),length=200),
                          log10_Sobs=0) %>% 
  mutate(log10_d0 = log10(d),
         log10_d.z = (log10_d0-mean(df_resultados$log10_d))/sd(df_resultados$log10_d) )
df_newdata$predito <- predict(gamm, type = "response",newdata=df_newdata,newdata.guaranteed=TRUE)
df_newdata %<>% mutate(p = p.z*sd(df_resultados$p) + mean(df_resultados$p),
                       log10_d = log10_d.z*sd(df_resultados$log10_d) + mean(df_resultados$log10_d),
                       d = 10^log10_d,
                       U = 10^predito)
# plots
l_p[[2]] <- ggplot(df_newdata,aes(x=p,y=d,fill=U)) +
  coord_cartesian(expand = FALSE) + 
  geom_tile() +
  scale_fill_distiller(palette = "Spectral") +
  theme_classic() + labs(title = "menos plausível (basis=tp)") +
  # facet_wrap(~IC_pred,ncol=1) +
  theme(legend.position="bottom",legend.title = element_blank()) +
  geom_contour(aes(z=U),alpha=0.4, col="black",
               breaks = round(quantile(df_newdata$U,probs = c(0.1,0.25,0.5,0.75,0.9)),5)) #+
  # geom_text_contour(aes(z=U),stroke=0.2,
                    # breaks = format(round(quantile(df_newdata$U,probs = c(0.1,0.25,0.5,0.75,0.9)),5),scientific=TRUE),
                    # rotate = FALSE,
                    # size=2.7)
grid.arrange(grobs=l_p,ncol=2)
```
...
Prosigo com o modelo mais plausivel

```{r}
load("~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U_k15_cr.Rdata")
```
...
# grafico
df_plot_U <- cbind(df_newdata,df_pred)
df_plot_U %<>% mutate(p = p.z*sd(df_resultados$p) + mean(df_resultados$p),
                      d = d.z*sd(df_resultados$d) + mean(df_resultados$d),
                      fit = 10^median,
                      lower = 10^IClower,
                      upper = 10^ICupper)
write.csv(df_plot_U,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/df_plot_U.Rdata",row.names = FALSE)
```
...
df_plot <- read.csv("~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/df_plot_U.Rdata",
                      header = TRUE,as.is = TRUE)
apply(df_plot[,c("fit","lower","upper")],2,function(X) formatC(X,format="e")) %>% summary
## graficos
l_p <- list()
# 
df_plot$IC_pred <- "5% quantile"
...
### 1o) comparar estruturas aleatórias e variáveis de dispersão:
...
# modelos
l_md <- list()
l_md[[1]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (d.z + I(d.z^2) + I(d.z^3)) + 
                      (d.z + I(d.z^2) + I(d.z^3)|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
l_md[[2]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (d.z + I(d.z^2) + I(d.z^3)) + 
                      (d.z + I(d.z^2)|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
l_md[[3]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (d.z + I(d.z^2) + I(d.z^3)) + 
                      (d.z|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
l_md[[4]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (d.z + I(d.z^2) + I(d.z^3)) + 
                      (1|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
l_md[[5]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (log10_d.z + I(log10_d.z^2) + I(log10_d.z^3)) + 
                      (log10_d.z + I(log10_d.z^2) + I(log10_d.z^3)|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
v_star <- getME(l_md[[5]],c("theta","fixef"))
l_md[[5]] <- update(l_md[[5]],start=v_star)
l_md[[6]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (log10_d.z + I(log10_d.z^2) + I(log10_d.z^3)) + 
                      (log10_d.z + I(log10_d.z^2)|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
v_star <- getME(l_md[[6]],c("theta","fixef"))
l_md[[6]] <- update(l_md[[6]],start=v_star)
l_md[[7]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (log10_d.z + I(log10_d.z^2) + I(log10_d.z^3)) + 
                      (log10_d.z|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
l_md[[8]] <- lmer(log10_U ~ offset(beta_log10_Sobs * log10_Sobs) + 
                      (p.z + I(p.z^2)) * (log10_d.z + I(log10_d.z^2) + I(log10_d.z^3)) + 
                      (1|SiteCode),
                 data=df_U,
                 na.action = "na.fail")
names(l_md) <- c("d ^3|Site",
                 "d ^2|Site",
                 "d ^1|Site",
                 "d 1|Site",
                 "log10(d) ^3|Site",
...
df_ranef %>% 
  ...
  ggplot(aes(sample=parameter_value)) + 
  stat_qq(alpha=0.3) + stat_qq_line(alpha=0.3,col="red") +
  facet_wrap(~parameter_class,scales="free",ncol=2)
...
```{r}
  # dados
df_md <- md_U@frame
df_md$observado <- df_md$log10_U
df_md$predito <- predict(md_U,type="response")
# graficos
ggplot(df_md,aes(x=observado,y=predito)) +
  geom_point(alpha=0.3) +
  geom_smooth(method = "lm",aes(group=SiteCode),se=FALSE,alpha=0.3,color="blue") +
  geom_abline(slope = 1,intercept = 0,col="red",alpha=0.8,size=1)
```

```{r}
# todas as combinações de modelo do modelo cheio
# registerDoMC(3)
lmer(formula = log10_U ~ I(log10_d.z^2) + (log10_d.z + I(log10_d.z^2) + 
    I(log10_d.z^3) | SiteCode) + offset(beta_log10_Sobs * log10_Sobs), 
    data = df_U, na.action = "na.fail")


l_call.md <- dredge(md_U,trace = FALSE,evaluate=FALSE,fixed = "offset(beta_log10_Sobs * log10_Sobs)")
l_md.dredge_U <- llply(dredge(md_U,trace = FALSE,evaluate=FALSE,fixed = "offset(beta_log10_Sobs * log10_Sobs)"), eval) #,.parallel = TRUE
teste <- laply(dredge(md_U,trace = FALSE,evaluate=FALSE,fixed = "offset(beta_log10_Sobs * log10_Sobs)"), eval) #,.parallel = TRUE

save(l_md.dredge_U,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.dredge_U.Rdata")
```
