...
```{r}
ggplot(df_plot,aes(x=df_plot$U_med)) +
...
# função
f_md <- function(dados){
  dados <- df_md %>% filter(md_class == "tp")
  ...
  if(model_class == "tp"){
  ...
  }else if(model_class == "ts"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="ts") + s(p.z,bs="ts") + ti(d.z,p.z,bs=c("ts","ts")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="ts"), m=1),
                data = dados, Gamma(link = "log"))
    ...
    md_ <- gam(U_med ~ 
                ...
                data = dados, Gamma(link = "log"))
  }else if(model_class == "cs"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="cs") + s(p.z,bs="cs") + ti(d.z,p.z,bs=c("cs","cs")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="cs"), m=1),
                data = dados, Gamma(link = "log"))
...
```{r}
dados <- df_resultados %>% filter(MN=="EE") %>% distinct
l_md <- list()
l_md[[1]] <- gam(U_med ~ 
                 s(d.z,bs="tp") + s(p.z,bs="tp") + ti(d.z,p.z,bs=c("tp","tp")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="tp"), m=1),
