...
```{r preparacao dos dados 1,message=FALSE}
# dados
...
filter(df_resultados,MN=="EI") %>% select(diff_S0, p, d, Stotal, Ntotal) %>% ggpairs() + labs(title="MNEI")
# do.call("grid.arrange",c(l_p,ncol=1))
```

__Figura 2.1__ diff_S = (S_obs - S_MN)/S_obs + (-1) * min(diff_S) + 0.01, inclinação_MNEE ~ 0 para todo k.

Para MNEE vou utilizar uma distribuição normal e avaliar
