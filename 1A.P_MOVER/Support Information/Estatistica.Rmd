...
# função
f_md <- function(dados){
  model_class <- unique(dados$md_class)
  if(model_class == "tp"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="tp") + s(p.z,bs="tp") + ti(d.z,p.z,bs=c("tp","tp")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="tp"), m=1),
                data = dados, Gamma(link = "log"))
  }else if(model_class == "ds"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="ds") + s(p.z,bs="ds") + ti(d.z,p.z,bs=c("ds","ds")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="ds"), m=1),
                data = dados, Gamma(link = "log"))
  }else if(model_class == "cr"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="cr") + s(p.z,bs="cr") + ti(d.z,p.z,bs=c("cr","cr")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="cr"), m=1),
                data = dados, Gamma(link = "log"))
  }else if(model_class == "cc"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="cc") + s(p.z,bs="cc") + ti(d.z,p.z,bs=c("cc","cc")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="cc"), m=1),
                data = dados, Gamma(link = "log"))
  }else if(model_class == "ps"){
    md_ <- gam(U_med ~ 
                 s(d.z,bs="ps") + s(p.z,bs="ps") + ti(d.z,p.z,bs=c("ps","ps")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="ps"), m=1),
                data = dados, Gamma(link = "log"))
  }else{
      md_ <- gam(U_med ~ 
                 s(d.z,bs="gp") + s(p.z,bs="gp") + ti(d.z,p.z,bs=c("gp","gp")) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="gp"), m=1),
                data = dados, Gamma(link = "log"))
  }
  return(md_)
}
registerDoMC(3)
l_md.U_gamEstudo <- dlply(df_md,"md_class",f_md,.parallel = TRUE)
```
