...
df_plot$logit_nRef <- car::logit(df_plot[,1][,1]/100)
df_plot %<>% mutate(res_ = logit_nRef - .fitted,
                    p = p.z * sd(df_resultados$p) + mean(df_resultados$p))

## 
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=res_)) + 
  geom_point() + geom_abline(intercept=0,slope = 0,col="red") + facet_grid(k~MN) +
  ylab("logit_obs - logit_fit") + xlab("Proportion of Tree Cover")
l_p[[2]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[11:20]),aes(x=p,y=res_)) + 
  geom_point() + geom_abline(intercept=0,slope = 0,col="red") + facet_grid(k~MN) +
  ylab("logit_obs - logit_fit") + xlab("Proportion of Tree Cover")
do.call("grid.arrange",c(l_p,ncol=2))
#
...
```{r}
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_nRef,k %in% levels(df_resultados$k)[1:10]),aes(x=p.z,y=dev_Res)) + 
  geom_point() + geom_smooth() +
  facet_grid(k~MN)
l_p[[2]] <- ggplot(filter(df_nRef,k %in% levels(df_resultados$k)[11:20]),aes(x=p.z,y=dev_Res)) + 
  geom_point() + geom_smooth() +
  facet_grid(k~MN)
do.call("grid.arrange",c(l_p,ncol=2))
