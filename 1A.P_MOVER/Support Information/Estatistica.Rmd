...
df_plot$title <- factor(df_plot$title,levels = unique(df_plot$title))
df_plot %>% ggplot(aes(x=d,y=log(U_med))) +
  geom_smooth(method = "loess",se = FALSE,color="red",alpha=0.5) +
...
df_U %>% ggplot(aes(x=log_Sobs,y=log(U_med))) +
...
```{r}
ggplot(df_U,aes(x=S_obs)) +
  geom_histogram(bins=60)  + labs(x="",title = "Species Richness")
```
