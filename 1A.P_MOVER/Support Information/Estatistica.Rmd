...
```{r backup 1, eval=FALSE,echo=FALSE,include=FALSE }
l_md <- vector("list",8)
names(l_md) <- c("k 1|Site","k MN|Site",
                 "d 1|Site","d MN|Site","d d*MN|Site",
                 "d_Lplot 1|Site","d_Lplot MN|Site","d_Lplot d_Lplot*MN|Site")
# k
...
l_md[[4]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                       ...
                       family = "binomial",data=df_resultados,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
l_md[[5]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (d.z + I(d.z^2)) * MN + 
                         ( (d.z + I(d.z^2) ) * MN|SiteCode),
                       family = "binomial",data=df_resultados,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
# d_Lplot
l_md[[6]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (d_Lplot.z + I(d_Lplot.z^2)) * MN + 
                         (1|SiteCode),
                       family = "binomial",data=df_resultados,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
l_md[[7]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (d_Lplot.z + I(d_Lplot.z^2)) * MN + 
                         (MN|SiteCode),
                       family = "binomial",data=df_resultados,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
l_md[[8]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (d_Lplot.z + I(d_Lplot.z^2)) * MN + 
                         ( (d.z + I(d.z^2) ) * MN|SiteCode),
                       family = "binomial",data=df_resultados,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e8)))
...
f_md <- function(dados){
  ...
  if(var_dispersao == "k"){
    ...
    # dados <- df_md %>% filter(var_dispersao == "k")
    l_md[[1]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
    ...
    # dados <- df_md %>% filter(var_dispersao == "d.z")
...
#### Avaliação de cada modelo que não passou no teste de convergência 


```{r}
df_md <- df_resultados %>% 
  select(n_nRef,p,SiteCode,MN,p.z,d.z,d_Lplot.z) %>% 
# df_md[,6:8] <- apply(df_md[,6:8],2,as.character)
# df_md %<>% 
  gather(key = var_dispersao, value = value_dispersao, d.z:d_Lplot.z) 
#
i <- 4
...
## update1:
dados <- df_md %>% filter(var_dispersao == "d.z")
v_par <- getME(l_nRef__modeloCheio[v_glmerUpdate][[i]],
               c("theta","fixef"))
# if 1|Site
l_nRef__modeloCheio[v_glmerUpdate][[i]] <- 
  glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (value_dispersao + I(value_dispersao^2)) * MN + 
                         (1|SiteCode),
                       family = "binomial",data=dados,start = v_par,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e10)))
# if MN|Site    
l_nRef__modeloCheio[v_glmerUpdate][[i]] <- 
  glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (value_dispersao + I(value_dispersao^2)) * MN + 
...
5: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model failed to converge with max|grad| = 0.00342971 (tol = 0.002, component 1)
...
4: In optwrap(optimizer, devfun, start, rho$lower, control = control,  :
  convergence code 1 from bobyqa: bobyqa -- maximum number of function evaluations exceeded
5: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model failed to converge with max|grad| = 0.00838345 (tol = 0.002, component 1)
6: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model is nearly unidentifiable: very large eigenvalue
 - Rescale variables?
...
4: In optwrap(optimizer, devfun, start, rho$lower, control = control,  :
  convergence code 1 from bobyqa: bobyqa -- maximum number of function evaluations exceeded
5: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model failed to converge with max|grad| = 40.1829 (tol = 0.002, component 1)
6: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model is nearly unidentifiable: very large eigenvalue
 - Rescale variables?
...
4: In optwrap(optimizer, devfun, start, rho$lower, control = control,  :
  convergence code 1 from bobyqa: bobyqa -- maximum number of function evaluations exceeded
5: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model failed to converge with max|grad| = 0.00749388 (tol = 0.002, component 1)
6: In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  Model is nearly unidentifiable: very large eigenvalue
 - Rescale variables?    
...
```{r rotina de ajustes,eval=FALSE,echo=FALSE }
#
##
df_md <- df_resultados %>% 
  select(n_nRef,p,SiteCode,MN,p.z,d.z,d_Lplot.z)
df_md[,6:7] <- apply(df_md[,6:7],2,as.character)
df_md %<>% 
  gather(key = var_dispersao, value = value_dispersao, d.z:d_Lplot.z) # %>% 
  # group_by(var_dispersao) %>% nest()
##
# f_dply <- function(glmer_object){
#   l_nRef__modeloCheio[[1]]
# }
# 
# 
# df_mdList <- ldply()
...
v_par <- getME(l_nRef__modeloCheio[v_glmerUpdate][[i]],
               c("theta","fixef"))
## if 1|Site
l_nRef__modeloCheio[v_glmerUpdate][[i]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                         (p.z + I(p.z^2)) * (value_dispersao + I(value_dispersao^2)) * MN + 
                         (1|SiteCode),
                       family = "binomial",data=dados,
                       control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
...
## update: inicio com os parâmetros estimados
f_update1 <- function(glmm_object){
  # glmm_object <- l_nRef__modeloCheio[v_glmerUpdate][[2]]
  v_vars <- getME(glmm_object,c("theta","fixef"))
  glmm_update <- update(glmm_object,start=v_vars)
}
############################################
################# error ####################
############################################
# Error in do.ply(i) : 
# task 1 failed - "'data' not found, and some variables missing from formula environment"
# registerDoMC(3)
# l_nRef__modeloCheio[v_glmerUpdate] <- llply(l_nRef__modeloCheio[v_glmerUpdate],f_update1) # ,
                                            # .parallel = TRUE,
                                            # .paropts=list(.export=c('data','main.function'),.packages=.packages(all.available=T)))
############################################
################# error ####################
############################################
for(i in 1:length(l_nRef__modeloCheio[v_glmerUpdate])){
  v_vars <- getME(l_nRef__modeloCheio[v_glmerUpdate][[i]],
                  c("theta","fixef"))
  l_nRef__modeloCheio[v_glmerUpdate][[i]] <- update(l_nRef__modeloCheio[v_glmerUpdate][[i]],
                                                    start=v_vars)
}
df_auditoria.md<- ldply(l_nRef__modeloCheio[c(v_glmerUpdate)],f_warning,.id = "glmer") %>% 
  dplyr::rename(warning_message = V1)
v_glmerUpdate <- df_auditoria.md[df_auditoria.md$warning_message != "OK",]$glmer %>% 
  as.character()
## update: aumentar número de iterações do otimizador
################### mesmo erro ###################
# f_update2 <- function(glmm_object){
#   glmm_update <- update(glmm_object,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e10)))
# }
# registerDoMC(3)
# l_nRef__modeloCheio[v_glmerUpdate] <- llply(l_nRef__modeloCheio[c(v_glmerUpdate)], f_update1,.parallel = TRUE) 
################### mesmo erro ###################
for(i in 1:length(l_nRef__modeloCheio[v_glmerUpdate])){
  l_nRef__modeloCheio[v_glmerUpdate][[i]] <- update(l_nRef__modeloCheio[v_glmerUpdate][[i]],
                                                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e10)))
...
v_glmerUpdate <- df_auditoria.md[df_auditoria.md$warning_message != "OK",]$glmer %>% 
  as.character()
## update: try all optimizers
temp <- allFit(l_nRef__modeloCheio[v_glmerUpdate][[1]],parallel = 'multicore', ncpus = 3)
...
```{r Tabela 21,eval=FALSE,cache=FALSE,include=FALSE}
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md_nRef_GLMM.Rdata")
df_auditoria.md <- ldply(l_nRef__modeloCheio,f_warning,.id = "glmer") %>% 
  rename(warning_message = V1)
knitr::kable(df_auditoria.md)
...
<!-- __Tabela 2.2__  Ajuste obtido para os modelos após as rotinas de melhora      -->
  
    
```{r Tabela 22,eval=FALSE,include=FALSE}
AICctab(l_nRef__modeloCheio)
```

    
```{r auditoria mais plausivel md_n.ref1, message=FALSE,eval=FALSE,include=FALSE}
# library(DHARMa)
md_nRef <- l_md_nRef.GLMM[["d.z d.z*MN|Site"]]
...
```{r avaliacao por site fig 14,fig.width=8, fig.height=30,message=FALSE,eval=FALSE,include=FALSE}
# Dados
df_plot <- df_md %>% filter(var_dispersao == "d.z")
df_plot$value_dispersao <- as.numeric(df_plot$value_dispersao)
# unique(df_resultados$d.z == df_plot$value_dispersao)
# class(df_resultados$d.z)
# unique(df_resultados$n_nRef == df_plot$n_nRef)
