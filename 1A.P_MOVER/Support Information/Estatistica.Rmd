...
l_p[[1]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=n_nRef)) + 
  ...
  ylab("logit(not ref. SAD)") + xlab("Proportion of Tree Cover") #+
...
```{r method smoothing parameter n_nRef e bs type}
...
############# bs class
l_md.BS <- vector("list",length = 8)
names(l_md.BS) <- c("tp","ts","cr","cs","cc","ps","cp","ad")
...
l_md.BS[[2]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="ts") +
                   ...
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md.BS[[3]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cr") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md.BS[[4]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cs") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md.BS[[5]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cc") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md.BS[[6]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="ps") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md.BS[[7]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cp") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md.BS[[8]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="ad") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_ <- l_md.BS[c(1,3,5:7)]
AICctab(l_,weights="TRUE")
# AICctab(l_md.SP[[4]],l_md.BS[[3]],weights=TRUE)
# appraise(l_md.BS[["cr"]])
...
Para MNEI o problema persiste.
  
##### transformação do conjunto de dados e distribuicao normal e gamma


```{r transformacao para logit e uso de outras distribuicoes}
# dados
df_resultados %<>% mutate(logit1_nRef = logit_nRef + min(logit_nRef)*(-1) +0.01)
# grafico
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_resultados,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=logit1_nRef)) + geom_point() + facet_grid(k~MN)
l_p[[2]] <- ggplot(filter(df_resultados,k %in% levels(df_resultados$k)[11:20]),aes(x=p,y=logit1_nRef)) + geom_point() + facet_grid(k~MN)
do.call("grid.arrange",c(l_p,ncol=2))
# modelos
l_md.Distr <- vector("list",length = 3)
names(l_md.Distr) <- c("normal","gamma inverse","gamma log")
l_md.Distr[[1]] <- gam(logit1_nRef ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cr") +
                        s(SiteCode,by=MN,bs="re"),
                        family = "gaussian",
                        data = df_resultados, method = "REML")
l_md.Distr[[2]] <- gam(logit1_nRef ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cr") +
                        s(SiteCode,by=MN,bs="re"),
                        family = "Gamma"(link = "inverse"),
                        data = df_resultados, method = "REML")
l_md.Distr[[3]] <- gam(logit1_nRef ~ k * MN + s(p.z,k,by=MN,bs="fs",xt="cr") +
                        s(SiteCode,by=MN,bs="re"),
                        family = "Gamma"(link = "log"),
                        data = df_resultados, method = "REML")
AICctab(l_md.Distr,weights=TRUE)
# save(l_md.Distr,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_nRef_Dist.Rdata")
# save(l_md,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_nRef.Rdata")
```

__Figura 1.9__ Resíduos logit_nRef normal  
  
    
```{r logit_nRef normal predicao e observacao}
## predição e observação
### dados
df_pred.k <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                          MN=levels(df_resultados$MN),
  ...
  mutate(p = p.z * sd(df_resultados$p) + mean(df_resultados$p))
df_pred.k <- cbind(df_pred.k,
                   predict.gam(l_md.Distr[[1]],
                           df_pred.k,type="response",se.fit=TRUE))
df_pred.k[,c("fit","se.fit")] <- arm::invlogit(df_pred.k[,c("fit","se.fit")]) 
### graficos
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_resultados,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=logit_nRef)) + 
  geom_point(alpha=0.3) + facet_grid(k~MN) +
  # k
  geom_ribbon(aes(ymin=I(fit - 2*se.fit), ymax=I(fit + 2*se.fit), x=p),
              data=filter(df_pred.k,k %in% levels(df_resultados$k)[1:10]),
  ...
  geom_line(aes(y=fit),color="white", data=filter(df_pred.k,k %in% levels(df_resultados$k)[1:10])) +
  ylab("n_nRef") + xlab("Proportion of Tree Cover") #+
l_p[[2]] <- ggplot(filter(df_resultados,k %in% levels(df_resultados$k)[11:20]),aes(x=p,y=logit_nRef)) + 
  geom_point(alpha=0.3) + facet_grid(k~MN) +
  # k
  geom_ribbon(aes(ymin=I(fit - 2*se.fit), ymax=I(fit + 2*se.fit), x=p),
              data=filter(df_pred.k,k %in% levels(df_resultados$k)[11:20]),
...
```{r gam binomial n_nRef MN=EI, echo=TRUE}
l_md.EE <- vector("list",3)
names(l_md.EE) <- c("k 1|SiteCode","k_1 1|SiteCode","d/L 1|SiteCode")
# muitos parametros para poucos dados "k_1 k_1 * MN|SiteCode","d_Lplot d_Lplot * MN|SiteCode")
# k
...
# k_1
l_md.EE[[2]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,bs="cr") + s(k_1.z,bs="cr") + ti(p.z,k_1.z,bs=c("cr","cr")) +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = filter(df_resultados,MN=="EE"), method = "REML")
# d/L
l_md.EE[[3]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,bs="cr") + s(d_Lplot.z,bs="cr") + ti(p.z,d_Lplot.z,bs=c("cr","cr")) +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = filter(df_resultados,MN=="EE"), method = "REML")
...
```{r modelo mais plausivel n_nRef MNEI}
gratia::appraise(l_md.EE[[1]])
```
