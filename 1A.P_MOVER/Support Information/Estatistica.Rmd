...
#                  s(log10_d.z,SiteCode,bs="fs",k=10),
...
#                  s(log10_d.z,SiteCode,bs="fs",k=15),
#                 data = df_U, family=gaussian, method = "REML")
# names(l_md) <- c("k=10","k=15")
# AICctab(l_md,weights=TRUE)
# l_md_GAMM_U <- l_md
# save(l_md_GAMM_U,file = "/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/piloto_MNEE_4x4km/resultados/l_md_GAMM_U.Rdata")
md_U <- gam(log10_U ~ offset(beta_log10_Sobs*log10_Sobs) +
...
df_plot_U %<>% mutate(p = p.z*sd(df_U$p) + mean(df_U$p),
                      log10_d = log10_d.z*sd(df_U$log10_d) + mean(df_U$log10_d),
                      d = 10^log10_d,
                      fit = 10^median,
                      lower = 10^IClower,
                      upper = 10^ICupper)
...
```{r GAMM U predicao para novo conjunto de dados}
