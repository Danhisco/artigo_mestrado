...
registerDoMC(2)
l_md <- dlply(df_md,"model_class",f_md,.parallel = TRUE)
```


```{r l_nRefEE_estudoGAMM, echo=FALSE, eval=FALSE}
l_nRefEE_estudoGAMM <- l_md
...
```{r AICctab ,eval=TRUE,echo=FALSE}
...
```{r comparacao modelos cheios,echo=TRUE,eval=TRUE}
# dados
...
md_nRefEE <- l_nRefEE_mdCheio[["gamm d|Site for each"]]
```
...
__Figura 2.2.1__ Observado e predito pelo modelo cheio mais plausível  

```{r fig 221 predito e observado por sitio md_nRefEE, echo=FALSE,eval=TRUE, fig.width=8, fig.height=30,message=FALSE}
# dados
df_plot <- df_resultados %>% filter(MN=="EE") %>% distinct()
df_plot$nRef_predito <- predict.gam(md_nRefEE,se.fit = TRUE,type="response")$fit*100
df_plot$nRef_se <- predict.gam(md_nRefEE,se.fit = TRUE,type="response")$se*100
df_plot %<>%
...
__Figura 2.2.2__ Diagnostico: avaliação do ajuste pelas preditoras lineares

```{r fig 222,echo=FALSE,eval=TRUE,fig.height=5}
#
df_plot <- augment(md_nRefEE) 
df_plot1 <- df_plot %>% 
...
```{r comparacao modelos cheios,echo=TRUE,eval=FALSE}
# dados
...
__Figura 2.2.1__ Gráficos Diagnostico modelo cheio mais plausível
