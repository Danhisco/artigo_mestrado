...
__Figura 1.15__ Resíduos Quantílicos do modelo cheio para n_nRef ~ p*d_Lplot*MN + (d_Lplot*MN|SiteCode)
...
__Figura 1.16__ Valores ajustados contra as preditoras lineares  
...
__Figura 1.17__ Predito e observado por SiteCode  
...
__Figura 1.18__ Probabilidade predito pelo modelo mais plausível  
...
# load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md_nRef_GLMM_EE.Rdata")
```
...
__Figura 1.19__ Resíduos Quantílicos do glmm cheio para MNEE  

  
#### Seleção de Variáveis
  

```{r glmm quadratico n_nRef MNEE selecao de variaveis}
...
dredge_nRef.EE <- dredge(global_md,trace = 2)
```



__Predições dos Modelos Plausíveis e Model Averaging destes__

```{r fig 120 predicoes por SiteCode }
library(AICcmodavg)
df_pred <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
...
teste <- modavgPred(l_md.nRef.GLMM_EEvars,newdata = filter(df_resultados,MN=="EE"))
teste %>% as.data.frame()
```
