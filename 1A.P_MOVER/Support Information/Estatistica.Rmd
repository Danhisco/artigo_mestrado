...
```{r modelo global, eval=FALSE,echo=TRUE}
md_nRef <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                   (p.z + I(p.z^2)) * (d.z + I(d.z^2)) * MN + 
                   ( (d.z + I(d.z^2) ) * MN|SiteCode),
                 family = "binomial",data=df_resultados,
...
__Tabela 2.5__ Tabela de Seleção de Modelos baseado em AICc


```{r modelo global dredge n_nRef,eval=FALSE,echo=FALSE}
dredge_glmmEE <- dredge(md_nRef,trace = FALSE,evaluate=TRUE)
dredge_GLMMnRef <- dredge_glmmEE
save(dredge_GLMMnRef,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/dredge_GLMMnRef1.Rdata")
```
  
  
```{r nRef dredge modelo subset>7,eval=TRUE,echo=FALSE}
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/dredge_GLMMnRef1.Rdata")
subset(dredge_GLMMnRef,delta<7)
```
...
registerDoMC(3)
l_md.dredge_nRef <- llply(dredge(md_nRef,trace = FALSE,evaluate=FALSE),eval,.parallel = TRUE)
save(l_md.dredge_nRef,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/dredge_GLMMnRef2.Rdata")
# Model Averaging for SiteCode predictions
mdAvg_nRef__MuMIn <- model.avg(l_md.dredge_nRef)
save(mdAvg_nRef__MuMIn,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_nRef__MuMIn.Rdata")
...
mdAvg_nRef__AICcmodavg <- modavgPred(l_md.dredge_nRef,newdata = df_pred) %>%
  cbind(df_pred)
save(mdAvg_nRef__AICcmodavg,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_nRef__AICcmodavg.Rdata")
```
