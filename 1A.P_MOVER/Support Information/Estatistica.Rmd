...
```{r glmm n_nRef comparacao modelos cheios,include=FALSE,echo=FALSE,eval=FALSE,cache=TRUE}
l_md <- vector("list",11)
...
__Figura 2.3__ diff_S para MNEI, preditoras de interesse e alguns possíveis variáveis de controle (número de indivíduos observado (Ntotal) e riqueza observada (Stotal))  
...
ggplot(df_plot,aes(x=d,y=diff_S)) +
  geom_point() +
...
```{r modelo cheio diff_S MNEI, echo=TRUE,warning=TRUE,message=TRUE}
...
```{r diff_S MNEI GAMM cheio,eval=FALSE}
l_md <- vector("list",4)
...
```{r updata gamm cheio mais plausivel diff_S MNEI,eval=FALSE}
l_md <- vector("list",2)
names(l_md) <- c("1|Site","d|Site")
...
appraise(l_md.diff_S.EI__GAMM2[["d|Site"]])
```
...
```{r updata gamm cheio mais plausível d Site,eval=FALSE}
df_md <- filter(df_resultados,MN=="EI")
l_md.diff_S.EI__GAMM2[[2]] <- gam(diff_S ~ s(p.z,bs="cr") + s(d.z,bs="cr") + ti(p.z,d.z,bs=c("cr","cr")) +
                   s(d.z,SiteCode,bs="fs",xt=list(bs="cp"), m=1),
                 data=df_md,family = "Gamma"(link="log"))

md_gam.diff_S.EI__update <- l_md.diff_S.EI__GAMM2[[2]]
...
```{r  diff_S MNEI gamm cheio segundo plausivel,message=FALSE,warning=FALSE,results="hide"}
gratia::draw(l_md.diff_S.EI__GAMM2[["d|Site"]])
gratia::appraise(l_md.diff_S.EI__GAMM2[["d|Site"]])
```
  
__Figura 2.8__ gratia::draw() gamm (1|Site)  
  
### Figura Final

```{r gamm diff_S predicao novo conjunto de dados}
df_pred <- df_pred <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                       d.z=seq(min(df_resultados$d.z),max(df_resultados$d.z),length=20),
                       p.z=seq(min(df_resultados$p.z),max(df_resultados$p.z),length=103))
df_pred <- cbind(df_pred,
                 predict(l_md.diff_S.EI__GAMM2[["1|Site"]],df_pred,type="response",se.fit=TRUE)) %>% 
  mutate(p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
         d = d.z * sd(df_resultados$d) + mean(df_resultados$d))
```


```{r predito pelo modelo cheio selecionado, fig.width=8, fig.height=30,eval=FALSE}
df_plot <- df_resultados %>% filter(MN=="EI")
df_plot$predicao <- predict(l_md[[4]],type = "response")
# Graficos
df_plot %<>% mutate(title = paste0(SiteCode, " p=",round(p,digits = 3))) %>% arrange(p)
df_plot$title <- factor(df_plot$title,levels = unique(df_plot$title))
ggplot(df_plot,aes(x=d.z,y=diff_S)) +
  geom_point() +
  geom_line(aes(y=predicao),color="red") +
  # theme_classic() + theme(legend.position="top") + 
  facet_wrap(~title,ncol=5,scales="free")
```

<!-- __Figura 2.6__ Predito pelo modelo mais plausível para diff_S MNEI pelo sítio de amostragem -->

<!-- Há um erro sistemático para superestimar valores elevados de d.z e talvez de subestimar para valores do 1o quarto de d.z.   -->
  
```{r fig 25 predicao para modelo cheio,eval=FALSE}
# Dados
df_pred <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                       d.z=seq(min(df_resultados$d.z),max(df_resultados$d.z),length=20),
                       p.z=seq(min(df_resultados$p.z),max(df_resultados$p.z),length=103))
df_pred$predito <- predict(l_md[[4]],df_pred,type="response")
...
teste <- glm(diff_S ~ p.z*d.z,data=df_md,family="Gamma"(link="identity"))
df_plot$predito <- predict(teste,type="response")
# Graficos
ggplot(df_plot,aes(x=p.z,y=d.z,fill=predito)) + geom_tile() 
...
<!-- O problema de estimativa errada persisteF701
. Vou avaliar outros tipos de smoothers. -->
