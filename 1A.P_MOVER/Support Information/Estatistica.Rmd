...
#                      (p.z + I(p.z^2)) * (d.z + I(d.z^2)) * MN + (d.z + I(d.z^2)|SiteCode),
#                    family = "binomial",data=df_resultados,
                   # control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
__tabela modelo cheio nRef MNEE__

```{r tabela md cheio para MNEE nRef}
tab_model(l_md_nRef.GLMM_EE[["d + d^2|Site"]])
```


  
   
```{r avaliacao por site fig 17 glmm EE p2 d2,fig.width=8, fig.height=30,message=FALSE}
# Dados
...
__Figura 1.8__ Predições por sítio de amostragem do modelo médio calculado a partir do modelo global 
...
__Figura 1.9__ Efeito predito de d e p na Probabilidade de não refutar segundo o modelo médio para MN==EE.  No painel superior a predição média, nos paineis inferiores o intervalo de confiança.  
...
```{r modelo global n_nRef MNEI, include=TRUE,echo=TRUE,cache=TRUE}
# dados
