...
```{r selecao de variaveis diffS_EI,eval=TRUE,echo=FALSE,results='hide'}
# AICctab
...
l_p[[1]] <- ggplot(df_plot,aes(x=p,y=d,fill=mod.avg.pred,z=mod.avg.pred)) +
  ...
  coord_cartesian(expand = FALSE) +
  theme(legend.position="top") +
  scale_fill_gradient2(midpoint=0, low="blue", high="red",mid="green") +
  ...
  labs(title="Viés da Riqueza Estimada",
       fill="") + theme_classic() + facet_wrap(~MN,ncol=1)
...
l_p[[2]] <- ggplot(df_plot,aes(x=p,y=d,fill=lower.CL,z=lower.CL)) +
  ...
  geom_contour(breaks=c(-0.25,0.25),color="red",alpha=0.5) +
  labs(title="",
       fill="") + theme_classic() + facet_wrap(~lowerIC,ncol=1)
...
l_p[[3]] <- ggplot(df_plot,aes(x=p,y=d,fill=upper.CL,z=upper.CL)) +
  ...
  theme(legend.position="none") +
  labs(title="",
       fill="") + theme_classic() + facet_wrap(~upperIC,ncol=1)
