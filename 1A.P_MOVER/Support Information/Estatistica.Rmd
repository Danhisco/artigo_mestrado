...
# merge2
df_references$refID %<>% as.integer()
...
# merge para replicas
df_replicas %<>% left_join(x=.,y=df_references,by=c("ordem","refID"))
####################
df_replicas %>% head
```

### avaliação de df_replicas

```{r df_replicas auditoria}
...
df_resultados %<>% left_join(x=.,                            y=select(df_simulacao,SiteCode,k,p,Ntotal,Stotal,U_med,U_var,DA,d,m_,theta),
                             by=c("SiteCode","k")) 
df_resultados %>% names
