...
md_U_sSobs <- gam(U_med ~ 
             ...
             s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="tp",m=1,k=10),
             data = df_md, family = Gamma(link="log"),method = "REML")
save(md_U_sSobs,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_U_sSobs.Rdata")
```
...
# gamm <- md_U.tp
# gamm s Sobs
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_U_sSobs.Rdata")
gamm <- md_U_sSobs
...
df_plot <- cbind(df,df_pred) %>% 
  mutate(fit = exp(median),
         ...
         p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
         d = d.z * sd(df_resultados$d) + mean(df_resultados$d))
write.csv(df_plot,file = "~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/df_plotPredU_sSobs.csv",row.names = FALSE)
```
