...
# names(df_resultados)
df_resultados.z <- as.data.frame(apply(df_resultados[c("p","Ntotal","Stotal","k.0","k.0_1","d_L.plot")],2,f_z))
...
l_md[[5]] <- gam(U_med ~ s(d_L.plot.z) + s(p.z) + ti(d_L.plot.z,p.z) +
                ...
                data = df_resultados, method = "REML",control=ctrl)
# k_1.z
l_md[[6]] <- gam(U_med ~ s(k_1.z) + s(p.z) + ti(k_1.z,p.z) +
                  log(Stotal) +
                  s(SiteCode,bs="re"),
                family="gaussian",
                data = df_resultados, method = "REML",control=ctrl)
l_md[[7]] <- gam(U_med ~ s(k_1.z) + s(p.z) + ti(k_1.z,p.z) +
                  log(Stotal) +
                  s(SiteCode,bs="re"),
                family=gaussian(link = "log"),
                data = df_resultados, method = "REML",control=ctrl)
l_md[[8]] <- gam(U_med ~ s(k_1.z) + s(p.z) + ti(k_1.z,p.z) +
                  log(Stotal) +
                  s(SiteCode,bs="re"),
                family=gaussian(link = "inverse"),
                data = df_resultados, method = "REML",control=ctrl)
l_md[[9]] <- gam(U_med ~ s(k_1.z) + s(p.z) + ti(k_1.z,p.z) +
                  log(Stotal) +
                  s(SiteCode,bs="re"),
                family=Gamma(link = "log"),
                data = df_resultados, method = "REML",control=ctrl)
l_md[[10]] <- gam(U_med ~ s(k_1.z) + s(p.z) + ti(k_1.z,p.z) +
                  log(Stotal) +
                  s(SiteCode,bs="re"),
                family=Gamma(link = "inverse"),
                data = df_resultados, method = "REML",control=ctrl)
...
Possibilidades de configuração (Pederson et al. 2017): a) número de funções bases e/ou local dos knots; b) os tipos de smoothers para utilizar (cubic ou thin plate regression spline); c) determinar em qual derivada a penalização sera feita; d) modificar a "estrutura" aleatória (figura 4 Perdesen et al. 2017). O maquinário de ajuste de GAM no R já estima o número ótimo de funções base. Então esta é a última opção a ser explorada.

```{r md_U comparacao entre smoothers,echo=TRUE}
l_md <- vector("list",4)
names(l_md) <- paste0("model",1:4)
...
A variável d é sempre a mais plausível e o único modelo dentro do intervalo de plausibilidade é 'd s+ti'. O diagnostico deste modelo foi feito acima. Vou explorar os outros 2 modelos de d e então se necessário avaliar o de 1-k s+ti.

__d t2, dAICc = 1073.0__  

```{r auditoria dos modelos segundo o rank md_U segunda tentativa 1}
par(mfrow=c(2,2))
gam.check(l_md[["d t2"]])
par(mfrow=c(1,2))
plot(l_md[["d t2"]],scheme=2)
```
  
__d te, dAICc = 1079.0__  
  
```{r auditoria dos modelos segundo o rank md_U segunda tentativa 2}
par(mfrow=c(2,2))
gam.check(l_md[["d te"]])
par(mfrow=c(1,2))
plot(l_md[["d te"]],scheme=2)
```
  
  
__1-k s+ti, dAICc = 1285.2__  
  
```{r auditoria dos modelos segundo o rank md_U segunda tentativa 3}
par(mfrow=c(2,2))
...
```{r comparação md_U para estruturas as 3 estruturas aleatórias propostas por Pedersen et al. 2017}
l_md <- vector("list",3)
...
```{r descrição preliminar de U_med em funcao de k}
l_p <- vector("list",3)
l_p[[1]] <- ggplot(df_resultados,aes(x=k,y=U_med)) +
  geom_point() +
  geom_line(aes(group=SiteCode)) +
