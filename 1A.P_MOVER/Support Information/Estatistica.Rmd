...
df_md <- df_resultados %>% filter(MN=="EE") %>% 
  distinct()
df_md.class <- data.frame(md_class=c("glmm 1|Site",
...
f_md <- function(dados){
  ...
  if(md_class == "glmm d+d^2|Site"){
    ...
    md_ <- gam(U_med ~ log_Sobs.z +
                 s(p.z,bs="cr",k=10) + s(d.z,bs="tp",k=10) + ti(p.z,d.z,bs=c("tp","tp"),k=c(10,10)) +
                 s(d.z,SiteCode,bs="fs",xt=list(bs="tp"), m=1,k=10),
                data = df_md, family = Gamma(link="log"),method="REML")
  }else{
    md_ <- gam(U_med ~ log_Sobs.z +
...
l_md.U_mdCheio <- dlply(df_md.class,"md_class",f_md)
```
...
df_plot <- cbind(df,df_pred) %>% 
  mutate(fit = exp(median),
         ...
         p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
         d = d.z * sd(df_resultados$d) + mean(df_resultados$d))
write.csv(df_plot,file = "~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/df_plotPredU.csv",row.names = FALSE)
```
