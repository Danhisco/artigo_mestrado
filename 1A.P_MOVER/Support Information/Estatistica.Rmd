...
```{r AICctab gamm cheio sem outliers}
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md_diff_S_EI__GAMM_sOut.Rdata")
AICctab(l_md.diff_S.EI__GAMM_sOut,weights=TRUE)
```

```{r appraise gamm diffS_EI sOut}
...
```{r graw gamm cheio diffS_EI sOut}
gratia::draw(l_md.diff_S.EI__GAMM_sOut[["d|Site"]])
```

__Figura 2.11__ Efeitos estimados para o gamm cheio diffS_EI sOut

Não vejo melhora quanto à variação no efeito de p.z.  


```{r fig 212 appraise gamm cheio 1Site}
gratia::appraise(l_md.diff_S.EI__GAMM_sOut[["1|Site"]])
```

__Figura 2.12__ graf diag gamm cheio 1|Site sOut


```{r fig 213 draw gamm cheio diffS_EI 1Site}
gratia::draw(l_md.diff_S.EI__GAMM_sOut[["1|Site"]])
```

__Figura 2.13__ Efeitos estimados do gamm cheio 
