...
```{r U padroes gerais,eval=FALSE,echo=FALSE,fig.height=8,fig.width=8}
...
```{r fig 42,eval=FALSE,echo=FALSE,fig.width=8, fig.height=27}
df_plot <- df_U %>%
...
```{r}
predict_y <- exp(fixef(glmer_U)[1] + fixef(glmer_U)[2] * df_md$log_Sobs)
plot(predict_y ~ df_md$log_Sobs)
```



```{r}
p_plot <- simulateResiduals(glmer_U)
plot(p_plot)
```
...
```{r}
df_pred <- cbind(glmer_U@frame,
                 predictInterval(glmer_U,type = "linear.prediction",level = 0.95))
df_pred %>% mutate(fit_response = exp(fit)) %>% 
  ggplot(aes(x=U_med,y=fit_response)) +
  geom_abline(intercept = 0,slope = 1,color="red") +
  geom_smooth(method = "loess",se = FALSE,color="blue",aes(group=SiteCode),alpha=0.3,size=0.2) +
  geom_smooth(method = "lm",se=TRUE,color="green",formula = y~0+x) +
  coord_cartesian(expand = TRUE) + labs(x="observado",y="predito") +
  geom_point(alpha=0.3)
```

O modelo não faz um bom ajuste do observado, para descrever a incerteza do ajuste vou utilizar o intervalo de confiança de 95% bootstrap considerando toda a variação explicada pelo modelo e a variação explicada apenas pela estrutura fixa

```{r}
MuMIn::r.squaredGLMM(glmer_U)
```
...
save(l_md.U_gamEstudo,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_gamEstudo.Rdata")
```

```{r load l_md.U_gamEstudo,eval=TRUE,echo=FALSE}
load("~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_gamEstudo.Rdata")
```
...
```{r}
save(l_md.U_mdCheio,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_mdCheio_semSobs.Rdata")
```
...
## melhor modelo
AICctab(md_U_semSobs,md_U_comSobs,weights=TRUE)
```
...
_____com termo parametrico log(Sobs)_____
...
____sem termo parametrico log(Sobs)__

```{r}
df_md <- df_resultados %>% filter(MN=="EE") %>% distinct()
md_U_sSobs <- gam(U_med ~ 
                    s(p.z,bs="tp",k=10) + s(d.z,bs="tp",k=10) + ti(p.z,d.z,bs=c("tp","tp"),k=c(10,10)) +
                    s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="tp",m=1,k=10),
                  data = df_md, family = Gamma(link="log"),method="REML")  
...
__Comparação gamm com e sem Sobs__

```{r}
glmer_U_cSobs <- l_md.U_mdCheio[["gamm d|Site for each"]]
glmer_U_sSobs <- load(file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U_sSobs.Rdata")  
glmer_U_cSobs %>% summary
```
...
save(md_U.update1,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U.update1.Rdata")
```
...
md_U.tp <- gam(U_med ~ 
             ...
             s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="tp",m=1,k=10),
             data = df_md, family = Gamma(link="log"),method = "REML")
save(md_U.tp,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U.tp.Rdata")
```
...
md_U.REML <- gam(U_med ~ 
             ...
             s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="cr",m=1,k=10),
             data = df_md, family = Gamma(link="log"),method = "REML")
save(md_U.REML,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U.REML.Rdata")
```
...
```{r}
df_md <- df_resultados %>% filter(MN=="EE") %>% distinct()
glmm_U <- glmer(U_med ~ log_Sobs + (1|SiteCode), 
                data = df_md, family =  Gamma(link="log"))
```
...
# gamm <- md_U.tp
# gamm s Sobs
load("~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U_sSobs.Rdata")
gamm <- md_U_sSobs
