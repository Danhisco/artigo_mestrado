...
```{r graf diag 2 glmm p^2 n_nRef fig 13, include=FALSE}
...
df_pred <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                       ...
                       d.z=seq(min(df_resultados$d_Lplot.z),max(df_resultados$d_Lplot.z),length=20),
...
ggplot(df_pred,aes(p,d,fill=fit)) + 
  geom_tile() +
  scale_fill_gradient2(limits = c(0,1),midpoint=0, low="red", high="blue",mid="green") +
...
```{r model dredge EE,message=FALSE,include=FALSE}
...
df_pred$mean <- apply(b3$t,2,mean)
df_pred$IC.low <- apply(b3$t,2,quantile, 0.025)
df_pred$IC.upp <- apply(b3$t,2,quantile, 0.975)
