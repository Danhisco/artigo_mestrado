...
```{r glmm n_nRef comparacao modelos cheios,include=FALSE,echo=FALSE}
# l_md <- vector("list",11)
...
__Figura 1.4__ Probabilidade predito pelo modelo mais plausível  

## 1.2 GLMM binomail subset:MN==EE

```{r glmm quadratico n_nRef MNEE modelo cheio,include=FALSE}
# l_md <- vector("list",3)
...
```{r modelo global MNEE, include=TRUE,echo=TRUE,cache=TRUE}
# dados
...
```{r model averaging EE MuMIn e AICcmodavg,message=FALSE,include=FALSE}
# library(AICcmodavg)
...
l_p.nRefEE_AICcmodavg[[1]] <- ggplot(df_plot,aes(x=p,y=d,fill=mod.avg.pred)) +
  geom_tile() +
  scale_fill_gradient2(limits = c(0,1),midpoint=0.5, low="red", high="blue",mid="black") +
  labs(title="Prob de não Refutar",
