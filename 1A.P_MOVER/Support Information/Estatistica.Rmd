...
f_CI_PosteriorDistribution <- function(gamm,df=df_pred0,N=1e5,output="quantil95"){
  Vb_ <- vcov(gamm)
  ...
  return(crit_)
   ...
   sims <- f_rmvn(N,mu=coef(gamm),sig=Vb_)
   fits <- Cg_ %*% t(sims)
   PI <- t(apply(fits,1,quantile,prob=c(.025,0.975)))
   pred <- pred_$fit
  return(PI)
  }
}
...
# Intevalo onde 95% das funções da distribuição a posteori 
## segundo Simon Wood 2011
...
Xp <- predict(gamm,newdata=df,type="lpmatrix")
lp <- Xp%*%br
fv <- arm::invlogit(lp)

yr <- matrix(rbinom(fv*0,size = 100,prob = ),nrow(fv),ncol(fv))
