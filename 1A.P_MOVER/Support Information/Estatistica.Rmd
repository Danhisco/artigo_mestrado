...
f_md <- function(model_class){
  ...
  if(model_class == "tp"){
    md_ <- gam(U_med ~ log_Sobs.z +
                ...
                data = df_md, Gamma(link = "log"),method = "REML")
...
f_md <- function(dados){
  ...
  if(md_class == "glmm d+d^2|Site"){
    ...
    md_ <- gam(U_med ~ log_Sobs.z +
                ...
                data = df_md, family = Gamma(link="log"),method="REML")
...
__Modelo Cheio U 2__

```{r modelo cheio md_U,echo=TRUE,eval=FALSE}
df_md <- df_resultados %>% filter(MN=="EE") %>% 
  distinct()
df_md.class <- data.frame(md_class=c("d^3|Site",
                                     "d^2|Site",
                                     "d|Site",
                                     "1|Site"))
# função de ajuste
f_md <- function(dados){
  md_class <- unique(dados$md_class)
  if(md_class == "d^3|Site"){
    md_ <- glmer(U_med ~ log_Sobs.z +
                   (p.z + I(p.z^2) + I(p.z^3)) * (d.z + I(d.z^2) + I(d.z^3)) + 
                   (d.z + I(d.z^2) + I(d.z^3)|SiteCode),
                 family = Gamma(link="log"),data=df_md,
                 control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)),
                 na.action = "na.fail")
  }else if(md_class == "d^2|Site"){
    md_ <- glmer(U_med ~ log_Sobs.z +
                   (p.z + I(p.z^2)) * (d.z + I(d.z^2)) + 
                   (d.z + I(d.z^2)|SiteCode),
                 family = Gamma(link="log"),data=df_md,
...
registerDoMC(3)
l_md.U_mdCheio2 <- dlply(df_md.class,"md_class",f_md,.parallel = TRUE)
```


```{r protocolo de avaliacao quando ha problema de convergencia, eval=TRUE}
# all fit do modelo com problema de convergência
md_UallFit <- allFit(l_md.U_mdCheio2[["d^2|Site"]],maxfun = 1e6,parallel = 'multicore', ncpus = 3)
is.OK <- sapply(md_UallFit,is,"merMod")
l_allFit.OK <- md_UallFit[is.OK]
lapply(l_allFit.OK,function(x) x@optinfo$conv$lme4$messages)
# todos apresentam problemas de convergência
AICctab(l_allFit.OK,weights=TRUE)
# não há um modelo mais plausível, mesmo aquele com dAIC>2 apresenta peso de evidência de 0.11
# graficos
lliks <- sort(sapply(l_allFit.OK,logLik))
aa.fixef <- t(sapply(l_allFit.OK,fixef))
aa.fixef.m <- reshape2::melt(aa.fixef)
...
ddply(aa.fixef.m,"Var2",summarise,CV = sd(value)/abs(mean(value))) %>% 
  ggplot(aes(x=Var2,y=CV)) + geom_point() + labs(y=" sd / |mean|")
###
relgrad <- with(l_md.U_mdCheio2[["d^3|Site"]]@optinfo$derivs,solve(Hessian,gradient))
max(abs(relgrad)) < 0.001
```



```{r save l_md.U_mdCheio,eval=FALSE,echo=FALSE}
save(l_md.U_mdCheio2,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_mdCheio2.Rdata")
```

```{r load l_md.U_mdCheio,eval=TRUE,echo=FALSE}
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_mdCheio2.Rdata")
```



```{r}
AICctab(l_md.U_mdCheio2,weights=TRUE)
```

O único modelo plausível com todo peso de evidência é aquele com problema de convergência. Todos os otimizadores que conseguiram estimar os parâmetros apresentam problemas de convergência. Houve dois parâmetros onde os coeficientes estimados apresentam grande variação entre os otimizadores (figura X1), na maior parte dos casos o coficientes de variação é inferior à 1. Escolhi o otimizar 'bobyqa' pois quando o CV é grande este otimizador está de acordo com os demais (ou seja, não se comporta como outlier) REESCREVER PENSAR...

```{r fig.width=8, fig.height=30}
md_U <- l_md.U_mdCheio2[["d^2|Site"]]
df_plot <- cbind(md_U@frame,
                 fit = predict(md_U,type="response"))
ggplot(df_plot,aes(x=d.z,y=U_med)) +
  geom_point(alpha=0.3) +
  geom_line(aes(y=fit),color="red",alpha=0.4) +
...
```{r}
registerDoMC(3)
l_md.dredge_U <- llply(dredge(md_U,trace = FALSE,evaluate=FALSE), eval,.parallel = TRUE)
save(l_md.dredge_U,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.dredge_U.Rdata")
# Model Averaging for SiteCode predictions
mdAvg_U__MuMIn <- model.avg(l_md.dredge_U)
save(mdAvg_U__MuMIn,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_U__MuMIn.Rdata")
# Model Averaging for new data predictions
df_pred <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                       d.z=seq(min(df_resultados$d.z),max(df_resultados$d.z),length=40),
                       p.z=seq(min(df_resultados$p.z),max(df_resultados$p.z),length=200),
                       log_Sobs.z=0)
mdAvg_U__AICcmodavg <- modavgPred(l_md.dredge_U,newdata = df_pred) %>%
  cbind(df_pred)
save(mdAvg_U__AICcmodavg,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_U__AICcmodavg.Rdata")
```


```{r eval=FALSE}
df_plot <- mdAvg_U__AICcmodavg %>% 
  mutate(p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
         d = d.z * sd(df_resultados$d) + mean(df_resultados$d))
#####
# plots Probabilidad de não refutar
...
labels_IC <- c(lower.CL="IC 5%", upper.CL="IC 95%")
## predição média para ambos modelos
l_p[[1]] <- ggplot(df_plot,aes(x=p,y=d,fill=mod.avg.pred)) +
  coord_cartesian(expand = FALSE) + 
  geom_tile() +
  scale_fill_distiller(palette = "Spectral") + #limits = c(0,1),
  labs(title="U (new singleton/birth event)") +
  theme_classic() + 
  theme(legend.position="top",legend.title = element_blank()) +
  geom_contour(aes(z=mod.avg.pred),alpha=0.4, col="black",
               breaks = quantile(df_plot$mod.avg.pred,probs = seq(0.1,0.9,by=0.1))) +
  geom_text_contour(aes(z=mod.avg.pred),stroke=0.2,
                    breaks = round(quantile(df_plot$mod.avg.pred,probs = seq(0.1,0.9,by=0.1)),digits = 5),
                    rotate = FALSE,
                    size=3)
## IC para MNEE
df_plotEE <- df_plot %>% filter(MN=="EE") %>%
...
```{r fig 224 nRefEE observado e predito modelo medio, comment=FALSE, message=FALSE,warning=FALSE,results="hide",fig.width=8, fig.height=30}
# Modelo Global
load(file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.dredge_U.Rdata")
# Predição
df_md <- df_resultados %>% filter(MN=="EE")
df_md$fit <- predict(mdAvg_U__MuMIn,type="response")
# Gráfico
df_md %<>% mutate(title = paste0(SiteCode," p=",round(p,3))) %>% arrange(p)
df_md$title <- factor(df_md$title,levels = unique(df_md$title))
ggplot(df_md,aes(x=d,y=U_med)) +
  geom_point() + geom_line(aes(y=fit),color="red") +
  labs(x="Dispersão Média",
       y="U",
       title="U") +
...
```{r}
load(file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_mdCheio.Rdata")
...
```{r U_med predito e observado para o modelo cheio,fig.width=8, fig.height=30}
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_U.update1.Rdata")
# dados
df_ <- broom::augment(md_U)
# l_pred <- predict.gam(md_U,link=response,se.fit = TRUE)
# df_$fit_response <- l_pred$fit
# df_$se.fit_response <- l_pred$se.fit
df_ %<>% mutate(fit_response = exp(.fitted),
                se.fit_response = exp(.se.fit),
                p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
                d = d.z * sd(df_resultados$d) + mean(df_resultados$d))
# df_ %>% summary
# grafico
df_ %<>% mutate(title = paste0(SiteCode," p=",round(p,3))) %>% arrange(p)
df_$title <- factor(df_$title,levels = unique(df_$title))
ggplot(df_,aes(x=d,y=U_med)) +
  geom_point(alpha=0.3) +
  geom_line(aes(y=fit_response),color="red",alpha=0.6) +
  # geom_ribbon(aes(ymin=fit_response - 1.96 * se.fit_response,
  #                 ymax=fit_response + 1.96 * se.fit_response),
  #             color="blue",alpha=0.4) +
  labs(title="U gam(bs=cr,k=10)") +
  facet_wrap(~title,ncol=5,scales = "free")
```

O modelo erra para alguns caso, vou comparar com um que utiliza 'tp'

```{r }
df_md <- df_resultados %>% filter(MN=="EE") %>% distinct()
md_U.tp <- gam(U_med ~ 
             s(p.z,bs="tp",k=10) + s(d.z,bs="tp",k=10) + ti(p.z,d.z,bs=c("tp","tp"),k=c(10,10)) +
             s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="tp",m=1,k=10),
             data = df_md, family = Gamma(link="log"),method = "REML")
save(md_U.tp,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_U.tp.Rdata")
```
...
ggplot(df_,aes(x=d,y=U_med)) +
  ...
  #                 ymax=fit_response + 1.96 * se.fit_response),
  #             color="blue",alpha=0.4) +
  labs(title="U; gam(bs=tp)") +
  facet_wrap(~title,ncol=5,scales = "free")
```



commit para codigos de comparação 

e887e47e8beea566d95d930f85003cd8bbb184f3
...
md_U.REML <- gam(U_med ~ 
             ...
             s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="cr",m=1,k=10),
             data = df_md, family = Gamma(link="log"),method = "REML")
save(md_U.REML,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_U.REML.Rdata")
```
...
df_pred <- t(apply(predict_link,1,function(X) quantile(X,probs = c(0.10,0.50,0.90)))) %>% 
  as.data.frame()
...
##############################################################
##############################################################
# plot(lm(df_plot$median ~ predict(md_U.tp,newdata=df)))
##############################################################
##############################################################
##############################################################
## graficos
...
## predição média para ambos modelos
df_plot$variable <- "U"
l_p[[1]] <- ggplot(df_plot,aes(x=p,y=d,fill=fit)) +
  coord_cartesian(expand = FALSE) + 
  geom_tile() +
  scale_fill_distiller(palette = "Spectral") +
...
df_plot2 <- df_plot %>%
  gather(key = IC_class ,value = IC_pred,lower:upper)
labels_IC <- c(lower="IC 5%", upper="IC 95%")
l_p[[2]] <- ggplot(df_plot2,aes(x=p,y=d,fill=IC_pred)) +
  coord_cartesian(expand = FALSE) + 
  geom_tile() +
  scale_fill_distiller(palette = "Spectral") +
  theme_classic() + 
...
beta <- coef(gamm) # coeficientes
Vb <- vcov(gamm) # matrix de variância-covariância
Xp <- predict(gamm,df,type="lpmatrix") # matrix para predito para o novo conjunto de dados
pred <- predict.gam(gamm,df,se.fit = TRUE) # esperado
## fator de correção
