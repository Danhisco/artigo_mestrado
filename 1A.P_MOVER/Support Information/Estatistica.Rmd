...
```{r gam binomial n_nRef selecao de variaveis}
l_md <- vector("list",6)
...
# k
l_md[[1]] <- gam(cbind(n_nRef,100-n_nRef) ~ k + s(p.z,MN,by=k,bs="fs") +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md[[2]] <- gam(cbind(n_nRef,100-n_nRef) ~ k + s(p.z,MN,by=k,bs="fs") +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
# k_1
l_md[[3]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,MN,bs="fs") + s(k_1.z,MN,bs="fs") + ti(p.z,k_1.z,by=MN,bs=c("tp","tp")) +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md[[4]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,MN,bs="fs") + s(k_1.z,MN,bs="fs") + ti(p.z,k_1.z,by=MN,bs=c("tp","tp")) +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
# d/L
l_md[[5]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,MN,bs="fs") + s(d_Lplot.z,MN,bs="fs") + ti(p.z,d_Lplot.z,by=MN,bs=c("tp","tp")) +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
l_md[[6]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,MN,bs="fs") + s(d_Lplot.z,MN,bs="fs") + ti(p.z,d_Lplot.z,by=MN,bs=c("tp","tp")) +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
...
#### Summary dos mais plausíveis

```{r sumario md_nRef mais plausivel}
summary(l_md[["k_1 MN|SiteCode"]])
# summary(l_md[["k MN|SiteCode"]])
# plot(l_md[["k MN|SiteCode"]])
```
...
df_pred.k_1 %<>% inner_join(x=.,
                            y=distinct(select(df_resultados,k,k_1.z)),
                            by="k_1.z")
# gráficos
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_resultados,k %in% levels(df_resultados$k)[1:10]),aes(x=p.z,y=n_nRef)) + 
  ...
  geom_ribbon(aes(ymin=exp(fit - 2*se.fit), ymax=exp(fit + 2*se.fit), x=p.z),
              ...
              inherit.aes=FALSE,
              color="red") +
  geom_line(aes(y=fit), data=filter(df_pred.k,k %in% levels(df_resultados$k)[1:10])) +
  # k_1
...
l_p[[2]] <- ggplot(filter(df_resultados,k %in% levels(df_resultados$k)[11:20]),aes(x=p.z,y=n_nRef)) + 
  geom_point() + facet_grid(k~MN) +
  # k
  geom_ribbon(aes(ymin=exp(fit - 2*se.fit), ymax=exp(fit + 2*se.fit), x=p.z),
              ...
              inherit.aes=FALSE,
              color="red") +
  geom_line(aes(y=fit), data=filter(df_pred.k,k %in% levels(df_resultados$k)[11:20])) +
  # k_1
  geom_ribbon(aes(ymin=exp(fit - 2*se.fit), ymax=exp(fit + 2*se.fit), x=p.z),
...
```{r draw dos dois mais plausiveis}
# load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/md_nRef.Rdata")
gratia::draw(l_md[["k_1 MN|SiteCode"]])
gratia::draw(l_md[["k MN|SiteCode"]])
plot(l_md[["k MN|SiteCode"]])
plot(l_md[["k_1 MN|SiteCode"]])
gam.check(l_md[["k_1 MN|SiteCode"]])
summary(l_md[[2]])
p_plot <- DHARMa::simulateResiduals(l_md[["k_1 MN|SiteCode"]],n=1000)
plot(p_plot)
