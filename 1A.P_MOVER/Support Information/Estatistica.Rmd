...
```{r graf diag 2 glmm p^2 n_nRef fig 13}
...
```{r avaliacao por site fig 14,fig.width=8, fig.height=30}
# Dados
...
ggplot(df_plot,aes(x=d_Lplot.z,y=n_nRef,group=MN)) +
...
```{r modelo global, include=TRUE,echo=TRUE}
# dados
...
```{r model averaging EE}
# model averagin de todas as combinações de modelo
mdAvg_nRefEE <- model.avg(lapply(dredge(global_md,trace = FALSE,evaluate=FALSE), eval),subset<2) #,
# armazenamento
save(mdAvg_nRefEE,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_nRefEE.Rdata")
```
...
# Modelo Global
load(file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_nRefEE.Rdata")
# Predição
...
```{r fig 119 res quant glmm cheio MNEI}
...
```{r modelo global e model averaging EI, include=TRUE,echo=TRUE}
# dados
...
global_md <- glmer(cbind(n_nRef,100-n_nRef) ~
                   ...
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)),na.action = "na.fail")
# model averagin de todas as combinações de modelo
mdAvg_nRefEI <- model.avg(lapply(dredge(global_md,trace = FALSE,evaluate=FALSE), eval),subset<7)
# armazenamento
save(mdAvg_nRefEI,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_nRefEI.Rdata")
```
...
# Model Averaging 
load(file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/mdAvg_nRefEI.Rdata")
# Predição
df_md$Pr_nRef <- predict(mdAvg_nRefEE,type="response")*100
# Gráfico
df_md %<>% mutate(title = paste0(SiteCode," p=",round(p,3))) %>% arrange(p)
