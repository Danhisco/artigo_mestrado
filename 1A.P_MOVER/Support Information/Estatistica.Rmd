...
Na ideia de se obter o modelo estatístico suficiente, irei inicar com a primeira opção: um smoother por sítio de amostragem com uma mesma penalização  
...
```{r}
l_md_U_GI <- list()
l_md_U_GI[[1]] <- gam(log10_U ~ offset(log10_Sobs) +
                 s(log10_d.z,bs="tp",k=10) + s(p.z,bs="tp",k=10) + ti(log10_d.z,p.z,bs=c("tp","tp"),k=c(10,10)) +
                 s(SiteCode,bs="re") + s(log10_d.z,by=SiteCode,bs="tp",m=1,k=10),
                data = df_U, family=gaussian,method = "REML")
l_md_U_GI[[2]] <- gam(log10_U ~ offset(log10_Sobs) +
                 s(log10_d.z,bs="cr",k=10) + s(p.z,bs="cr",k=10) + ti(log10_d.z,p.z,bs=c("cr","cr"),k=c(10,10)) +
                 s(SiteCode,bs="re") + s(log10_d.z,by=SiteCode,bs="cr",m=1,k=10),
                data = df_U, family=gaussian,method = "REML")
AICctab(l_md_U_GI,weights=TRUE)
```
