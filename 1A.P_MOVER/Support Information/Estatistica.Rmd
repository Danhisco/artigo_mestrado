...
df_pred.k <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
  ...
  mutate(p = p.z * sd(df_resultados$p) + mean(df_resultados$p))
df_pred.k <- cbind(df_pred.k,
                   predict.gam(l_md.Distr[["normal"]],
                           df_pred.k,type="response",se.fit=TRUE))
df_pred.k %<>% mutate(response_nRef.fit = fit - min(df_resultados$logit_nRef)*(-1) - 0.01,
                      response_nRef.sd = se.fit - min(df_resultados$logit_nRef)*(-1) - 0.01)
...
__Figura 1.10__ Predito pelo modelo com distribuição normal

Apesar de possibilitar uma boa estimativa da média o modelos é muito 
...
__Figura 1.11__ Gráficos Diagnósticos do modelo mais plausível para o conjunto de dados MN:EE    
...
__Figura 1.12__ Predito para o conjunto de dados MNEE  
...
__Figura 1.13__ Graficos Diagnosticos para MNEE e bs=tp  
...
__Figura 1.14__ Predito para MNEE e bs="tp"
...
__Figura 1.15__ Gráficos Diagnósticos do modelo mais plausível para o conjunto de dados MN:EI    
  
  
```{r predito para conjunto de dados MNEI}
## modelo mais plausível
df_pred.k <- expand.grid(SiteCode=levels(df_resultados$SiteCode)[1],
                          k=levels(df_resultados$k),
                          p.z=seq(min(df_resultados$p.z),max(df_resultados$p.z),length=103)) %>% 
...
__Figura 1.16__ Predito para o conjunto de dados MNEI  
...
O smoother type "cr", modelo utilizado até o momento, é o mais plausívels
