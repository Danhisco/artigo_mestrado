...
df_pred.k <- cbind(df_pred.k,
                   predict.gam(l_md[["k MN|SiteCode"]],
                           df_pred.k,type="link",se.fit=TRUE))
...
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=logit_nRef)) + 
...
l_p[[2]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[11:20]),aes(x=p,y=logit_nRef)) + 
  geom_point() + facet_grid(k~MN) +
  # k
  geom_ribbon(aes(ymin=I(fit - 2*se.fit), ymax=I(fit + 2*se.fit), x=p),
...
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=devRes)) + 
  geom_point() + geom_smooth() + facet_grid(k~MN,scales="free") +
  ylab("Deviance Residuals") + xlab("Proportion of Tree Cover")
l_p[[2]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[11:20]),aes(x=p,y=devRes)) + 
  geom_point() + geom_smooth() + facet_grid(k~MN,scales="free") +
  ylab("Deviance Residuals") + xlab("Proportion of Tree Cover")
do.call("grid.arrange",c(l_p,ncol=2))
```

__Figura 1.4__ residuals.gam(l_md[["k MN|SiteCode"]]) ~ p (~k*MN).

Os resíduos do tipo 'deviance' não indicam erro na estimativa quanto à tendência global.  
  
```{r fig 1.5 residuos caseiros pelas preditoras}
## dados
df_plot <- augment(l_md[["k MN|SiteCode"]]) %>% as.data.frame()
df_plot <- cbind(df_plot,
                 predict(l_md[["k MN|SiteCode"]],type="response",se=TRUE))
df_plot %<>% mutate(p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
                    Prob_nRef = df_plot[,1][,1]/100,
                    logit_nRef = arm::logit(Prob_nRef),
                    link_res = logit_nRef - .fitted,
                    response_res = Prob_nRef - fit)
df_plot$n_nRef <- df_plot[,1][,1]
## graficos 
l_p <- vector("list",2)
l_p[[1]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=response_res)) + 
  geom_point() + geom_smooth(method="loess") + facet_grid(k~MN) +
  ylab("Prob_nRef - fit") + xlab("Proportion of Tree Cover")
l_p[[2]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[11:20]),aes(x=p,y=response_res)) + 
  geom_point() + geom_smooth(method="loess") + facet_grid(k~MN) +
  ylab("Prob_nRef - fit") + xlab("Proportion of Tree Cover")
do.call("grid.arrange",c(l_p,ncol=2))
#
...
l_p[[1]] <- ggplot(filter(df_plot,k %in% levels(df_resultados$k)[1:10]),aes(x=p,y=n_nRef)) + 
  geom_point() + facet_grid(k~MN) +
  ylab("logit_obs") + xlab("Proportion of Tree Cover")
...
Ainda não estou convencido de que o padrão esta fazendo um bom ajuste - não vejo o padrão estimado nos resíduos CONTINUAR

### Melhoria do modelo mais plausível: CONSTRUÇÃO

__Opções__

#### 1) modificar método de estimativa de smoother parameter: 4 opções

```{r method smoothing parameter n_nRef}
...
#### 2) modificar o tipo de smoother: 

```{r}
l_md.BS <- vector("list",length = )
names(l_md.BS) <- c()
l_md.BS[[1]] <- gam(cbind(n_nRef,100-n_nRef) ~ k * MN + s(p.z,k,by=MN,bs="fs",) +
                   s(SiteCode,by=MN,bs="re"),
                   family = "binomial",
                   data = df_resultados, method = "REML")
AICctab(l_md.BS,weights="TRUE")

# ggplot(df_resultados,aes(x=MN,y=n_nRef)) + geom_jitter() + geom_boxplot() + facet_wrap(~k,ncol=4)
```

__Figura 1.4__ Resíduos do modelo mais plausível como na figura 1.3
...
l_md <- vector("list",3)
names(l_md) <- c("k 1|SiteCode","k_1 1|SiteCode","d/L 1|SiteCode")
# muitos parametros para poucos dados "k_1 k_1 * MN|SiteCode","d_Lplot d_Lplot * MN|SiteCode")
# k
l_md[[1]] <- gam(cbind(n_nRef,100-n_nRef) ~ k + s(p.z,by=k,bs="cr") +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = filter(df_resultados,MN=="EI"), method = "REML")
# k_1
l_md[[2]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,bs="cr") + s(k_1.z,bs="cr") + ti(p.z,k_1.z,bs=c("cr","cr")) +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = filter(df_resultados,MN=="EI"), method = "REML")
# d/L
l_md[[3]] <- gam(cbind(n_nRef,100-n_nRef) ~ s(p.z,bs="cr") + s(d_Lplot.z,bs="cr") + ti(p.z,d_Lplot.z,bs=c("cr","cr")) +
                   s(SiteCode,bs="re"),
                   family = "binomial",
                   data = filter(df_resultados,MN=="EI"), method = "REML")
AICctab(l_md,weights=TRUE)
...
ggplot(filter(df_resultados,MN=="EI"),aes(x=p,y=n_nRef)) + 
  ...
  theme(panel.background = element_rect(fill = "gray"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
df_resultados %>% filter(MN=="EI") %>% ddply("SiteCode",summarise,mean_nRef = mean(n_nRef))

#################################################################################

teste <- update(l_md[[1]],data=filter(df_resultados,MN=="EI" & !(SiteCode %in% c("MGlavr3","SCside4"))))
gratia::draw(teste)
