...
load(file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md.U_mdCheio.Rdata")
gamm <- l_md.U_mdCheio[["gamm d|Site for each"]]
# dados
df_ <- broom::augment(gamm)
beta <- coef(gamm)
V <- vcov(gamm)
...
df_plot <- cbind(df_,df_pred) %>% 
  mutate(fit = exp(median),
         ...
         p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
         d = d.z * sd(df_resultados$d) + mean(df_resultados$d))
# plot(fit ~ fit_predict,data=df_plot)
# grafico
df_plot %<>% mutate(title = paste0(SiteCode," p=",round(p,3))) %>% arrange(p)
df_plot$title <- factor(df_plot$title,levels = unique(df_plot$title))
ggplot(df_plot,aes(x=d,y=U_med)) +
...
```{r U predito, fig.height=9,fig.width=6}
...
df_plot$IC_pred <- "IC 5%"
l_p[[2]] <- ggplot(df_plot,aes(x=p,y=d,fill=lower)) +
  coord_cartesian(expand = FALSE) + 
  geom_tile() +
  scale_fill_distiller(palette = "Spectral") +
  theme_classic() + 
