...
__Figura 3.2__ |diffS| ~ n_nRef * k, subset=MNEI
...
# Figuras
f_d_ply <- function(X){
  # X <- df_SAD_e_paisagem %>% filter(SiteCode == levels(df_resultados$SiteCode)[1])
  X %<>% mutate(string_main=paste0("MN=",MN,"; k=",k,"; n_nRef=",n_nRef,"; |diffS|=",round(modulo_diffS,3)))
  X %>% ddply(c("MN","k"),summarise,)
  # df_ <- X %>% filter(MN=="EI" & k==0.05)
  v_SADobs <- read.csv(X[1,"SAD_obs.name"],header = TRUE)[,"N"]
  f_plot <- function(df_,v_obs){
   plot(rad(v_SADobs),main=head(df_$string_main,1),
        xlim=c(1,head(df_$max_rank,1)))
    for(j in 1:nrow(df_)){
      v_SAD_MN <- read.table(df_[j,"SAD_MN.name"],header = TRUE,as.is = TRUE)$SAD_predita
      lines(rad(v_SAD_MN),type="l",col=rgb(0.25, 0.25, 0.25, 0.25))
    }
    points(rad(v_SADobs),pch = 19, col="red",cex=1,bg="black",
           main=head(df_$string_main,1))
  }
  par(mfrow=c(8,5))
  d_ply(X,c("MN","k"),function(A) f_plot(df_=A,v_obs=v_SADobs))
}
...
for(i in 1:length(v_Site)){
  df_plot <- filter(df_SAD_e_paisagem,SiteCode == v_Site[i])
  df_plot %>% filter(MN=="EE" & k==0.05) %>% head
}
```
