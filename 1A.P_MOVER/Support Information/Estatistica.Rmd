...
#                   s(SiteCode,bs="re"),
#                 family=gaussian(link = "inverse"),
#                 data = df_resultados, method = "REML",control=ctrl) # "d log gamma"
...
O Modelo mais plausível diverge do pressuposto de que a variável resposta pode ser aproximada por uma distribuição Gamma(link=log).  

```{r diag 1 modelo cheio GAMM(U),eval=FALSE,echo=FALSE,include=FALSE}

gam.check(md_U)
md_U.upgrade <- gam(U_med ~ s(d_L.plot.z,k=20) + s(p.z) + ti(d_L.plot.z,p.z,k=20) +
                  log(Stotal) +
                  s(SiteCode,bs="re"),
                family=Gamma(link = "log"),
                data = df_resultados, method = "REML",control=ctrl)
gam.check(md_U.upgrade)
```
