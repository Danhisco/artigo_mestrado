...
# escala da função de ligação
df_resultados %<>% mutate(logit_nRef = car::logit(n_nRef/100))
# plot(x=df_resultados$n_nRef,y=df_resultados$logit_nRef)
l_p <- vector("list",2)
...
# 
# v_Sites <- df_resultados %>% filter(n_nRef == 0 & p>0.5 & p<0.7) %>% .$SiteCode %>% unique
# 
# df_resultados %>% filter(SiteCode %in% v_Sites) %>% ddply("SiteCode",summarise,mean_nRef = mean(n_nRef))
# 
# 
# 
# l_p <- vector("list",2)
# l_p[[1]] <- ggplot(filter(df_resultados,SiteCode %in% v_Sites & k %in% levels(df_resultados$k)[1:10]),
#                    aes(x=p,y=n_nRef)) + 
#   geom_point() + 
#   # geom_smooth(method="loess") +
#   facet_grid(k~MN)
# l_p[[2]] <- ggplot(filter(df_resultados,SiteCode %in% v_Sites & k %in% levels(df_resultados$k)[11:20]),
#                    aes(x=p,y=n_nRef)) + 
#   geom_point() + 
#   # geom_smooth(method="loess") +
#   facet_grid(k~MN) 
...
## GLMM com resposta quadratica  

### Modelo Cheio


```{r glmm n_nRef comparacao modelos cheios,include=FALSE}
# l_md <- vector("list",8)
...
#                    family = "binomial",data=df_resultados,
#                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
# l_md[[6]] <- glmer(cbind(n_nRef,100-n_nRef) ~ p.z * d_Lplot.z * MN + I(p.z^2) * d_Lplot.z * MN + (1|SiteCode),
#                    family = "binomial",data=df_resultados,
#                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
# l_md[[7]] <- glmer(cbind(n_nRef,100-n_nRef) ~ p.z * d_Lplot.z * MN + I(p.z^2) * d_Lplot.z * MN + (MN|SiteCode),
#                    family = "binomial",data=df_resultados,
#                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
# l_md[[8]] <- glmer(cbind(n_nRef,100-n_nRef) ~ p.z * d_Lplot.z * MN + I(p.z^2) * d_Lplot.z * MN + (d_Lplot.z*MN|SiteCode),
#                    family = "binomial",data=df_resultados,
#                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
# l_md_nRef.GLMM <- l_md
...
md_nRef <- l_md_nRef.GLMM[["d/L_plot d/L_plot*MN|Site"]]
...
```{r graf diag 2 glmm p^2 n_nRef}
df_plot <- augment(md_nRef) %>% as.data.frame()
# df_plot %>% names
l_p <- vector("list",3)
...
__Figura 1.3__ Valores ajustados contra as preditoras lineares  
  
  
```{r avaliacao por site}
...
# df_plot %<>% mutate(Site = as.character(SiteCode))
# par(mfrow=c(4,26))
title <- df_resultados %>% mutate(title = paste(SiteCode," p=",round(p,3),sep="")) %>% arrange(.,p) %>% 
  .$title %>% unique
Site <- df_resultados %>% mutate(title = paste(SiteCode," p=",round(p,3),sep="")) %>% arrange(.,p) %>% 
  .$SiteCode %>% as.character() %>% unique
for(i in 1:length(Site)){
  print(ggplot(filter(df_plot,SiteCode == Site[i]),aes(x=d_Lplot.z,y=n_nRef,group=MN)) +
  geom_point(aes(color=MN)) +
  geom_line(aes(y=Pr_nRef,color=MN)) +
  scale_color_manual(values=c("#FF0000", "#000000")) +
  ggtitle(title[i]) + theme_classic())
  # save plots as .png
...
# for(i in 1:length(Site)){
# l_p[[i]]  <- ggplot(filter(df_plot,SiteCode == Site[i]),aes(x=d_Lplot.z,y=n_nRef,group=MN)) + 
#   geom_point(aes(color=MN)) + 
#   geom_line(aes(y=Pr_nRef,color=MN)) +
#   scale_color_manual(values=c("#FF0000", "#000000")) +
#   ggtitle(title[i]) + theme_classic()
# }
# do.call("grid.arrange",c(l_p,ncol=4))
```

__Figura 1.4__ Predito e observado por SiteCode  
  
__Tabela do modelo mais plausível:__  
  
```{r codigo do modelo mais plausivel, include=FALSE,echo=TRUE}
md_nRef <- glmer(cbind(n_nRef,100-n_nRef) ~ p.z * d_Lplot.z * MN + I(p.z^2) * d_Lplot.z * MN + (d_Lplot.z*MN|SiteCode),
                   family = "binomial",data=df_resultados,
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
```


```{r summary modelo mais plausivel glmm n_nref}
tab_model(md_nRef)
```
...
df_pred <- cbind(df_pred,
  predictInterval(merMod = md_nRef, newdata = df_pred,
                         level = 0.95, n.sims=200,
                         stat="median",type = "probability",
                         include.resid.var = TRUE)
                 ) %>% 
  mutate(p = p.z * sd(df_resultados$p) + mean(df_resultados$p),
         d_Lplot = d_Lplot.z*sd(df_resultados$d_Lplot) + mean(df_resultados$d_Lplot))

ggplot(df_pred,aes(p,d_Lplot,fill=fit)) + 
  geom_tile() +
  scale_fill_gradient2(low="red",mid="white",  high="blue", 
                       midpoint=median(df_pred$fit)) +
  labs(title="Prob de não Refutar",
       fill="Probability") + facet_wrap(~MN,ncol=2) + theme_classic()
```
  
__Figura 1.5__ Probabilidade predito pelo modelo mais plausível  

### n_nRef subset:MN==EE

```{r glmm quadratico n_nRef MNEE modelo cheio}
l_md <- vector("list",3)
names(l_md) <- c("1|Site","d|Site","d + d^2|Site")
l_md[[1]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     ( p.z + I(p.z^2) ) * ( d_Lplot.z + I(d_Lplot.z^2) ) +
                     (1|SiteCode),
...
l_md[[3]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     ( p.z + I(p.z^2) ) * ( d_Lplot.z + I(d_Lplot.z^2) ) +
                     (d_Lplot.z + I(d_Lplot.z^2)|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
```{r glmm quadratico n_nRef MNEE selecao de variaveis, include=FALSE,echo=FALSE}
l_md <- vector("list",length = 12)
names(l_md) <- c("full model",
                 "-(p2:d2)",
                 "-(p2:d2)-(d:p2)","-(p2:d2)-(d2:p)",
                 "-(p2:d2)-(d2:p)-(d:p2)",
                 "no interactions",
                 "p + d + p2","p + d + d2",
                 "p + d","p","d","1")
l_md[[1]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     p.z + d_Lplot.z + I(p.z^2) + I(d_Lplot.z^2) +
                     p.z:d_Lplot.z + 
                     p.z:I(d_Lplot.z^2) + d_Lplot.z:I(p.z^2) +
                     I(p.z^2):I(d_Lplot.z^2) +
                     (d_Lplot.z + I(d_Lplot.z^2)|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md[[2]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
...
l_md[[7]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     p.z + d_Lplot.z + I(p.z^2) +
                     (d_Lplot.z|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md[[8]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     p.z + d_Lplot.z + I(d_Lplot.z^2) +
                     (d_Lplot.z + I(d_Lplot.z^2)|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md[[9]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     p.z + d_Lplot.z +
                     (d_Lplot.z|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md[[10]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     p.z +
                     (1|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md[[11]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     d_Lplot.z +
                     (d_Lplot.z|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md[[12]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     1 +
                     (1|SiteCode),
                   family = "binomial",data=filter(df_resultados,MN=="EE"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md.nRef.GLMM_EEvars <- l_md
AICctab(l_md.nRef.GLMM_EEvars,weights=TRUE) 
```




```{r glmm quadratico n_nRef MNEE selecao de variaveis VERSAO MUMIN}
# Modelo Global
...
# Todas as combinações de Modelos
mdAvg_nRefEE <- model.avg(lapply(dredge(global_md,trace = 2,evaluate=FALSE), eval),subset<7)
# Predição
df_md$Pr_nRef <- predict(mdAvg_nRefEE,type="response")*100
# Gráfico
df_md %<>% mutate(title = paste0(SiteCode," p=",round(p,3))) %>% arrange(p)
df_md$title <- factor(df_md$title,levels = unique(df_md$title))
ggplot(filter(df_md,SiteCode %in% levels(df_md$SiteCode)[1:8]),aes(x=d_Lplot,y=n_nRef)) + 
  geom_point() + geom_line(aes(y=Pr_nRef)) + 
  facet_wrap(~title,ncol=5) + theme_classic()
```

__Figura 1.7__ Predições por sítio de amostragem; pontos: número de SADs neutras refutadas para uma bateria de simulações com um determinada distância média de dispersão; eixo x = distância média de dispersão / largura da área de amostragem; a linha é a probabilidade de não refutar uma SAD neutra segundo a predição média do conjunto de sub-modelos dentro do intervalo de plausibilidade de 7 (Burnham et al 2011)
...
```{r glmm quadratico n_nRef MNEI modelo cheio}
l_md <- vector("list",3)
names(l_md) <- c("1|Site","d|Site","d + d^2|Site")
l_md[[1]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                     p.z + d_Lplot.z + I(p.z^2) + I(d_Lplot.z^2) +
...
# save(l_md_nRef.GLMM_EE,file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md_nRef_GLMM_EE.Rdata")
```  
  O modelo mais plausível considera um termo linear e quadrático para d na estrutura aletória. Segue seleção de variáveis.

```{r glmm quadratico n_nRef MNEI selecao de variaveis}
l_md <- vector("list",length = 12)
names(l_md) <- c("full model",
                 "-(p2:d2)",
                 "-(p2:d2)-(d:p2)","-(p2:d2)-(d2:p)",
...
l_md[[12]] <- glmer(cbind(n_nRef,100-n_nRef) ~ 
                   ...
                   family = "binomial",data=filter(df_resultados,MN=="EI"),
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
l_md.nRef.GLMM_EIvars <- l_md
AICctab(l_md.nRef.GLMM_EIvars,weights=TRUE) 
```
