...
<<<<<<< HEAD
```{r comparacao testes KS, echo=TRUE,eval=FALSE}
...
for(row_label in 1:nrow(df_testeKS)){
  ...
  f_KSeS <- function(v_OBS = v_SAD.obs, path_SAD.MN){
    ...
    a <- data.frame(D_KSgeneral = a$statistic, p.valor_KSgeneral = a$p.value)
  ...
  df_testeKS$resultados[[row_label]] <- rbind.fill(l_)
}
...
```{r comparacao ks teste base e KSgeneral,eval=TRUE,cahce=TRUE}
df_replicas <- read.csv("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/resultados/df_replicas__KSgeneral.csv",
                        header = TRUE,as.is = TRUE)
l_p <- list()
l_p[[1]] <- ggplot(df_replicas,aes(y=D_KSgeneral,x=KS.D)) + geom_point(alpha=0.3)
l_p[[2]] <- ggplot(df_replicas,aes(y=p.valor_KSgeneral,x=KS.p)) + geom_point(alpha=0.3)
do.call("grid.arrange",c(l_p,ncol=2))
```
...
df_replicas <- read.csv("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/resultados/df_replicas__KSgeneral.csv",
                        header = TRUE,as.is = TRUE)
registerDoMC(3)
df_nRef.KSgeneral <- df_replicas %>% ddply(c("SiteCode","k","MN"),summarise,
                                           D_mean = mean(D_KSgeneral),D_sd = sd(D_KSgeneral),
                                           p.valor_mean = mean(p.valor_KSgeneral),p.valor_sd = sd(p.valor_KSgeneral),
                                           n_nRef=sum(p.valor_KSgeneral>=0.05),
                                           .parallel = TRUE)
...
```{r}
library(KSgeneral)
df_auditoria %>% filter(SiteCode == "SCsjo1" & k==0.1 & MN=="EI") %>% head
df_SAD0bs <- read.csv(filter(df_auditoria,SiteCode == "PRtiba1")$SAD_obs.name[1],header = TRUE,as.is = TRUE)
df_SAD0bs$N %>% sum

df_SAD_MN <- read.csv(filter(df_auditoria,SiteCode == "PRtiba1" & MN=="EI" & k==0.99)$SAD_MN.name[1],header = TRUE,as.is = TRUE)
