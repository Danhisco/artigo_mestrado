...
```{r fig 42,eval=FALSE,echo=FALSE,fig.width=8, fig.height=27,warning=FALSE,message=FALSE}
df_plot <- df_U %>%
...
# U_med na escala padrão
lmer_U <- lmer(U_med ~ log10_Sobs + (1|SiteCode),data=df_U)
summary(lmer_U)
...
```{r}
l_md <- list()
l_md[[1]] <- gam(log10_U ~ log10_Sobs +
                 s(d.z,bs="tp",k=10) + s(p.z,bs="tp",k=10) + ti(d.z,p.z,bs=c("tp","tp"),k=c(10,10)) +
                 s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="tp",m=1,k=10),
                data = df_U, family=gaussian,method = "REML")
l_md[[2]] <- gam(log10_U ~ log10_Sobs +
                 s(d.z,bs="cr",k=10) + s(p.z,bs="cr",k=10) + ti(d.z,p.z,bs=c("cr","cr"),k=c(10,10)) +
                 s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="cr",m=1,k=10),
                data = df_U, family=gaussian,method = "REML")
# l_md[[3]] <- gam(log10_U ~ log10_Sobs +
#                  s(d.z,bs="cc",k=10) + s(p.z,bs="cc",k=10) + ti(d.z,p.z,bs=c("cc","cc"),k=c(10,10)) +
#                  s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="cc",m=1,k=10),
#                 data = df_U, family=gaussian,method = "REML")
# l_md[[4]] <- gam(log10_U ~ log10_Sobs +
#                  s(d.z,bs="ps",k=10) + s(p.z,bs="ps",k=10) + ti(d.z,p.z,bs=c("ps","ps"),k=c(10,10)) +
#                  s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="ps",m=1,k=10),
#                 data = df_U, family=gaussian,method = "REML")
# l_md[[5]] <- gam(log10_U ~ log10_Sobs +
#                  s(d.z,bs="gp",k=10) + s(p.z,bs="gp",k=10) + ti(d.z,p.z,bs=c("gp","gp"),k=c(10,10)) +
#                  s(SiteCode,bs="re") + s(d.z,by=SiteCode,bs="gp",m=1,k=10),
#                 data = df_U, family=gaussian,method = "REML")
...
l_md_log10.U_gamEstudo<- l_md
save(l_md_log10.U_gamEstudo,file="~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/l_md_log10.U_gamEstudo.Rdata")
```
...
```{r}
AICctab(l_md_log10.U_gamEstudo,weights=TRUE)
...
l_md.U_mdCheio <- dlply(df_U.class,"md_class",f_md)
```
...
md_U_GS <- gam(log10_U ~ offset(log10_Sobs) +
                 s(d.z,bs="cr",k=20,m=1) + s(p.z,bs="cr",k=20,m=1) + ti(d.z,p.z,bs=c("cr","cr"),k=c(20,20),m=1) +
                 s(d.z,SiteCode,bs="fs",m=1,k=20,xt = list("cr")),
                data = df_U, family=gaussian,method = "REML")
save(md_U_GS,file = "~/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/md_U_GS.Rdata")
...
#                 data = df_U, family=gaussian,method = "REML")
gratia::appraise(md_U_GS)


AICctab(md_U,md_U_sSobs,weights=T)
gratia::appraise(md_U_GS)
...
l_p[[1]] <- ggplot(df_diag, aes(x=p.z,y=.resid)) +
  geom_smooth(method="loess") +x
  geom_point(alpha=0.3)
l_p[[2]] <- ggplot(df_diag, aes(x=d.z,y=.resid)) +
  geom_smooth(method="loess") +
  geom_point(alpha=0.3)
grid.arrange(grobs=l_p,ncol=2,top="deviance residual ~ continuous predictors")
```
