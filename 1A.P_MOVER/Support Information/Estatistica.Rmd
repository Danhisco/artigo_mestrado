...
```{r auditoria U }
df_auditoria <- read.csv("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/df_auditoria",header = TRUE, as.is = TRUE)
ggplot(df_auditoria,aes(x=p,y=U_med)) + 
  geom_point() + 
  geom_smooth(method="lm") + 
  facet_wrap(~k_prop,ncol=4,scales = "free")
```
...
## numero de arquivos na pasta SADs_preditas: 392000
...
df_SAD.predita %<>% mutate(MN=str_match(SAD_MN.name,"EE|EI")[,1],
                           k=str_match(SAD_MN.name,"__k(.*?).E")[,2],
                           file.tag=gsub("__k","",str_match(SAD_MN.name,"ref(.*?)__k")[,1]),
                           rep=str_match(SAD_MN.name,"rep_(.*?).csv")[,2])
df_SAD.predita <- cbind(df_SAD.predita,do.call(rbind,strsplit(df_SAD.predita$file.tag,"_*_"))[,c(1,3)])
names(df_SAD.predita)[6:7] <- c("refID","ordem")
df_SAD.predita$refID <- gsub("ref","",df_SAD.predita$refID) 
# 
df_SAD.auditoria <- ddply(df_SAD.predita,c("MN","k","refID","ordem"),summarise,n_SAD.predita = length(SAD_MN.name))
df_SAD.auditoria %<>% select(refID,ordem,n_SAD.predita) %>% distinct()
#
df_SAD.auditoria$refID <- as.integer(df_SAD.auditoria$refID)
df_SAD.auditoria$ordem <- as.integer(as.character(df_SAD.auditoria$ordem))
df_auditoria %<>% left_join(x=.,
                            y=df_SAD.auditoria,
                            by=c("refID","ordem"))

df_auditoria %>% filter(is.na(n_SAD.predita)) %>% select(SiteCode) %>% distinct()
df_auditoria %<>% select(-k_prop)
write.csv(filter(df_auditoria,is.na(n_SAD.predita)),
          file="~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/df_sitios_nao_simulados.csv",
          row.names = FALSE)
```

Apenas 98 sítio foram simulados
