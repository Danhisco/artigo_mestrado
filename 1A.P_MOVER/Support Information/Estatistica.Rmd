...
i <- 1
md_name <- names(l_nRef__modeloCheio[v_glmerUpdate])[i] %>% 
...
__Tabela 2.1__  Ajuste obtido para os modelos após as rotinas de melhora    
    
```{r Tabela 21,eval=TRUE,cache=FALSE}
load("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/Support Information/l_md_nRef_GLMM.Rdata")
df_auditoria.md <- ldply(l_nRef__modeloCheio,f_warning,.id = "glmer") %>% 
  rename(warning_message = V1)
knitr::kable(df_auditoria.md)
...
```{r avaliacao por site fig 14,fig.width=8, fig.height=30,message=FALSE,eval=TRUE}
# Dados
...
```{r md_nRef2 e r2cm,echo=TRUE,cache=TRUE,eval=TRUE}
