...
# graficos
l_p <- list()
l_p[[1]] <- ggplot(df_plot,aes(x=k,y=modulo_diffS)) + 
  geom_boxplot(col="blue") +
  geom_jitter(alpha=0.3)
l_p[[2]] <- ggplot(df_plot,aes(x=k,y=n_nRef)) + 
  geom_boxplot(col="blue") +
  geom_jitter(alpha=0.3)
l_p[[3]] <- ggplot(df_plot,aes(y=modulo_diffS,x=n_nRef)) +
...
# dados
df_plot %<>% left_join(x=.,
                       y=ddply(.,"SiteCode",summarise,max_mod_diffS=max(modulo_diffS)),
                       by="SiteCode") %>% 
  arrange(max_mod_diffS) %>%
  mutate(title=paste0(SiteCode," max(|diffS|)=",round(max_mod_diffS,2) ) )
df_plot$title <- factor(df_plot$title,levels = unique(df_plot$title))
# graficos
df_plot %>% ggplot(aes(x=n_nRef,y=modulo_diffS)) +
  geom_point() +
  xlim(0,100) +
