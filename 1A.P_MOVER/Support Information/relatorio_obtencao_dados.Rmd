...
## SAD replica
df_EI <- map_df(Sys.glob("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/SADs_preditas/*EI*"), read_csv)
df_EE <- map_df(Sys.glob("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/SADs_preditas/*EE*"), read_csv)
names(df_EI)[1] <- "replica"
df_SADreplica <- bind_rows(df_EE,df_EI)
# df_SADreplica <- map_df(Sys.glob("~/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/SADs_preditas/*.csv"), read_csv)
df_SADreplica %<>% group_by(SiteCode) %>% nest %>% rename(SAD_MN=data)
## merge
df_resultados <- inner_join(x=df_SADreplica,
                            y=df_SAD.obs,
                            by="SiteCode")
f_resultados <- function(X){
  ...
  registerDoMC(3)
  df_return <- ddply(X$SAD_MN[[1]],c("replica","MN","k"),function(Y) f_KS(v_MN=Y$N),.parallel = TRUE)
}
