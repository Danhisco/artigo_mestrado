...
df_SAD.obs %<>% 
  ...
  ddply("SiteCode",summarise,
        Ntotal = sum(N), S_obs = length(species.correct), SAD_integer = f_summarise(N) )
...
df_dados <- rbind(df_filtro.UC,df_filtro.estate) %>% 
  distinct()
df_dados <- df_dados[df_dados$SAD_integer,]
```
...
```{r janela de codigo 2 _ scale of effect, echo=TRUE,results="hide"}
# dados brutos
...
raio_medio <- round(weighted.mean(x = df_averageSE$raio_paisagem,w = df_averageSE$weight),2)
```
