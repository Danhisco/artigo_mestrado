...
df_SAD.obs <- read.csv(file="/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/dados_brutos/abundances.csv",
                       header = TRUE,as.is = TRUE)
df_SAD.obs %<>% filter(SiteCode %in% unique(df_resultados_final$SiteCode))
...
for(i in 1:nrow(df_resultados)){
  ...
  f_KS <- function(v_obs=v_SADobs,v_MN,boots=300){
...
```{r figura 4,echo=FALSE,eval=TRUE,cache=TRUE}
#
...
```{r anexo 2 - criacao figuras,echo=FALSE,eval=FALSE}
# dados
df_resultados <- read.csv("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/resultados/df_resultados.csv",
                          header = TRUE,as.is = TRUE)
df_resultados$k <- factor(df_resultados$k,levels = unique(as.character(df_resultados$k))[20:1])
levels(df_resultados$k)[19] <- "0.10"
df_resultados %<>% mutate(title_quadriculo = paste0("MN=",MN," k=",k,"\n nRef=",n_nRef," diffS_mean=",round(diffS_mean,3)))
#

# versão antiga
for(s in 1:length(unique(df_resultados$SiteCode))){
  # s <- 1
  X <- df_resultados %>% filter(SiteCode == unique(df_resultados$SiteCode)[s])
  #
  v_SADobs <- rad(read.csv(X[1,"path_SADobs"],header = TRUE)[,"N"])
  f_function <- function(df_predito){
    # dados
    # df_predito <- df_[1,]
    df_SADpredita <- read.csv(df_predito$path_SADrep,header = TRUE,as.is = TRUE)
    # plot base
    plot(v_SADobs,main=df_predito$title_quadriculo,cex=0.5,
         xlim=c(1,head(df_$max_rank,1)))
    # abundância por replica
    d_ply(df_SADpredita,"replica",function(df) lines(rad(df$N),type="l",col=rgb(0.25, 0.25, 0.25, 0.25) ) )
    # abundância observada + titulo
    lines(v_SADobs,pch = 19, col="red",cex=0.5,bg="black",type="b")
  }
  # MNEE
  df_ <- X %>% filter(MN=="EE")
  png(paste0("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/figuras/",
             unique(df_resultados$SiteCode)[s],"_",df_$MN[1],"_SADs.png"),
      width = 980, height = 600)
  par(mfrow=c(4,5))
  a_ply(df_,1,f_function)
  dev.off()
  # MNEI
  df_ <- X %>% filter(MN=="EI")
  png(paste0("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/figuras/",
...
```{r anexo 2 plot das figuras,echo=FALSE,eval=TRUE}
#
library(imager)
#
df_plot <- Sys.glob("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/figuras/*SADs.png") %>% 
  gsub("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/figuras/","",.) %>% gsub("_SADs.png","",.) %>% 
  adply(.,1,function(x) unname( unlist( strsplit(x,"_",fixed = TRUE))),.id = NULL) %>% 
  rename(SiteCode = V1,MN = V2)
df_plot$png_path <- Sys.glob("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/simulacao/figuras/*SADs.png")
#
for(i in 1:nrow(df_plot)){
  print(paste0(df_plot$SiteCode[i]," MN=",df_plot$MN[i]))
  png_file <- load.image(df_plot$png_path[i])
  plot(png_file,axes=FALSE)
}
```
