...
<!-- i) preparar um df_references com as colunas de interesse e já considerando os dados disponíveis para ficar no zip do artigo -->


```{r janela de codigo 1}
### dados
...
```{r SAD obs e raster disponivel, echo=FALSE,eval=FALSE}
# SAD obs
...
df_SAD.obs %<>% filter(species.correct != "Mortas") %>% 
...
# merge
df_db <- inner_join(x = inner_join(x = df_ref.S_UCprotecaoIntegral,
                                   y = df_SAD.obs,
                                   by = "SiteCode"),
                    y = df_tif, 
                    by = c("ordem","refID"))
...
```{r separar SADs por df,echo=FALSE,eval=FALSE}
f_SADobs <- function(dados){
  data_ <- dados[[1]] %>% as.data.frame()
  path_SAD <- paste0("/home/danilo/Documentos/Doutorado/artigo_mestrado/1A.P_MOVER/dados_brutos/SAD.obs__ref",
                     head(data_$RefID,1),"_NA_",head(data_$ordem,1))
  df_ <- data_ %>% dplyr::select(species.correct,N)
  write.csv(df_, file = path_SAD, row.names = FALSE)
}
registerDoMC(2)
a_ply(df_db,1,function(X) f_SADobs(data_ = X$data),.parallel = TRUE)
```
...
```{r dados para df Scale of Effect,echo=FALSE,eval=FALSE}
# dados
...
```{r janela de codigo 2 _ scale of effect, echo=TRUE}
# dados brutos
...
raio_medio <- weighted.mean(x = df_averageSE$raio_paisagem,w = df_averageSE$weight)
```
...
f_mat.tri <- function(png, abund){ #png.file, número de indivíduos presente 
  ...
  binario.focal <- as.matrix( focal(raster_binario, janela, func_focal, pad=TRUE, padvalues = 0))Distância média de dispersão por k

Criamos funções no ambiente de programação R para estimar a distância média de dispersão necessária para que k propágulos permaneçam até a distância entre unidades adjacente de habitat (Apêndice X). A distância entre unidades adjacentes de habitat pode ser obtido por 100/sqrt(DA), onde DA = densidade observada (indivíduos/ha).
...
p_rm_ < rm_
 ...
 rm_ <- rm_ - p_rm_
# else 
 janela_observao[p_inicio[1]:p_final[1],p_inicio[2]:p_final[2]][1:rm_] <- 2Distância média de dispersão por k
...
>>>>>>> 706b934187ce0f1f47de33cdb69d55dcff8343e1
