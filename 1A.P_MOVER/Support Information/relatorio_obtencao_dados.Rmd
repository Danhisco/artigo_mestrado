...
l_p[[1]] <- ggplot(df_resultados,aes(x=p.value_mean,y=n_SAD.N.ref)) + geom_point() + 
  geom_smooth(method ="loess") + labs(y="número de SADs não refutadas",x="p-valor médio")
l_p[[2]] <- ggplot(df_resultados,aes(x=D_mean,y=p.value_mean)) + geom_point() + 
  geom_smooth(method ="loess") + labs(x="estatística D média",y="p-valor médio")
l_p[[3]] <- ggplot(df_resultados,aes(x=p.value_mean,y=p.value_var)) + geom_point() + geom_smooth(method="loess") + labs(y="variância p-valor",x="p-valor médio")
l_p[[4]] <- ggplot(df_resultados,aes(x=D_mean,y=D_var)) + geom_point() + geom_smooth(method="loess") +
