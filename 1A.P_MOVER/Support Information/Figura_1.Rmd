...
l_p[[1]] <- ggplot(df_plotEE,aes(x=p,y=d,fill=mod.avg.pred)) +
  ...
  labs(fill="Prob. of not refuting a predicted SAD") +
...
# l_p[[1]] <- ggdraw() + draw_label("Prob. of not refuting a predicted SAD")
a_prob_of_not_refuting <- plot_grid(a_SENM,b_SINM,ncol=2)
a_prob_of_not_refuting <- plot_grid(v_legend,a_prob_of_not_refuting,ncol=1,rel_heights = c(0.15,1))
