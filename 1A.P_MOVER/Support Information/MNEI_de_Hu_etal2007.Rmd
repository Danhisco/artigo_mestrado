...
```
```{r conversão de parâmetros,echo=FALSE,eval=TRUE}
...
l_p[[1]] <- df_resultados |> 
  ...
  labs(x="probabilidade m corrigida por p",y="taxa U",title="Aproxidamente, m >= U") +
  theme_classic()
l_p[[2]] <- df_resultados |> 
  ggplot(aes(x=m_,y=U_med)) +
  geom_bin2d(bins=70) +
...
A maior implicação de m >=U é que não se torna razoável negligenciar U sem negligenciar m.
...
```{r parametros para estudo da f_eq5,echo=FALSE,eval=FALSE}
# .JM <- 10^6
# .JL <- 10^4
# .U <- 10^(-5)
# .m <- 10^(-6)
...
__Figura 2__ Comparação da figura 3 de Hu et al. 2007 e aquela obtida pela versão do código em R. Nesse exemplo as abundâncias relativas abaixo de 0.03 não foram adiconados.

</br>
  
##### Uma dúvida:  
  
Como é possível em uma mesma amostra existir mais de uma espécie com abundância relativa de 0.99 ou maior?    
...
<!-- Porém, a equação 4 conserva o comportamento de U - não houve mudança qualitativa.  -->

<!-- </br> -->

### Eq.5 com os parâmetros estimados por MNEE

Por algum motivo há apenas 1032/2060 baterias de simulação. Não entendo o motivo.  
```{r dados para simulacao,echo=FALSE,eval=TRUE}
...
df_SAD.predita$status <- 1
df_resultados %<>% mutate(k=factor(k))
df_SAD.predita <- full_join(x=df_resultados,
                            y=select(df_SAD.predita,-MN),
                            by=c("SiteCode","k"))
# X <- df_SAD.predita %>% filter(path=="BAlenc4_k0.9_EE.csv")
```
```{r aplicar eq 5 usando as classes de abundância,echo=FALSE,eval=FALSE,cache=FALSE,error=FALSE}
f_ExpRich_EEeEI <- function(X){
  ...
  # save df_SADrep
  write_csv(df_SADrep,
            file=paste0("./simulacao_codigos_e_resultados/SADs_preditas/",
                        gsub("EE","EEeEI",x=X$path)))
  # summary
...
<!-- __Mensagem de Erro__   -->

<!-- Error in integrate(f2, 1/JM, 1) : the integral is probably divergent -->

</br>

```{r auditoria de EEeEI.csv, echo=FALSE,eval=FALSE}
# dados
df_EEeEI <- data.frame(path_EEeEI=
                         list.files(path = "./simulacao_codigos_e_resultados/SADs_preditas/",
                                                    pattern = "EEeEI.csv")) |> 
  extract(path_EEeEI,c("SiteCode","k"),"^([[:alnum:]]+)_k(0.[0-9]*)_[a-zA-Z]*.csv$",remove=FALSE) %>% 
  mutate(k=factor(k))
# estatísticas de resumo
f_summary <- function(X){
  df_SADrep <- read_csv(file = paste0("./simulacao_codigos_e_resultados/SADs_preditas/",X$path_EEeEI),
                        show_col_types = FALSE)
  ddply(df_SADrep,"replica",summarise,
        S_EE=sum(EE),S_EI=sum(EI),
        x_max=max(x)) %>% select(-replica) %>%
    apply(.,2,mean) %>% t %>% as.data.frame()
}
registerDoMC(2)
df_plot <- adply(df_EEeEI,1,f_summary,.parallel = TRUE)
# join
df_auditoria <- full_join(x=df_SAD.predita,
                          y=select(df_plot,-path_EEeEI),
                          by=c("SiteCode","k"))
df_auditoria$status[is.na(df_auditoria$status)] <- 0
write_csv(df_auditoria,file = "./resultados_e_SI/df_auditoria_HUetal.csv")
```
```{r SADs EE disponiveis e SAD EI integraveis,echo=FALSE,eval=TRUE,cache=TRUE,fig.width=10,fig.height=4}
df_auditoria <- read_csv("./resultados_e_SI/df_auditoria_HUetal.csv",show_col_types = FALSE)
# gráficos
l_p <- list()
l_p[[1]] <- df_auditoria %>% mutate(status=factor(status)) %>% 
  ggplot(aes(x=p,y=d,color=status)) +
  geom_point(alpha=0.4) +
  scale_color_manual(values=c("red","blue")) +
  labs(title = "0 = sem SAD EE")
l_p[[2]] <- df_auditoria %>% 
  filter(status!=0) |> 
  mutate(status2=factor(ifelse(is.na(S_EI),0,1))) %>% 
  ggplot(aes(x=U_med,y=m_,color=status2)) +
  geom_point(alpha=0.4) +
  labs(title = "0 = sem SAD EI, erro de integração") +
  scale_color_manual(values=c("red","blue"))
  # scale_y_continuous(trans="log10") +
  # scale_x_continuous(trans="log10")
gridExtra::grid.arrange(grobs=l_p,ncol=2)
...
__Figura 3__ Painél da esquerda: Sítios e cenários de limitação de dispersão em que está disponível a SAD do modelo espacialmente explícito (SAD EE). Painél da direita: combinação de parâmetros onde o modelo integrou. O erro de integração foi: "Error in integrate(f2, 1/JM, 1) : the integral is probably divergent".  

</br>


```{r comparacao riqueza media EE e EI,echo=FALSE,eval=TRUE}
df_auditoria %>% 
  filter(!is.na(S_EI)) %>% 
  pivot_longer(S_EE:S_EI,names_to = "S_MN",values_to = "S_value") %>% 
  ggplot(aes(x=x_max,y=S_value,color=S_MN)) +
    geom_point(alpha=0.5) +
  labs(title="riqueza predita e abundância relativa máxima",x="Ab. Rel. Max.",y="riqueza predita") +
  scale_y_continuous(trans="log10")
```
  
</br>

__Figura 4__ Riqueza predita (S_MN) e a  maior abundância relativa de cada SAD de EE (eixo x). Usamos as classes de abundância relativa simulados por MNEE para calcular o número médio de espécies naquela classe segundo o modelo de Hu et al. 2007. Então somei o número esperado em cada classe (eixo y). As predições do modelo de Hu et al. (2007) são várias ordens de magnitude maiores do que as predições da simulação coalescente, que erra pouco a riqueza observada. S_EE = modelo espacialmente explícito, S_EI = modelo espacialmente implícito.
...
```{r eq 4 e figura de phi de x com abund relat completa,echo=FALSE,eval=FALSE}
######
# equação 4
