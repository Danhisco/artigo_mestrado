...
```{r gráficos,include=FALSE,eval=FALSE,echo=FALSE,message=FALSE,warning=FALSE}
#
...
# gráfico 
###
df_S1$Unidade_de_conservacao[is.na(df_S1$Unidade_de_conservacao)] <- "no info"
df_S1$Unidade_de_conservacao[df_S1$Unidade_de_conservacao=="private_in_APA"] <- "priv. APA"
df_S1$Unidade_de_conservacao[df_S1$Unidade_de_conservacao=="universities and research centers"] <- "res. cent."
df_S1$Unidade_de_conservacao[df_S1$Unidade_de_conservacao=="UC Uso Sustentavel"] <- "UC Sust."
df_S1$Unidade_de_conservacao[df_S1$Unidade_de_conservacao=="UC Protecao Integral"] <- "UC Int."
df_S1$forest_succession[df_S1$forest_succession=="capoeira"] <- "<=20 y"
df_S1$forest_succession[df_S1$forest_succession=="secondary"] <- "<=20 y"
df_S1$forest_succession[df_S1$forest_succession=="primary/secondary"] <- "]20,80[ y"
df_S1$forest_succession[df_S1$forest_succession=="primary"] <- ">=80 y"
# df_S1 %>% .$forest_succession %>% unique
df_S1 %>% #.$Unidade_de_conservacao %>% unique
  select(Unidade_de_conservacao,forest_succession) %>% 
  mutate(Uso_Solo = factor(Unidade_de_conservacao,
                           levels=unique(.$Unidade_de_conservacao)[c(6:5,3,2,4,1)]),
         Forest_Age = factor(forest_succession,
                             levels=unique(.$forest_succession)[3:1]),
         count=1) %>% 
  # select(Uso_Solo,Forest_Age) %>% 
  # table
  group_by(Uso_Solo,Forest_Age,.drop = FALSE) %>% 
  summarize(Freq=n()) %>% 
  ggplot(aes(x=Uso_Solo,y=Forest_Age,fill=Freq)) +
  geom_tile(color = "black",alpha=0.5) +
  geom_text(aes(label = Freq), color = "black", size = 3.5) +
  scale_fill_gradient(low = "white", high = "green") +
  labs(x="",y="",fill="",title="Patch Scale Information: Age and Use") +
  theme(legend.position="none",
...
df_S2 %>%
  ...
  theme(axis.text.y=element_blank(),
        axis.ticks.y=element_blank())
df_S2$Unidade_de_conservacao %>% table 
  # ggplot(aes(x=Unidade_de_conservacao,y=forest_succession)) +
  # geom_boxplot() +
  # geom_jitter()
```
