...
Te mando o exemplo mínimo que corresponde a simulação da SAD observada em uma parcela para os 20 cenários de limitação à dispersão no nível dos indivíduos. Para estimar U usamos 10 réplicas por combinação única de parâmetros (bateria de simulação), pois observamos variação muito pequena entre réplicas (tabela 1).  
     
__tabela 1__ As 11 primeiras linhas (de 200) do data frame usada para guiar a simulação coalescente usada para estimar a taxa de especiação média necessária para obter a riqueza observada (U) no exemplo mínimo. k = proporção de propágulos que permanece até a distância de dois indivíduos vizinhos imediatos; S_obs =  número de espécies observado na parcela amostrada; kernel_code = código do tipo de kernel de dispersão usado na simulação coalescente, 2 indica distribuição de Laplace; d = distância em metros usado para parametrizar o desvio padrão da função de dispersão; txt.name = caminho do arquivo .txt que contém o mapa de cobertura vegetal com a resolução ajustada tal que a densidade de pixels seja igual a densidade de indivíduos na parcela; U = coluna que irá armazenar a estimativa da taxa de especiação necessária para obter a riqueza observada; .n = identificador da réplica, são 10 réplicas por bateria de simulação.  
...
```{r media de U por bateria de simulacao, echo=TRUE,eval=FALSE,tidy=TRUE}
df_U <- map_df(Sys.glob("./U/*.csv"), read_csv)
df_simulacao <- 
  inner_join(x=distinct(dplyr::select(df_U,
                                      SiteCode, k, Ntotal, S_obs, kernel_code, DA, d, p, txt.name)),
