...
df_dadosTreeCo <- read.csv(file="/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/dados_para_iniciar_artigo/references_TreeCo.csv",
  ...
  mutate(ID = NA) %>% 
   dplyr::select(ID,state,SiteCode,year_data,year,effort_ha,
                 ...
                 forest_disturbance,forest_succession,
                 Reference,refID)
...
```{r Sitios de amostragem forest age e uso do solo, fig.height=4,fig.width=10}
# dados
...
df_tabelaSitios$forest_succession[df_tabelaSitios$forest_succession=="primary"] <- ">=80 y"
df_tabelaSitios$forest_succession[df_tabelaSitios$forest_succession=="primary/secondary"] <- "[50;80[ y"
df_tabelaSitios$forest_succession[df_tabelaSitios$forest_succession=="secondary"] <- "[20;50[ y"
df_tabelaSitios$forest_succession[df_tabelaSitios$forest_succession=="capoeira"] <- "<20 y"
...
l_p[[1]] <- df_tabelaSitios %>% 
  select(Unidade_de_conservacao,forest_succession) %>% 
  table %>% as.data.frame() %>% 
  ggplot(aes(x=Unidade_de_conservacao,y=forest_succession,fill=Freq)) +
  geom_tile() +
  geom_text(aes(x=Unidade_de_conservacao,y=forest_succession,label=Freq)) +
  labs(x="land use",y="Time since last perturbation") +
  theme(legend.position = "none")
l_p[[2]] <- df_tabelaSitios %>% 
  mutate(frag_area = as.numeric(frag_area)) %>%  
...
# df_tabelaSitios %>% str
df_tabelaSitios %>% 
  filter(Unidade_de_conservacao %in% c("UC Protecao Integral","universities and research centers")) %>% 
  .$UC_area_ha
```
