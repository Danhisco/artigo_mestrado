...
# write.csv(df_dados,
#           "/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/dados_para_iniciar_artigo/df_dados_disponiveis.csv",
#           row.names = FALSE)
```
...
### dados
df_dados.brutos <- read.csv("/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/dados_para_iniciar_artigo/df_SADs_disponiveis.csv",
                     header = TRUE,as.is = TRUE)
### filtro geral
...
df_dados <- rbind(df_filtro.UC,df_filtro.estate) %>% 
  distinct()
### atualizando referencias de pastas
...
```{r transformar as SADs preditas em classes de abundância e contabilizar as classes }
# dados para simular:
...
# df_SAD.predita$MN |> table()
# df_SAD.predita$SiteCode |> unique() %>% length()
# df_resultados$SiteCode %>% unique %>% as.character() %>% sort == df_SAD.predita$SiteCode %>% unique %>% as.character() %>% sort
# df_resultados %>% filter(!SiteCode %in% unique(df_SAD.predita$SiteCode)) %>% .$SiteCode %>% unique
...
f_ExpRich_EEeEI <- function(X){
  ...
  # summary
  df_SADrep %>% ddply(.,"replica",summarise,
