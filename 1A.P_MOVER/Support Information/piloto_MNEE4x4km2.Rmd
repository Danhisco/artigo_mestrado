...
Anteriormente eu havia rodado as simulações em paisagens de 10.2 x 10.2 km2, porém o sugerido pela análise de escala de efeito é 4.07 x 4.07 km2 (Fig. 1a). Em média, a redução da extensão espacial da paisagem aumentou a porcentagem de cobertura vegetal na paisagem, apesar de reduzi-la em alguns casos, modificando a ordenação dos sítios ao longo do gradiente de cobertura vegetal (Fig 1b). Para avaliar se as predições de MNEE são robustas a essa mudança selecionei alguns pontos para fazer um piloto (Fig 1b, pontos vermelhos). Conclui que as mudanças não eram significantes (Figs) e segui atualizando os modelos estatístiscos com a nova ordenação dos sítios ao longo do gradiente. Observei melhora no ajuste do modelo estatístico, com aumento dos coeficientes de determinação (principalmente o condicional), sem mudança nos padrões qualitativos tanto na mediana quanto nos percentis de 5% e 95% (Figs ) 
...
df_sims <- read.csv(file = "/home/danilo/Documentos/artigo_mestrado/1A.P_MOVER/Support Information/df_piloto.csv",
                    header = TRUE)
df_plot <- df_sims[3,] %>% 
  select(SiteCode,txt.name,txt.name_antigo) #%>% 
...
df_esforcoComputacional <- data.frame(item = c("Sim. U","Sim. SADs","teste KS"),
                                      min = c(73,80,18)) %>% 
  mutate(min_todosSitios = min *103/23,horas = min_todosSitios/60) %>% select(-min_todosSitios)
kable(df_esforcoComputacional,
      caption = "Uso Computacional observado (n=23) e estimado (n=103)",
      col.names = c("","min", "horas estimadas"))
```
...
```{r comparacao U,warning=FALSE,message=FALSE,eval=TRUE,include=TRUE,echo=FALSE,fig.height=4,fig.width=8}
...
```{r comparacao SADs preditas congruentes,fig.height=4,fig.width=8,eval=TRUE,include=TRUE}
...
```{r graficos relacao diff Respostas e diff p, echo=FALSE,eval=TRUE,include=TRUE,fig.height=5,fig.width=10}
select <- dplyr::select
# dados
...
__Fig. __ Gráficos exploratórios da mudança nas variáveis respostas (eixo y) e da variável resposta proporção de cobertura vegetal (eixo x). A linha azul é feita pelo geom_smooth(method="sloess").    
...
```{r  Comparacao predito modelo medio entre escalas, fig.height=5.5, fig.width=9}
...
__Fig. __ Comparação predição pelo modelo médio. 
