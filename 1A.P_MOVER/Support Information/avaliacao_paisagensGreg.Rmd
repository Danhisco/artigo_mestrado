...
```{r padronizacao dos recortes de paisagem para o formato de TreeCo,eval=FALSE}
df_tif
...
```{r padronização da extensão do raster,fig.width=10,fig.height=4.1,eval=FALSE}
# função
f_crop <- function(df){
...
```{r auditoria padronizacao das paisagens,eval=FALSE}
df_aud <- data.frame(tif.name = list.files(path="./dads_para_iniciar_artigo/paisagens_mapbiomas6_padronizadas/",
...
```{r salvar todas as paisagens em formato png,eval=FALSE}
df_tif_aud <- data.frame(tif.name=list.files(path = "./dados_para_iniciar_artigo/paisagens_mapbiomas6_padronizadas",
                                             pattern = ".tif",full.names = T)) |> 
  mutate(SiteCode = str_extract(tif.name,"(?<=padronizadas\\/).*?(?=\\.tif)"))
# leitura padronizada de rasters
f_ggplotRaster <- function(df){
  r <- raster(df$tif.name) |> 
    as.data.frame(xy=TRUE) |> 
    drop_na() |> 
    rename(value=3) |> 
    mutate(label=df$SiteCode)
  ggplot(r) +
    geom_tile(aes(x=x,y=y,fill=value)) +
    scale_fill_gradientn(colours = rev(terrain.colors(10))) +
    coord_cartesian(expand = FALSE) +
    facet_wrap(~label) +
    theme_bw() + labs(x="",y="") + theme(legend.position="none")
  ggsave(paste0("./dados_para_iniciar_artigo/paisagens_mapbiomas6_padronizadas/png/",df$SiteCode,".png"))
}
registerDoMC(2)
a_ply(df_tif_aud,1,f_ggplotRaster,.parallel = TRUE)
```
```{r plotar todas as paisagens em formato png,echo=FALSE,eval=TRUE,out.width="25%"}
# f_png <- function(path) rasterGrob(as.raster(readPNG(path)),interpolate=F) 
# l_png <- alply(list.files(path = "./dados_para_iniciar_artigo/paisagens_mapbiomas6_padronizadas/png/",pattern = ".png",full.names = T),
#                1,f_png)
# grid.arrange(grobs=l_png,ncol=4)
l_png.path <- list.files(path = "./dados_para_iniciar_artigo/paisagens_mapbiomas6_padronizadas/png/",pattern = ".png",full.names = T)
# knitr::include_graphics(l_png.path[[1]])
teste <- readPNG(l_png.path[[1]])
plot(teste)
p <- grid.raster(teste)
